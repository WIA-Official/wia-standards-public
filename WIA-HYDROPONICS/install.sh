...
#######################################################
# WIA-HYDROPONICS Installation Script
# Version: 1.0.0
...
print_message "$BLUE" "======================================"
print_message "$BLUE" "  WIA-HYDROPONICS Installation Script"
...
# Make CLI executable
print_message "$BLUE" "\nConfiguring CLI..."
if [ -f "cli/wia-hydroponics.sh" ]; then
    chmod +x cli/wia-hydroponics.sh
    print_message "$GREEN" "CLI made executable"
else
    print_message "$YELLOW" "Warning: CLI script not found at cli/wia-hydroponics.sh"
fi
...
# Test CLI
print_message "$BLUE" "\nTesting CLI..."
if ./cli/wia-hydroponics.sh --version &> /dev/null; then
...
print_message "$NC" "  2. Run CLI: ./cli/wia-hydroponics.sh --help"
print_message "$NC" "  3. Use SDK:"
echo ""
print_message "$NC" "     import { WIADeFiSDK } from '@wia/hydroponics-sdk';"
print_message "$NC" "     const sdk = new WIADeFiSDK({ apiKey: '...', network: 'mainnet' });"
echo ""
print_message "$NC" "Documentation:"
print_message "$NC" "  - Spec: ./spec/WIA-HYDROPONICS-PHASE1.md"
