...
################################################################################
# WIA-DEF-010: Military Satellite - Installation Script
#
# @version 1.0.0
# @license MIT
...
#
# This script installs the WIA-DEF-010 standard components:
# - CLI tool
# - TypeScript SDK
# - Documentation
################################################################################
...
print_header() {
    echo -e "${SLATE}"
    echo "╔════════════════════════════════════════════════════════════════╗"
    echo "║      🛰️  WIA-DEF-010: Military Satellite Installer           ║"
    echo "║                      Version 1.0.0                             ║"
    echo "╚════════════════════════════════════════════════════════════════╝"
    echo -e "${RESET}"
}
...
install_cli() {
    ...
    local cli_path="$SCRIPT_DIR/cli/wia-def-010.sh"
    ...
    if [ -w "$install_dir" ]; then
        cp "$cli_path" "$install_dir/wia-def-010"
        print_success "Installed CLI to $install_dir/wia-def-010"
    else
        print_info "Installing to $install_dir requires sudo..."
        sudo cp "$cli_path" "$install_dir/wia-def-010"
        print_success "Installed CLI to $install_dir/wia-def-010 (with sudo)"
    fi

    # Verify installation
    if command -v wia-def-010 &> /dev/null; then
        print_success "CLI tool is now available as 'wia-def-010'"
        print_info "Try: wia-def-010 --help"
    else
...
install_docs() {
    ...
    local docs_dir="$HOME/.wia/def-010/docs"
...
run_tests() {
    ...
    if command -v wia-def-010 &> /dev/null; then
        ...
        wia-def-010 version &> /dev/null && print_success "CLI test passed" || print_error "CLI test failed"
    fi
...
show_completion() {
    ...
    echo ""
    echo -e "${GREEN}The WIA-DEF-010 standard has been successfully installed!${RESET}"
    echo ""
    echo "Available commands:"
    echo -e "${CYAN}  wia-def-010 calc-orbit${RESET}        - Calculate orbital parameters"
    echo -e "${CYAN}  wia-def-010 validate-link${RESET}     - Validate communication link"
    echo -e "${CYAN}  wia-def-010 track${RESET}             - Track satellite position"
    echo -e "${CYAN}  wia-def-010 generate-keys${RESET}     - Generate encryption keys"
    echo -e "${CYAN}  wia-def-010 calc-imaging${RESET}      - Calculate imaging parameters"
    echo -e "${CYAN}  wia-def-010 help${RESET}              - Show help message"
    echo ""
    echo "Quick start:"
    echo -e "${GRAY}  # Calculate orbital parameters for LEO satellite${RESET}"
    echo -e "${CYAN}  wia-def-010 calc-orbit --altitude 550000 --type leo${RESET}"
    echo ""
    echo -e "${GRAY}  # Validate communication link${RESET}"
    echo -e "${CYAN}  wia-def-010 validate-link --sat SAT-001 --station GS-01 --frequency 8e9${RESET}"
    echo ""
    echo -e "${GRAY}  # Track satellite position${RESET}"
    echo -e "${CYAN}  wia-def-010 track --sat SAT-001 --duration 90${RESET}"
    echo ""
    echo "TypeScript SDK:"
    echo -e "${GRAY}  import { calculateOrbitalParameters } from '@wia/def-010';${RESET}"
    echo ""
    echo "Documentation:"
    echo -e "${GRAY}  $HOME/.wia/def-010/docs/${RESET}"
    echo ""
