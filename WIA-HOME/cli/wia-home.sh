...
#
# Comprehensive CLI for interacting with Home protocols
################################################################################
...
readonly API_BASE="${WIA_DEFI_API_BASE:-https://api.wia-home.org/v1}"
readonly CONFIG_FILE="${HOME}/.wia-home/config.json"
...
load_api_key() {
  ...
  if [[ -z "${WIA_API_KEY}" ]]; then
    error "API key not found. Set WIA_API_KEY environment variable or run: wia-home config set-key YOUR_API_KEY"
  fi
}
...
cmd_config() {
  ...
  case "${subcommand}" in
    set-key)
      local api_key="${2:-}"
      if [[ -z "${api_key}" ]]; then
        error "Usage: wia-home config set-key YOUR_API_KEY"
      fi
    ...
    show)
      ...
      ;;
    *)
      echo "Usage: wia-home config {set-key|show}"
      exit 1
      ;;
  esac
}
  ...
  case "${subcommand}" in
    ...
    info)
      ...
      if [[ -z "${protocol_id}" ]]; then
        error "Usage: wia-home protocol info PROTOCOL_ID"
      fi
      ...
      ;;
    *)
      echo "Usage: wia-home protocol {list|info}"
      exit 1
      ;;
  esac
}
...
cmd_pool() {
  ...
  case "${subcommand}" in
    ...
    info)
      local pool_id="${2:-}"
      if [[ -z "${pool_id}" ]]; then
        error "Usage: wia-home pool info POOL_ID"
      fi
    ...
    top)
      ...
      ;;
    *)
      echo "Usage: wia-home pool {list|info|top} [PROTOCOL]"
      exit 1
      ;;
  esac
}
  ...
  case "${subcommand}" in
    quote)
      ...
      if [[ -z "${token_in}" ]] || [[ -z "${token_out}" ]] || [[ -z "${amount_in}" ]]; then
        error "Usage: wia-home swap quote CHAIN_ID TOKEN_IN TOKEN_OUT AMOUNT_IN [SLIPPAGE]"
      fi
    ...
    recent)
      local pool_id="${2:-}"
      if [[ -z "${pool_id}" ]]; then
        error "Usage: wia-home swap recent POOL_ID"
      fi
      ...
      ;;
    *)
      echo "Usage: wia-home swap {quote|recent}"
      exit 1
      ;;
  esac
}
...
cmd_stake() {
  ...
  case "${subcommand}" in
    ...
    info)
      local pool_id="${2:-}"
      if [[ -z "${pool_id}" ]]; then
        error "Usage: wia-home stake info POOL_ID"
      fi
      ...
      ;;
    *)
      echo "Usage: wia-home stake {list|info}"
      exit 1
      ;;
  esac
}
...
cmd_lend() {
  ...
  case "${subcommand}" in
    ...
    info)
      local market_id="${2:-}"
      if [[ -z "${market_id}" ]]; then
        error "Usage: wia-home lend info MARKET_ID"
      fi
      ...
      ;;
    *)
      echo "Usage: wia-home lend {markets|info} [PROTOCOL]"
      exit 1
      ;;
  esac
}
...
cmd_user() {
  ...
  case "${subcommand}" in
    positions)
      local address="${2:-}"
      if [[ -z "${address}" ]]; then
        error "Usage: wia-home user positions ADDRESS"
      fi
      ...
      ;;
    *)
      echo "Usage: wia-home user {positions} ADDRESS"
      exit 1
      ;;
  esac
}
...
cmd_token() {
  ...
  case "${subcommand}" in
    search)
      local query="${2:-}"
      if [[ -z "${query}" ]]; then
        error "Usage: wia-home token search QUERY [CHAIN_ID]"
      fi
    ...
    info)
      ...
      local chain_id="${3:-1}"
      if [[ -z "${address}" ]]; then
        error "Usage: wia-home token info ADDRESS [CHAIN_ID]"
      fi
      ...
      ;;
    *)
      echo "Usage: wia-home token {search|info}"
      exit 1
      ;;
  esac
}
...
cmd_help() {
  ...
  cat << EOF
${CYAN}Usage:${NC}
  wia-home [command] [subcommand] [arguments]
...
${CYAN}Examples:${NC}
  # Configure API key
  wia-home config set-key YOUR_API_KEY

  # List all protocols
  wia-home protocol list

  # Get swap quote
  wia-home swap quote 1 \\
  ...
  # Get user positions
  wia-home user positions 0x742d35Cc6634C0532925a3b844Bc9e7595f0bEb

${CYAN}Environment Variables:${NC}
  WIA_API_KEY         API key for authentication
  WIA_DEFI_API_BASE   API base URL (default: https://api.wia-home.org/v1)
...
main() {
  ...
  case "${command}" in
    ...
    version|--version|-v)
      ...
      ;;
    *)
      error "Unknown command: ${command}. Run 'wia-home help' for usage."
      ;;
  esac
}
