...
################################################################################
# WIA-AUTO-020: Maglev Train - Installation Script
#
# @version 1.0.0
# @license MIT
...
#
# This script installs the WIA-AUTO-020 standard components:
# - CLI tool
# - TypeScript SDK
# - Documentation
################################################################################
...
print_header() {
    echo -e "${ORANGE}"
    echo "╔════════════════════════════════════════════════════════════════╗"
    echo "║         🚄 WIA-AUTO-020: Maglev Train Installer              ║"
    echo "║                      Version 1.0.0                             ║"
    echo "╚════════════════════════════════════════════════════════════════╝"
    echo -e "${RESET}"
}
...
install_cli() {
    ...
    local cli_path="$SCRIPT_DIR/cli/wia-auto-020.sh"
    ...
    if [ -w "$install_dir" ]; then
        cp "$cli_path" "$install_dir/wia-auto-020"
        print_success "Installed CLI to $install_dir/wia-auto-020"
    else
        print_info "Installing to $install_dir requires sudo..."
        sudo cp "$cli_path" "$install_dir/wia-auto-020"
        print_success "Installed CLI to $install_dir/wia-auto-020 (with sudo)"
    fi

    # Verify installation
    if command -v wia-auto-020 &> /dev/null; then
        print_success "CLI tool is now available as 'wia-auto-020'"
        print_info "Try: wia-auto-020 --help"
    else
...
install_docs() {
    ...
    local docs_dir="$HOME/.wia/auto-020/docs"
...
run_tests() {
    ...
    if command -v wia-auto-020 &> /dev/null; then
        ...
        wia-auto-020 version &> /dev/null && print_success "CLI test passed" || print_error "CLI test failed"
    fi
...
show_completion() {
    ...
    echo ""
    echo -e "${GREEN}The WIA-AUTO-020 Maglev Train standard has been successfully installed!${RESET}"
    echo ""
    echo "Available commands:"
    echo -e "${CYAN}  wia-auto-020 calc-levitation${RESET}   - Calculate magnetic levitation force"
    echo -e "${CYAN}  wia-auto-020 design-guideway${RESET}   - Design guideway specifications"
    echo -e "${CYAN}  wia-auto-020 optimize-power${RESET}    - Optimize power consumption"
    echo -e "${CYAN}  wia-auto-020 simulate${RESET}          - Simulate maglev operation"
    echo -e "${CYAN}  wia-auto-020 calc-braking${RESET}      - Calculate braking distance"
    echo -e "${CYAN}  wia-auto-020 help${RESET}              - Show help message"
    echo ""
    echo "Quick start:"
    echo -e "${GRAY}  # Calculate levitation force${RESET}"
    echo -e "${CYAN}  wia-auto-020 calc-levitation --flux 1.5 --area 0.25 --gap 0.01${RESET}"
    echo ""
    echo -e "${GRAY}  # Design guideway for 600 km/h${RESET}"
    echo -e "${CYAN}  wia-auto-020 design-guideway --speed 600 --radius 12000${RESET}"
    echo ""
    echo -e "${GRAY}  # Optimize power for Tokyo-Osaka route${RESET}"
    echo -e "${CYAN}  wia-auto-020 optimize-power --distance 438 --speed 450 --passengers 1000${RESET}"
    echo ""
    echo "TypeScript SDK:"
    echo -e "${GRAY}  import { calculateLevitationForce } from '@wia/auto-020';${RESET}"
    echo ""
    echo "Documentation:"
    echo -e "${GRAY}  $HOME/.wia/auto-020/docs/${RESET}"
    echo ""
