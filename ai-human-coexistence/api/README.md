# WIA AI Human Coexistence — API SDK

Human detection, zone management, and safety arbitration for shared AI/human spaces

This directory provides a publishable API contract and language SDKs for
implementations conforming to WIA standard `ai-human-coexistence`.

## Layout

```
api/
  openapi-3.0.yaml          # OpenAPI 3.0.3 contract (production: https://api.wia.live/ai-human-coexistence/v1)
  sdk/
    typescript/             # @wia/ai-human-coexistence — TypeScript SDK
    python/                 # wia-ai-human-coexistence  — Python SDK
```

## Resources

The contract defines REST collections for the following resources:

- `Detection` — `/detections`
- `Zone` — `/zones`
- `Incident` — `/incidents`
- `Policy` — `/policies`


## Authentication

- `X-API-Key` header (preferred for service-to-service)
- OAuth 2.0 authorization-code flow at `https://auth.wia.live/oauth/authorize`

## License

MIT — see repository root.

弘益人間 (홍익인간) · Benefit All Humanity
