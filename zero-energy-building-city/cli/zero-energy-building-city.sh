#!/usr/bin/env bash
# zero-energy-building-city — Zero Energy Building and City CLI helper.
#
# Sample envelope generators for the WIA-ENE-ZEC-001 standard.
# Requires: bash 4+, jq.
set -euo pipefail

STD_ID="WIA-ENE-ZEC-001"
SLUG="zero-energy-building-city"
...
cmd_manifest() {
    jq -n --arg std "$STD_ID" --arg slug "$SLUG" '{
        standard_id: $std,
        slug: $slug,
        title: "Zero Energy Building and City",
        title_ko: "제로에너지 빌딩·도시",
        version: "1.0.0",
...
cmd_refs() {
    cat <<REFS
Normative references for WIA-ENE-ZEC-001 (Zero Energy Building and City):
