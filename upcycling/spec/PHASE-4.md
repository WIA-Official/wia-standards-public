...
```bash
# IFC to WIA
wia-ifc-convert --input product.ifc --output project.json

# WIA to IFC
wia-ifc-convert --input project.json --output product.ifc
...
## Lifecycle, Reporting & Public Disclosure (Phase 4 Closing)

### Public Disclosure

A deployment claiming WIA Upcycling Phase 4 conformance publishes, at minimum, the following on a public-facing page:

- the conformance manifest summary (active scope, declared service classes, accountable parties),
- the lifecycle audit retention policy,
- the complaint pathway and the named accountable person,
- the most recent annual report covering operations, material throughput summary, and any material non-conformities discovered and remediated during the year.

The public disclosure is a first-class affordance, not buried behind several clicks. A deployment that hides its disclosure is non-conformant in transparency, even if every individual metric is met.

### Annual Reporting Discipline

The annual report is a structured document. Its minimum sections include:

1. **Operations summary.** Total intake by material class, total outbound by product class, the upcycling ratio (the share of intake that resulted in higher-value outbound by the standard's definition), the residue ratio (the share that became lower-value or waste).
2. **Quality summary.** Lots tested, lots passed, lots remediated, lots discarded, with rationale for the discarded population.
3. **Resource summary.** Energy and water totals, with the reconciliation against utility meters.
4. **Audit summary.** Non-routine events of the year, broken down by category, with remediation status.
5. **Accessibility & accountability.** Complaints received, complaints resolved, time-to-resolution distribution.
6. **Forward declaration.** Material changes anticipated in the coming year and their re-commissioning plan.

A report missing any required section is itself a non-conformance.

### Lifecycle Audit Retrieval

The full lifecycle audit — manifest, calibration, conformance test results, audit chain, annual reports — must be retrievable on lawful request. A deployment unable to produce its lifecycle audit is non-conformant by omission, regardless of how well its day-to-day operations have run.

### Decommissioning

A deployment leaving the WIA Upcycling program is decommissioned by:

1. publishing a notice of decommissioning,
2. retiring the public-facing complaint pathway (with a redirection horizon for complaints in flight),
3. signing the final lifecycle audit and forwarding it to the WIA registry,
4. revoking platform credentials from the audit endpoint.

A deployment that exits silently — leaving its public-facing claim active without backing operations — has converted its prior conformance into a misrepresentation. The standard treats silent exit as a serious finding.
