...
################################################################################
# WIA-QUA-002: Quantum Algorithm - Installation Script
#
# @version 1.0.0
# @license MIT
...
#
# This script installs the WIA-QUA-002 standard components:
# - CLI tool
# - TypeScript SDK
# - Documentation
################################################################################
...
print_header() {
    echo -e "${INDIGO}"
    echo "╔════════════════════════════════════════════════════════════════╗"
    echo "║      🔢 WIA-QUA-002: Quantum Algorithm Installer              ║"
    echo "║                      Version 1.0.0                             ║"
    echo "╚════════════════════════════════════════════════════════════════╝"
    echo -e "${RESET}"
}
...
install_cli() {
    ...
    local cli_path="$SCRIPT_DIR/cli/wia-qua-002.sh"
    ...
    if [ -w "$install_dir" ]; then
        cp "$cli_path" "$install_dir/wia-qua-002"
        print_success "Installed CLI to $install_dir/wia-qua-002"
    else
        print_info "Installing to $install_dir requires sudo..."
        sudo cp "$cli_path" "$install_dir/wia-qua-002"
        print_success "Installed CLI to $install_dir/wia-qua-002 (with sudo)"
    fi

    # Verify installation
    if command -v wia-qua-002 &> /dev/null; then
        print_success "CLI tool is now available as 'wia-qua-002'"
        print_info "Try: wia-qua-002 --help"
    else
...
install_docs() {
    ...
    local docs_dir="$HOME/.wia/qua-002/docs"
...
run_tests() {
    ...
    if command -v wia-qua-002 &> /dev/null; then
        ...
        wia-qua-002 version &> /dev/null && print_success "CLI test passed" || print_error "CLI test failed"
    fi
...
show_completion() {
    ...
    echo ""
    echo -e "${GREEN}The WIA-QUA-002 standard has been successfully installed!${RESET}"
    echo ""
    echo "Available commands:"
    echo -e "${CYAN}  wia-qua-002 shor${RESET}              - Run Shor's factorization"
    echo -e "${CYAN}  wia-qua-002 grover${RESET}            - Run Grover's search"
    echo -e "${CYAN}  wia-qua-002 circuit create${RESET}    - Create quantum circuit"
    echo -e "${CYAN}  wia-qua-002 vqe${RESET}               - Run VQE simulation"
    echo -e "${CYAN}  wia-qua-002 qaoa${RESET}              - Run QAOA"
    echo -e "${CYAN}  wia-qua-002 error-correct${RESET}     - Simulate error correction"
    echo -e "${CYAN}  wia-qua-002 help${RESET}              - Show help message"
    echo ""
    echo "Quick start:"
    echo -e "${GRAY}  # Factor 15 using Shor's algorithm${RESET}"
    echo -e "${CYAN}  wia-qua-002 shor --number 15${RESET}"
    echo ""
    echo -e "${GRAY}  # Search for item 7 in database of size 16${RESET}"
    echo -e "${CYAN}  wia-qua-002 grover --size 16 --target 7${RESET}"
    echo ""
    echo -e "${GRAY}  # Run VQE for H2 molecule${RESET}"
    echo -e "${CYAN}  wia-qua-002 vqe --molecule H2${RESET}"
    echo ""
    echo "TypeScript SDK:"
    echo -e "${GRAY}  import { shorFactorize, groverSearch } from '@wia/qua-002';${RESET}"
    echo ""
    echo "Documentation:"
    echo -e "${GRAY}  $HOME/.wia/qua-002/docs/${RESET}"
    echo ""
