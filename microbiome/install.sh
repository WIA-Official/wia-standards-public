...
################################################################################
# WIA-BIO-013: Microbiome - Installation Script
#
# @version 1.0.0
# @license MIT
...
#
# This script installs the WIA-BIO-013 standard components:
# - CLI tool
# - TypeScript SDK
# - Documentation
################################################################################
...
print_header() {
    echo -e "${TEAL}"
    echo "╔════════════════════════════════════════════════════════════════╗"
    echo "║         🦠 WIA-BIO-013: Microbiome Installer                   ║"
    echo "║                      Version 1.0.0                             ║"
    echo "╚════════════════════════════════════════════════════════════════╝"
    echo -e "${RESET}"
}
...
install_cli() {
    ...
    local cli_path="$SCRIPT_DIR/cli/wia-bio-013.sh"
    ...
    if [ -w "$install_dir" ]; then
        cp "$cli_path" "$install_dir/wia-bio-013"
        print_success "Installed CLI to $install_dir/wia-bio-013"
    else
        print_info "Installing to $install_dir requires sudo..."
        sudo cp "$cli_path" "$install_dir/wia-bio-013"
        print_success "Installed CLI to $install_dir/wia-bio-013 (with sudo)"
    fi

    # Verify installation
    if command -v wia-bio-013 &> /dev/null; then
        print_success "CLI tool is now available as 'wia-bio-013'"
        print_info "Try: wia-bio-013 --help"
    else
...
install_docs() {
    ...
    local docs_dir="$HOME/.wia/bio-013/docs"
...
run_tests() {
    ...
    if command -v wia-bio-013 &> /dev/null; then
        ...
        wia-bio-013 version &> /dev/null && print_success "CLI test passed" || print_error "CLI test failed"
    fi
...
show_completion() {
    ...
    echo ""
    echo -e "${GREEN}The WIA-BIO-013 Microbiome standard has been successfully installed!${RESET}"
    echo ""
    echo "Available commands:"
    echo -e "${CYAN}  wia-bio-013 diversity${RESET}         - Calculate diversity metrics"
    echo -e "${CYAN}  wia-bio-013 classify${RESET}          - Classify taxonomy"
    echo -e "${CYAN}  wia-bio-013 dysbiosis${RESET}         - Assess dysbiosis"
    echo -e "${CYAN}  wia-bio-013 report${RESET}            - Generate clinical report"
    echo -e "${CYAN}  wia-bio-013 help${RESET}              - Show help message"
    echo ""
    echo "Quick start:"
    echo -e "${GRAY}  # Calculate Shannon diversity${RESET}"
    echo -e "${CYAN}  wia-bio-013 diversity --abundance taxa.tsv --metric shannon${RESET}"
    echo ""
    echo -e "${GRAY}  # Classify 16S sequences${RESET}"
    echo -e "${CYAN}  wia-bio-013 classify --sequences reads.fasta --database silva${RESET}"
    echo ""
    echo -e "${GRAY}  # Assess dysbiosis${RESET}"
    echo -e "${CYAN}  wia-bio-013 dysbiosis --abundance taxa.tsv${RESET}"
    echo ""
    echo "TypeScript SDK:"
    echo -e "${GRAY}  import { MicrobiomeSDK } from '@wia/bio-013';${RESET}"
    echo ""
    echo "Documentation:"
    echo -e "${GRAY}  $HOME/.wia/bio-013/docs/${RESET}"
    echo ""
