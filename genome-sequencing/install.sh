...
################################################################################
# WIA-BIO-002: Genome Sequencing - Installation Script
#
# @version 1.0.0
# @license MIT
...
#
# This script installs the WIA-BIO-002 standard components:
# - CLI tool
# - TypeScript SDK
# - Documentation
################################################################################
...
print_header() {
    echo -e "${TEAL}"
    echo "╔════════════════════════════════════════════════════════════════╗"
    echo "║        🧬 WIA-BIO-002: Genome Sequencing Installer           ║"
    echo "║                      Version 1.0.0                             ║"
    echo "╚════════════════════════════════════════════════════════════════╝"
    echo -e "${RESET}"
}
...
install_cli() {
    ...
    local cli_path="$SCRIPT_DIR/cli/wia-bio-002.sh"
    ...
    if [ -w "$install_dir" ]; then
        cp "$cli_path" "$install_dir/wia-bio-002"
        print_success "Installed CLI to $install_dir/wia-bio-002"
    else
        print_info "Installing to $install_dir requires sudo..."
        sudo cp "$cli_path" "$install_dir/wia-bio-002"
        print_success "Installed CLI to $install_dir/wia-bio-002 (with sudo)"
    fi

    # Verify installation
    if command -v wia-bio-002 &> /dev/null; then
        print_success "CLI tool is now available as 'wia-bio-002'"
        print_info "Try: wia-bio-002 --help"
    else
...
install_docs() {
    ...
    local docs_dir="$HOME/.wia/bio-002/docs"
...
run_tests() {
    ...
    if command -v wia-bio-002 &> /dev/null; then
        ...
        wia-bio-002 version &> /dev/null && print_success "CLI test passed" || print_error "CLI test failed"
    fi
...
show_completion() {
    ...
    echo ""
    echo -e "${GREEN}The WIA-BIO-002 standard has been successfully installed!${RESET}"
    echo ""
    echo "Available commands:"
    echo -e "${CYAN}  wia-bio-002 calc-coverage${RESET}      - Calculate sequencing coverage"
    echo -e "${CYAN}  wia-bio-002 validate-quality${RESET}   - Validate quality metrics"
    echo -e "${CYAN}  wia-bio-002 calc-phred${RESET}         - Calculate Phred scores"
    echo -e "${CYAN}  wia-bio-002 calc-variants${RESET}      - Calculate variant statistics"
    echo -e "${CYAN}  wia-bio-002 qc-report${RESET}          - Generate QC report"
    echo -e "${CYAN}  wia-bio-002 help${RESET}               - Show help message"
    echo ""
    echo "Quick start:"
    echo -e "${GRAY}  # Calculate coverage for 150M reads${RESET}"
    echo -e "${CYAN}  wia-bio-002 calc-coverage --reads 150000000 --length 150${RESET}"
    echo ""
    echo -e "${GRAY}  # Validate sequencing quality${RESET}"
    echo -e "${CYAN}  wia-bio-002 validate-quality --q30 92 --coverage 35${RESET}"
    echo ""
    echo -e "${GRAY}  # Calculate Phred score${RESET}"
    echo -e "${CYAN}  wia-bio-002 calc-phred --accuracy 0.999${RESET}"
    echo ""
    echo -e "${GRAY}  # Generate QC report${RESET}"
    echo -e "${CYAN}  wia-bio-002 qc-report${RESET}"
    echo ""
    echo "TypeScript SDK:"
    echo -e "${GRAY}  import { calculateCoverage, validateQuality } from '@wia/bio-002';${RESET}"
    echo ""
    echo "Documentation:"
    echo -e "${GRAY}  $HOME/.wia/bio-002/docs/${RESET}"
    echo ""
