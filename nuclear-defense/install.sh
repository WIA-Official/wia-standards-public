...
################################################################################
# WIA-DEF-014: Nuclear Defense - Installation Script
#
# @version 1.0.0
# @license MIT
...
#
# This script installs the WIA-DEF-014 standard components:
# - CLI tool
# - TypeScript SDK
# - Documentation
################################################################################
...
print_header() {
    echo -e "${SLATE}"
    echo "╔════════════════════════════════════════════════════════════════╗"
    echo "║      ☢️  WIA-DEF-014: Nuclear Defense Installer               ║"
    echo "║                      Version 1.0.0                             ║"
    echo "╚════════════════════════════════════════════════════════════════╝"
    echo -e "${RESET}"
}
...
install_cli() {
    ...
    local cli_path="$SCRIPT_DIR/cli/wia-def-014.sh"
    ...
    if [ -w "$install_dir" ]; then
        cp "$cli_path" "$install_dir/wia-def-014"
        print_success "Installed CLI to $install_dir/wia-def-014"
    else
        print_info "Installing to $install_dir requires sudo..."
        sudo cp "$cli_path" "$install_dir/wia-def-014"
        print_success "Installed CLI to $install_dir/wia-def-014 (with sudo)"
    fi

    # Verify installation
    if command -v wia-def-014 &> /dev/null; then
        print_success "CLI tool is now available as 'wia-def-014'"
        print_info "Try: wia-def-014 --help"
    else
...
install_docs() {
    ...
    local docs_dir="$HOME/.wia/def-014/docs"
...
run_tests() {
    ...
    if command -v wia-def-014 &> /dev/null; then
        ...
        wia-def-014 version &> /dev/null && print_success "CLI test passed" || print_error "CLI test failed"
    fi
...
show_completion() {
    ...
    echo ""
    echo -e "${GREEN}The WIA-DEF-014 Nuclear Defense standard has been successfully installed!${RESET}"
    echo ""
    echo "Available commands:"
    echo -e "${CYAN}  wia-def-014 monitor${RESET}              - Monitor radiation levels"
    echo -e "${CYAN}  wia-def-014 fallout${RESET}              - Calculate fallout prediction"
    echo -e "${CYAN}  wia-def-014 shelter${RESET}              - Assess shelter protection"
    echo -e "${CYAN}  wia-def-014 emp${RESET}                  - Assess EMP protection"
    echo -e "${CYAN}  wia-def-014 decon${RESET}                - Generate decontamination plan"
    echo -e "${CYAN}  wia-def-014 emergency${RESET}            - Simulate emergency response"
    echo -e "${CYAN}  wia-def-014 validate${RESET}             - Validate civil defense readiness"
    echo -e "${CYAN}  wia-def-014 help${RESET}                 - Show help message"
    echo ""
    echo "Quick start:"
    echo -e "${GRAY}  # Monitor radiation levels${RESET}"
    echo -e "${CYAN}  wia-def-014 monitor --sensor geiger --threshold 1.0${RESET}"
    echo ""
    echo -e "${GRAY}  # Calculate fallout from nuclear event${RESET}"
    echo -e "${CYAN}  wia-def-014 fallout --yield 100 --distance 10 --wind-speed 5${RESET}"
    echo ""
    echo -e "${GRAY}  # Assess shelter protection${RESET}"
    echo -e "${CYAN}  wia-def-014 shelter --type basement --walls concrete --thickness 30${RESET}"
    echo ""
    echo -e "${GRAY}  # Assess EMP protection${RESET}"
    echo -e "${CYAN}  wia-def-014 emp --shielding faraday-cage${RESET}"
    echo ""
    echo "TypeScript SDK:"
    echo -e "${GRAY}  import { monitorRadiation, calculateFallout } from '@wia/def-014';${RESET}"
    echo ""
    echo "Documentation:"
    echo -e "${GRAY}  $HOME/.wia/def-014/docs/${RESET}"
    echo ""
