# WIA-video PHASE 2 — API-INTERFACE Specification
...
`urn:wia:video:status-transition`.
...
`422` with type
`urn:wia:video:source-mix-required`.
...
with type
`urn:wia:video:laboratory-not-accredited`.
...
`409` with type
`urn:wia:video:quality-certificate-required`.
...
- `urn:wia:video:hydrogen-quality-out-of-grade`
- `urn:wia:video:cryogenic-temperature-excursion`
- `urn:wia:video:guarantee-of-origin-mismatch`
- `urn:wia:video:evidence-mismatch`
