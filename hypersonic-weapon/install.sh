...
################################################################################
# WIA-DEF-008: Hypersonic Weapon - Installation Script
#
# @version 1.0.0
# @license MIT
...
#
# This script installs the WIA-DEF-008 standard components:
# - CLI tool
# - TypeScript SDK
# - Documentation
################################################################################
...
print_header() {
    echo -e "${SLATE}"
    echo "╔════════════════════════════════════════════════════════════════╗"
    echo "║      🚀 WIA-DEF-008: Hypersonic Weapon System Installer      ║"
    echo "║                      Version 1.0.0                             ║"
    echo "╚════════════════════════════════════════════════════════════════╝"
    echo -e "${RESET}"
}
...
install_cli() {
    ...
    local cli_path="$SCRIPT_DIR/cli/wia-def-008.sh"
    ...
    if [ -w "$install_dir" ]; then
        cp "$cli_path" "$install_dir/wia-def-008"
        print_success "Installed CLI to $install_dir/wia-def-008"
    else
        print_info "Installing to $install_dir requires sudo..."
        sudo cp "$cli_path" "$install_dir/wia-def-008"
        print_success "Installed CLI to $install_dir/wia-def-008 (with sudo)"
    fi

    # Verify installation
    if command -v wia-def-008 &> /dev/null; then
        print_success "CLI tool is now available as 'wia-def-008'"
        print_info "Try: wia-def-008 --help"
    else
...
install_docs() {
    ...
    local docs_dir="$HOME/.wia/def-008/docs"
...
run_tests() {
    ...
    if command -v wia-def-008 &> /dev/null; then
        ...
        wia-def-008 version &> /dev/null && print_success "CLI test passed" || print_error "CLI test failed"
    fi
...
show_completion() {
    ...
    echo ""
    echo -e "${GREEN}The WIA-DEF-008 standard has been successfully installed!${RESET}"
    echo ""
    echo "Available commands:"
    echo -e "${CYAN}  wia-def-008 calc-heat${RESET}        - Calculate heat flux and thermal loads"
    echo -e "${CYAN}  wia-def-008 trajectory${RESET}       - Optimize flight trajectory"
    echo -e "${CYAN}  wia-def-008 detect${RESET}           - Analyze detectability"
    echo -e "${CYAN}  wia-def-008 simulate${RESET}         - Simulate complete hypersonic flight"
    echo -e "${CYAN}  wia-def-008 help${RESET}             - Show help message"
    echo ""
    echo "Quick start:"
    echo -e "${GRAY}  # Calculate heat flux at Mach 8${RESET}"
    echo -e "${CYAN}  wia-def-008 calc-heat --velocity 2744 --altitude 30000${RESET}"
    echo ""
    echo -e "${GRAY}  # Optimize trajectory for 1500 km range${RESET}"
    echo -e "${CYAN}  wia-def-008 trajectory --range 1500000 --angle 25 --velocity 2000${RESET}"
    echo ""
    echo -e "${GRAY}  # Simulate HGV flight${RESET}"
    echo -e "${CYAN}  wia-def-008 simulate --type hgv --range 2000000 --mach 8${RESET}"
    echo ""
    echo "TypeScript SDK:"
    echo -e "${GRAY}  import { calculateHeatFlux } from '@wia/def-008';${RESET}"
    echo ""
    echo "Documentation:"
    echo -e "${GRAY}  $HOME/.wia/def-008/docs/${RESET}"
    echo ""
