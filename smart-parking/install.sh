...
################################################################################
# WIA-AUTO-013: Smart Parking - Installation Script
#
# @version 1.0.0
# @license MIT
...
#
# This script installs the WIA-AUTO-013 standard components:
# - CLI tool
# - TypeScript SDK
# - Documentation
################################################################################
...
print_header() {
    echo -e "${ORANGE}"
    echo "╔════════════════════════════════════════════════════════════════╗"
    echo "║      🅿️  WIA-AUTO-013: Smart Parking Installer                 ║"
    echo "║                      Version 1.0.0                             ║"
    echo "╚════════════════════════════════════════════════════════════════╝"
    echo -e "${RESET}"
}
...
check_prerequisites() {
    ...
    if command -v jq &> /dev/null; then
        ...
        print_info "jq not found (optional, for JSON parsing)"
...
install_cli() {
    ...
    local cli_path="$SCRIPT_DIR/cli/wia-auto-013.sh"
    ...
    if [ -w "$install_dir" ]; then
        cp "$cli_path" "$install_dir/wia-auto-013"
        print_success "Installed CLI to $install_dir/wia-auto-013"
    else
        print_info "Installing to $install_dir requires sudo..."
        sudo cp "$cli_path" "$install_dir/wia-auto-013"
        print_success "Installed CLI to $install_dir/wia-auto-013 (with sudo)"
    fi

    # Verify installation
    if command -v wia-auto-013 &> /dev/null; then
        print_success "CLI tool is now available as 'wia-auto-013'"
        print_info "Try: wia-auto-013 --help"
    else
...
install_docs() {
    ...
    local docs_dir="$HOME/.wia/auto-013/docs"
...
run_tests() {
    ...
    if command -v wia-auto-013 &> /dev/null; then
        ...
        wia-auto-013 version &> /dev/null && print_success "CLI test passed" || print_error "CLI test failed"
    fi
...
show_completion() {
    ...
    echo ""
    echo -e "${GREEN}The WIA-AUTO-013 Smart Parking standard has been successfully installed!${RESET}"
    echo ""
    echo "Available commands:"
    echo -e "${ORANGE}  wia-auto-013 find${RESET}             - Find available parking spaces"
    echo -e "${ORANGE}  wia-auto-013 reserve${RESET}          - Reserve a parking space"
    echo -e "${ORANGE}  wia-auto-013 status${RESET}           - Check lot occupancy status"
    echo -e "${ORANGE}  wia-auto-013 charge${RESET}           - Start EV charging session"
    echo -e "${ORANGE}  wia-auto-013 report${RESET}           - Generate occupancy report"
    echo -e "${ORANGE}  wia-auto-013 help${RESET}             - Show help message"
    echo ""
    echo "Quick start:"
    echo -e "${GRAY}  # Find parking near location${RESET}"
    echo -e "${ORANGE}  wia-auto-013 find --lat 37.7749 --lng -122.4194 --radius 500${RESET}"
    echo ""
    echo -e "${GRAY}  # Reserve a parking space${RESET}"
    echo -e "${ORANGE}  wia-auto-013 reserve --space-id SP-EV-042 --duration 120 --plate ABC123${RESET}"
    echo ""
    echo -e "${GRAY}  # Check lot status${RESET}"
    echo -e "${ORANGE}  wia-auto-013 status --lot-id lot-downtown-01${RESET}"
    echo ""
    echo -e "${GRAY}  # Start EV charging${RESET}"
    echo -e "${ORANGE}  wia-auto-013 charge --space-id SP-EV-042 --connector ccs2${RESET}"
    echo ""
    echo "TypeScript SDK:"
    echo -e "${GRAY}  import { findAvailableSpaces } from '@wia/auto-013';${RESET}"
    echo ""
    echo "Documentation:"
    echo -e "${GRAY}  $HOME/.wia/auto-013/docs/${RESET}"
    echo ""
    echo "Environment Variables:"
    echo -e "${GRAY}  export WIA_PARKING_API_KEY='your-api-key'${RESET}"
    echo -e "${GRAY}  export WIA_PARKING_API='https://api.parking.example.com/v1'${RESET}"
    echo ""
