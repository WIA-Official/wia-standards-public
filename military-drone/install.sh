...
################################################################################
# WIA-DEF-002: Military Drone - Installation Script
#
# @version 1.0.0
# @license MIT
...
#
# This script installs the WIA-DEF-002 standard components:
# - CLI tool
# - TypeScript SDK
# - Documentation
################################################################################
...
print_header() {
    echo -e "${SLATE}"
    echo "╔════════════════════════════════════════════════════════════════╗"
    echo "║        🚁 WIA-DEF-002: Military Drone Installer               ║"
    echo "║                      Version 1.0.0                             ║"
    echo "╚════════════════════════════════════════════════════════════════╝"
    echo -e "${RESET}"
}
...
install_cli() {
    ...
    local cli_path="$SCRIPT_DIR/cli/wia-def-002.sh"
    ...
    if [ -w "$install_dir" ]; then
        cp "$cli_path" "$install_dir/wia-def-002"
        print_success "Installed CLI to $install_dir/wia-def-002"
    else
        print_info "Installing to $install_dir requires sudo..."
        sudo cp "$cli_path" "$install_dir/wia-def-002"
        print_success "Installed CLI to $install_dir/wia-def-002 (with sudo)"
    fi

    # Verify installation
    if command -v wia-def-002 &> /dev/null; then
        print_success "CLI tool is now available as 'wia-def-002'"
        print_info "Try: wia-def-002 --help"
    else
...
install_docs() {
    ...
    local docs_dir="$HOME/.wia/def-002/docs"
...
run_tests() {
    ...
    if command -v wia-def-002 &> /dev/null; then
        ...
        wia-def-002 version &> /dev/null && print_success "CLI test passed" || print_error "CLI test failed"
    fi
...
show_completion() {
    ...
    echo ""
    echo -e "${GREEN}The WIA-DEF-002 standard has been successfully installed!${RESET}"
    echo ""
    echo "Available commands:"
    echo -e "${CYAN}  wia-def-002 configure${RESET}          - Configure drone system"
    echo -e "${CYAN}  wia-def-002 validate-mission${RESET}   - Validate mission plan"
    echo -e "${CYAN}  wia-def-002 plan-flight${RESET}        - Generate flight plan"
    echo -e "${CYAN}  wia-def-002 calc-endurance${RESET}     - Calculate fuel requirements"
    echo -e "${CYAN}  wia-def-002 border-patrol${RESET}      - Plan border security patrol"
    echo -e "${CYAN}  wia-def-002 disaster-response${RESET}  - Plan disaster response mission"
    echo -e "${CYAN}  wia-def-002 help${RESET}               - Show help message"
    echo ""
    echo "Quick start:"
    echo -e "${GRAY}  # Configure a MALE UAV${RESET}"
    echo -e "${CYAN}  wia-def-002 configure --class \"Class IV - MALE\" --endurance 30${RESET}"
    echo ""
    echo -e "${GRAY}  # Plan a border patrol mission${RESET}"
    echo -e "${CYAN}  wia-def-002 border-patrol --start \"37.5,127.0\" --end \"38.0,128.0\"${RESET}"
    echo ""
    echo -e "${GRAY}  # Plan disaster response${RESET}"
    echo -e "${CYAN}  wia-def-002 disaster-response --type earthquake --center \"37.5,127.0\" --radius 50${RESET}"
    echo ""
    echo "TypeScript SDK:"
    echo -e "${GRAY}  import { createDroneSystem, calculateMissionParameters } from '@wia/def-002';${RESET}"
    echo ""
    echo "Documentation:"
    echo -e "${GRAY}  $HOME/.wia/def-002/docs/${RESET}"
    echo ""
    echo -e "${SLATE}弘益人間 (Benefit All Humanity)${RESET}"
    echo -e "${GRAY}Focus: Defensive & Humanitarian Applications${RESET}"
