...
################################################################################
# WIA-QUA-004: Quantum Sensor - Installation Script
#
# @version 1.0.0
# @license MIT
...
#
# This script installs the WIA-QUA-004 standard components:
# - CLI tool
# - TypeScript SDK
# - Documentation
################################################################################
...
print_header() {
    echo -e "${INDIGO}"
    echo "╔════════════════════════════════════════════════════════════════╗"
    echo "║        📡 WIA-QUA-004: Quantum Sensor Installer              ║"
    echo "║                      Version 1.0.0                             ║"
    echo "╚════════════════════════════════════════════════════════════════╝"
    echo -e "${RESET}"
}
...
install_cli() {
    ...
    local cli_path="$SCRIPT_DIR/cli/wia-qua-004.sh"
    ...
    if [ -w "$install_dir" ]; then
        cp "$cli_path" "$install_dir/wia-qua-004"
        print_success "Installed CLI to $install_dir/wia-qua-004"
    else
        print_info "Installing to $install_dir requires sudo..."
        sudo cp "$cli_path" "$install_dir/wia-qua-004"
        print_success "Installed CLI to $install_dir/wia-qua-004 (with sudo)"
    fi

    # Verify installation
    if command -v wia-qua-004 &> /dev/null; then
        print_success "CLI tool is now available as 'wia-qua-004'"
        print_info "Try: wia-qua-004 --help"
    else
...
install_docs() {
    ...
    local docs_dir="$HOME/.wia/qua-004/docs"
...
run_tests() {
    ...
    if command -v wia-qua-004 &> /dev/null; then
        ...
        wia-qua-004 version &> /dev/null && print_success "CLI test passed" || print_error "CLI test failed"
    fi
...
show_completion() {
    ...
    echo ""
    echo -e "${GREEN}The WIA-QUA-004 Quantum Sensor standard has been successfully installed!${RESET}"
    echo ""
    echo "Available commands:"
    echo -e "${CYAN}  wia-qua-004 clock${RESET}            - Atomic clock operations"
    echo -e "${CYAN}  wia-qua-004 magnetometer${RESET}     - Magnetometer measurements"
    echo -e "${CYAN}  wia-qua-004 gravimeter${RESET}       - Gravimeter measurements"
    echo -e "${CYAN}  wia-qua-004 gyroscope${RESET}        - Gyroscope measurements"
    echo -e "${CYAN}  wia-qua-004 imaging${RESET}          - Quantum imaging"
    echo -e "${CYAN}  wia-qua-004 radar${RESET}            - Quantum radar detection"
    echo -e "${CYAN}  wia-qua-004 calibrate${RESET}        - Sensor calibration"
    echo -e "${CYAN}  wia-qua-004 help${RESET}             - Show help message"
    echo ""
    echo "Quick start:"
    echo -e "${GRAY}  # Measure time with atomic clock${RESET}"
    echo -e "${CYAN}  wia-qua-004 clock strontium${RESET}"
    echo ""
    echo -e "${GRAY}  # Measure magnetic field with SQUID${RESET}"
    echo -e "${CYAN}  wia-qua-004 magnetometer SQUID${RESET}"
    echo ""
    echo -e "${GRAY}  # Measure gravity with atom interferometer${RESET}"
    echo -e "${CYAN}  wia-qua-004 gravimeter rubidium${RESET}"
    echo ""
    echo -e "${GRAY}  # Quantum ghost imaging${RESET}"
    echo -e "${CYAN}  wia-qua-004 imaging ghost 1000${RESET}"
    echo ""
    echo "TypeScript SDK:"
    echo -e "${GRAY}  import { QuantumSensorSDK } from '@wia/qua-004';${RESET}"
    echo -e "${GRAY}  const sdk = new QuantumSensorSDK();${RESET}"
    echo -e "${GRAY}  const clock = sdk.createAtomicClock({...});${RESET}"
    echo ""
    echo "Documentation:"
    echo -e "${GRAY}  $HOME/.wia/qua-004/docs/${RESET}"
    echo ""
