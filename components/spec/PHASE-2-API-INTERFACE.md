# WIA-components PHASE 2 — API-INTERFACE Specification

**Standard:** WIA-components
...
`urn:wia:components:status-transition`.
...
`422` with type
`urn:wia:components:source-mix-required`.
...
with type
`urn:wia:components:laboratory-not-accredited`.
...
`409` with type
`urn:wia:components:quality-certificate-required`.
...
- `urn:wia:components:hydrogen-quality-out-of-grade`
- `urn:wia:components:cryogenic-temperature-excursion`
- `urn:wia:components:guarantee-of-origin-mismatch`
- `urn:wia:components:evidence-mismatch`
