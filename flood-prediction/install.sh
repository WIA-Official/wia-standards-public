...
################################################################################
# WIA-ENE-033: Flood Prediction Standard - Installation Script
#
# 弘益人間 (홍익人間) - Benefit All Humanity
#
# This script installs dependencies and sets up the WIA-ENE-033
# Flood Prediction Standard SDK and CLI tools.
#
# Usage:
...
while [[ $# -gt 0 ]]; do
  case $1 in
    ...
    --cli-only)
      ...
      ;;
    --help)
      echo "WIA-ENE-033 Installation Script"
      echo ""
      echo "Usage:"
...
print_header() {
  ...
  echo "╔════════════════════════════════════════════════════════════════╗"
  echo "║                                                                ║"
  echo "║         🌊  WIA-ENE-033: Flood Prediction Standard            ║"
  echo "║                                                                ║"
...
if $INSTALL_CLI; then
  ...
  if [ -f "${CLI_DIR}/flood-prediction.sh" ]; then
    chmod +x "${CLI_DIR}/flood-prediction.sh"
    print_step "CLI script made executable"
  else
    print_error "CLI script not found: ${CLI_DIR}/flood-prediction.sh"
    exit 1
  fi

  # Create symlink in /usr/local/bin (optional)
  if [ -w "/usr/local/bin" ]; then
    ln -sf "${CLI_DIR}/flood-prediction.sh" /usr/local/bin/wia-flood 2>/dev/null || true
    if [ -L "/usr/local/bin/wia-flood" ]; then
      print_step "Symlink created: /usr/local/bin/wia-flood"
      print_info "You can now run 'wia-flood' from anywhere"
    fi
  else
    print_warning "Cannot create symlink in /usr/local/bin (no write permission)"
    print_info "You can run the CLI using: ${CLI_DIR}/flood-prediction.sh"
  fi
...
if $INSTALL_SDK; then
  ...
  echo "  Usage:"
  echo "    import { FloodPredictionSDK } from '@wia/ene-033';"
  echo ""
fi

if $INSTALL_CLI; then
  echo -e "${GREEN}✓${NC} CLI tools installed"
  echo "  Location: ${CLI_DIR}/flood-prediction.sh"
  echo "  Usage:"
  echo "    ./cli/flood-prediction.sh --help"
  if [ -L "/usr/local/bin/wia-flood" ]; then
    echo "  Or: wia-flood --help"
  fi
  echo ""
fi
...
echo "   - spec/WIA-ENE-033-v1.0.md: Detailed specification"
echo ""
echo "2. Try the SDK:"
echo "   cd ${SDK_DIR}"
echo "   npm test"
echo ""
echo "3. Try the CLI:"
echo "   ${CLI_DIR}/flood-prediction.sh --help"
echo ""
echo "4. Join the community:"
echo "   - GitHub: https://github.com/WIA-Official/wia-standards"
echo "   - Website: https://wia.org/standards/ene-033"
