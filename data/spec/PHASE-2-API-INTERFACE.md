# WIA-data PHASE 2 — API-INTERFACE Specification
...
`urn:wia:data:status-transition`.
...
`422` with type
`urn:wia:data:source-mix-required`.
...
with type
`urn:wia:data:laboratory-not-accredited`.
...
`409` with type
`urn:wia:data:quality-certificate-required`.
...
- `urn:wia:data:hydrogen-quality-out-of-grade`
- `urn:wia:data:cryogenic-temperature-excursion`
- `urn:wia:data:guarantee-of-origin-mismatch`
- `urn:wia:data:evidence-mismatch`
