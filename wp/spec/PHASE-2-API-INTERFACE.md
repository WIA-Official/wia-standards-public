# WIA-wp PHASE 2 — API-INTERFACE Specification
...
`422` with type
`urn:wia:wp:source-mix-required`.
...
with type
`urn:wia:wp:laboratory-not-accredited`.
...
`409` with type
`urn:wia:wp:quality-certificate-required`.
...
- `urn:wia:wp:hydrogen-quality-out-of-grade`
- `urn:wia:wp:cryogenic-temperature-excursion`
- `urn:wia:wp:guarantee-of-origin-mismatch`
- `urn:wia:wp:evidence-mismatch`
