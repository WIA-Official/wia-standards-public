...
################################################################################
# WIA-DEF-011: Reconnaissance Satellite - Installation Script
#
# @version 1.0.0
# @license MIT
...
#
# This script installs the WIA-DEF-011 standard components:
# - CLI tool
# - TypeScript SDK
# - Documentation
################################################################################
...
print_header() {
    echo -e "${SLATE}"
    echo "╔════════════════════════════════════════════════════════════════╗"
    echo "║    🔭 WIA-DEF-011: Reconnaissance Satellite Installer         ║"
    echo "║                      Version 1.0.0                             ║"
    echo "╚════════════════════════════════════════════════════════════════╝"
    echo -e "${RESET}"
}
...
install_cli() {
    ...
    local cli_path="$SCRIPT_DIR/cli/wia-def-011.sh"
    ...
    if [ -w "$install_dir" ]; then
        cp "$cli_path" "$install_dir/wia-def-011"
        print_success "Installed CLI to $install_dir/wia-def-011"
    else
        print_info "Installing to $install_dir requires sudo..."
        sudo cp "$cli_path" "$install_dir/wia-def-011"
        print_success "Installed CLI to $install_dir/wia-def-011 (with sudo)"
    fi

    # Verify installation
    if command -v wia-def-011 &> /dev/null; then
        print_success "CLI tool is now available as 'wia-def-011'"
        print_info "Try: wia-def-011 --help"
    else
...
install_docs() {
    ...
    local docs_dir="$HOME/.wia/def-011/docs"
...
run_tests() {
    ...
    if command -v wia-def-011 &> /dev/null; then
        ...
        wia-def-011 version &> /dev/null && print_success "CLI test passed" || print_error "CLI test failed"
    fi
...
show_completion() {
    ...
    echo ""
    echo -e "${GREEN}The WIA-DEF-011 standard has been successfully installed!${RESET}"
    echo ""
    echo "Available commands:"
    echo -e "${CYAN}  wia-def-011 calc-resolution${RESET}   - Calculate ground resolution"
    echo -e "${CYAN}  wia-def-011 analyze-coverage${RESET}  - Analyze orbital coverage"
    echo -e "${CYAN}  wia-def-011 calc-sar${RESET}          - Calculate SAR resolution"
    echo -e "${CYAN}  wia-def-011 calc-revisit${RESET}      - Calculate revisit time"
    echo -e "${CYAN}  wia-def-011 process-image${RESET}     - Process reconnaissance image"
    echo -e "${CYAN}  wia-def-011 help${RESET}              - Show help message"
    echo ""
    echo "Quick start:"
    echo -e "${GRAY}  # Calculate optical sensor ground resolution${RESET}"
    echo -e "${CYAN}  wia-def-011 calc-resolution --altitude 500000 --focal-length 10.5${RESET}"
    echo ""
    echo -e "${GRAY}  # Analyze coverage for Seoul (37.5°N)${RESET}"
    echo -e "${CYAN}  wia-def-011 analyze-coverage --altitude 550000 --target-lat 37.5${RESET}"
    echo ""
    echo -e "${GRAY}  # Calculate SAR resolution${RESET}"
    echo -e "${CYAN}  wia-def-011 calc-sar --bandwidth 600e6 --antenna 5${RESET}"
    echo ""
    echo -e "${GRAY}  # Calculate revisit time for 4-satellite constellation${RESET}"
    echo -e "${CYAN}  wia-def-011 calc-revisit --altitude 550000 --satellites 4${RESET}"
    echo ""
    echo "TypeScript SDK:"
    echo -e "${GRAY}  import { calculateGroundResolution } from '@wia/def-011';${RESET}"
    echo ""
    echo "Documentation:"
    echo -e "${GRAY}  $HOME/.wia/def-011/docs/${RESET}"
    echo ""
