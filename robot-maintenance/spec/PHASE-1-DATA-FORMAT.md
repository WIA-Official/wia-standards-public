# WIA-ROBOT-MAINT-001 — Phase 1: DATA-FORMAT

> Robot Maintenance canonical Phase 1 specification per the WIA Standards four-Phase architecture.

> Domain: 로봇 유지보수 — 로봇 유지보수 · 예지보전 · 진동 분석 · OEE · IoT 진단.

## A.1 Scope

This Phase covers the canonical data-format layer of the WIA-ROBOT-MAINT-001 standard. It composes with the Phase 2 document for cross-Phase integration and inherits cross-standard behaviour from the wider WIA Standards family per the README.md scope statement.
...
with `wia.standard.slug` = `robot-maintenance` and `wia.standard.phase` =
...
`/.well-known/wia-robot-maintenance-capabilities` enumerating per-endpoint
