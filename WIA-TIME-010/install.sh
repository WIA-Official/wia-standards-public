...
################################################################################
# WIA-TIME-010: Paradox Prevention - Installation Script
#
# @version 1.0.0
# @license MIT
...
#
# This script installs the WIA-TIME-010 standard components:
# - CLI tool
# - TypeScript SDK
# - Documentation
################################################################################
...
print_header() {
    echo -e "${VIOLET}"
    echo "╔════════════════════════════════════════════════════════════════╗"
    echo "║       ⚠️  WIA-TIME-010: Paradox Prevention Installer         ║"
    echo "║                      Version 1.0.0                             ║"
    echo "╚════════════════════════════════════════════════════════════════╝"
    echo -e "${RESET}"
}
...
install_cli() {
    ...
    local cli_path="$SCRIPT_DIR/cli/wia-time-010.sh"
    ...
    if [ -w "$install_dir" ]; then
        cp "$cli_path" "$install_dir/wia-time-010"
        print_success "Installed CLI to $install_dir/wia-time-010"
    else
        print_info "Installing to $install_dir requires sudo..."
        sudo cp "$cli_path" "$install_dir/wia-time-010"
        print_success "Installed CLI to $install_dir/wia-time-010 (with sudo)"
    fi

    # Verify installation
    if command -v wia-time-010 &> /dev/null; then
        print_success "CLI tool is now available as 'wia-time-010'"
        print_info "Try: wia-time-010 --help"
    else
...
install_docs() {
    ...
    local docs_dir="$HOME/.wia/time-010/docs"
...
run_tests() {
    ...
    if command -v wia-time-010 &> /dev/null; then
        ...
        wia-time-010 version &> /dev/null && print_success "CLI test passed" || print_error "CLI test failed"
    fi
...
show_completion() {
    ...
    echo ""
    echo -e "${GREEN}The WIA-TIME-010 standard has been successfully installed!${RESET}"
    echo ""
    echo "Available commands:"
    echo -e "${CYAN}  wia-time-010 detect${RESET}           - Detect paradoxes in actions"
    echo -e "${CYAN}  wia-time-010 verify-timeline${RESET}  - Verify timeline integrity"
    echo -e "${CYAN}  wia-time-010 classify${RESET}         - Classify paradox severity"
    echo -e "${CYAN}  wia-time-010 branch${RESET}           - Create timeline branch"
    echo -e "${CYAN}  wia-time-010 rollback${RESET}         - Emergency timeline rollback"
    echo -e "${CYAN}  wia-time-010 list-types${RESET}       - List all paradox types"
    echo -e "${CYAN}  wia-time-010 monitoring${RESET}       - Show monitoring status"
    echo -e "${CYAN}  wia-time-010 help${RESET}             - Show help message"
    echo ""
    echo "Quick start:"
    echo -e "${GRAY}  # Detect grandfather paradox${RESET}"
    echo -e "${CYAN}  wia-time-010 detect --action eliminate_ancestor --target ancestor-001${RESET}"
    echo ""
    echo -e "${GRAY}  # Classify paradox severity${RESET}"
    echo -e "${CYAN}  wia-time-010 classify --paradox-type GRANDFATHER --affected 3${RESET}"
    echo ""
    echo -e "${GRAY}  # Emergency rollback${RESET}"
    echo -e "${CYAN}  wia-time-010 rollback --target-state last_stable${RESET}"
    echo ""
    echo "TypeScript SDK:"
    echo -e "${GRAY}  import { detectParadox } from '@wia/time-010';${RESET}"
    echo ""
    echo "Documentation:"
    echo -e "${GRAY}  $HOME/.wia/time-010/docs/${RESET}"
    echo ""
