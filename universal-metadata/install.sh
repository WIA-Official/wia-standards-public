...
################################################################################
# WIA-CORE-008: Universal Metadata - Installation Script
#
# @version 1.0.0
# @license MIT
...
#
# This script installs the WIA-CORE-008 standard components:
# - CLI tool
# - TypeScript SDK
# - Documentation
################################################################################
...
print_header() {
    echo -e "${INDIGO}"
    echo "╔════════════════════════════════════════════════════════════════╗"
    echo "║      📋 WIA-CORE-008: Universal Metadata Installer            ║"
    echo "║                      Version 1.0.0                             ║"
    echo "╚════════════════════════════════════════════════════════════════╝"
    echo -e "${RESET}"
}
...
install_cli() {
    ...
    local cli_path="$SCRIPT_DIR/cli/wia-core-008.sh"
    ...
    if [ -w "$install_dir" ]; then
        cp "$cli_path" "$install_dir/wia-core-008"
        print_success "Installed CLI to $install_dir/wia-core-008"
    else
        print_info "Installing to $install_dir requires sudo..."
        sudo cp "$cli_path" "$install_dir/wia-core-008"
        print_success "Installed CLI to $install_dir/wia-core-008 (with sudo)"
    fi

    # Verify installation
    if command -v wia-core-008 &> /dev/null; then
        print_success "CLI tool is now available as 'wia-core-008'"
        print_info "Try: wia-core-008 --help"
    else
...
install_docs() {
    ...
    local docs_dir="$HOME/.wia/core-008/docs"
...
run_tests() {
    ...
    if command -v wia-core-008 &> /dev/null; then
        ...
        wia-core-008 version &> /dev/null && print_success "CLI test passed" || print_error "CLI test failed"
    fi
...
show_completion() {
    ...
    echo ""
    echo -e "${GREEN}The WIA-CORE-008 standard has been successfully installed!${RESET}"
    echo ""
    echo "Available commands:"
    echo -e "${INDIGO}  wia-core-008 create${RESET}      - Create new metadata"
    echo -e "${INDIGO}  wia-core-008 validate${RESET}    - Validate metadata file"
    echo -e "${INDIGO}  wia-core-008 enrich${RESET}      - Enrich metadata"
    echo -e "${INDIGO}  wia-core-008 search${RESET}      - Search metadata"
    echo -e "${INDIGO}  wia-core-008 template${RESET}    - Generate template"
    echo -e "${INDIGO}  wia-core-008 quality${RESET}     - Check quality"
    echo -e "${INDIGO}  wia-core-008 help${RESET}        - Show help message"
    echo ""
    echo "Quick start:"
    echo -e "${GRAY}  # Create metadata for a dataset${RESET}"
    ...
    echo ""
    echo -e "${GRAY}  # Generate healthcare template${RESET}"
    echo -e "${INDIGO}  wia-core-008 template --domain healthcare${RESET}"
    echo ""
    echo "TypeScript SDK:"
    echo -e "${GRAY}  import { createMetadata, validateMetadata } from '@wia/core-008';${RESET}"
    echo ""
    echo "Documentation:"
    echo -e "${GRAY}  $HOME/.wia/core-008/docs/${RESET}"
    echo ""
