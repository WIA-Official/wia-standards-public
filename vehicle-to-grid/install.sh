...
################################################################################
# WIA-AUTO-029: Vehicle-to-Grid (V2G) - Installation Script
#
# @version 1.0.0
# @license MIT
...
#
# This script installs the WIA-AUTO-029 standard components:
# - CLI tool
# - TypeScript SDK
# - Documentation
################################################################################
...
print_header() {
    echo -e "${ORANGE}"
    echo "╔════════════════════════════════════════════════════════════════╗"
    echo "║       🔋 WIA-AUTO-029: Vehicle-to-Grid (V2G) Installer       ║"
    echo "║                      Version 1.0.0                             ║"
    echo "╚════════════════════════════════════════════════════════════════╝"
    echo -e "${RESET}"
}
...
install_cli() {
    ...
    local cli_path="$SCRIPT_DIR/cli/wia-auto-029.sh"
    ...
    if [ -w "$install_dir" ]; then
        cp "$cli_path" "$install_dir/wia-auto-029"
        print_success "Installed CLI to $install_dir/wia-auto-029"
    else
        print_info "Installing to $install_dir requires sudo..."
        sudo cp "$cli_path" "$install_dir/wia-auto-029"
        print_success "Installed CLI to $install_dir/wia-auto-029 (with sudo)"
    fi

    # Verify installation
    if command -v wia-auto-029 &> /dev/null; then
        print_success "CLI tool is now available as 'wia-auto-029'"
        print_info "Try: wia-auto-029 --help"
    else
...
install_docs() {
    ...
    local docs_dir="$HOME/.wia/auto-029/docs"
...
run_tests() {
    ...
    if command -v wia-auto-029 &> /dev/null; then
        ...
        wia-auto-029 version &> /dev/null && print_success "CLI test passed" || print_error "CLI test failed"
    fi
...
show_completion() {
    ...
    echo ""
    echo -e "${GREEN}The WIA-AUTO-029 standard has been successfully installed!${RESET}"
    echo ""
    echo "Available commands:"
    echo -e "${CYAN}  wia-auto-029 start${RESET}            - Start V2G session"
    echo -e "${CYAN}  wia-auto-029 status${RESET}           - Monitor current status"
    echo -e "${CYAN}  wia-auto-029 arbitrage${RESET}        - Calculate arbitrage opportunity"
    echo -e "${CYAN}  wia-auto-029 battery-health${RESET}   - Check battery health"
    echo -e "${CYAN}  wia-auto-029 optimize${RESET}         - Optimize charging schedule"
    echo -e "${CYAN}  wia-auto-029 revenue${RESET}          - View revenue report"
    echo -e "${CYAN}  wia-auto-029 help${RESET}             - Show help message"
    echo ""
    echo "Quick start:"
    echo -e "${GRAY}  # Start frequency regulation service${RESET}"
    echo -e "${CYAN}  wia-auto-029 start --vehicle EV-12345 --service frequency-regulation${RESET}"
    echo ""
    echo -e "${GRAY}  # Calculate arbitrage opportunity${RESET}"
    echo -e "${CYAN}  wia-auto-029 arbitrage --buy-price 0.08 --sell-price 0.35${RESET}"
    echo ""
    echo -e "${GRAY}  # Check battery health${RESET}"
    echo -e "${CYAN}  wia-auto-029 battery-health --cycles 500 --avg-dod 0.4${RESET}"
    echo ""
    echo "TypeScript SDK:"
    echo -e "${GRAY}  import { V2GController, calculateArbitrageRevenue } from '@wia/auto-029';${RESET}"
    echo ""
    echo "Documentation:"
    echo -e "${GRAY}  $HOME/.wia/auto-029/docs/${RESET}"
    echo ""
    echo "Learn more:"
    echo -e "${GRAY}  Specification: $SCRIPT_DIR/spec/WIA-AUTO-029-v1.0.md${RESET}"
    echo -e "${GRAY}  Website: https://wiastandards.com/standards/WIA-AUTO-029${RESET}"
    echo ""
