//! HTTP client and API implementations for the WIA-CHILD-006-child-data-privacy SDK
//!
...
/// Main WIA-CHILD-006-child-data-privacy SDK client
///
/// # Example
///
/// ```rust,no_run
...
/// }
/// ```
pub struct Child006ChildDataPrivacyClient {
    http: Client,
    config: Config,
    base_url: String,
}

impl Child006ChildDataPrivacyClient {
    /// Create a new WIA-CHILD-006-child-data-privacy client
    pub fn new(config: Config) -> Self {
        let base_url = config
            .base_url
            .clone()
            .unwrap_or_else(|| "https://api.wiastandards.com/WIA-CHILD-006-child-data-privacy".to_string());
    ...
    /// Get data by ID
    pub async fn get(&self, id: &str) -> Result<Child006ChildDataPrivacyData> {
        ...
        let result: Child006ChildDataPrivacyResponse = response.json().await?;
        result.data.ok_or_else(|| Error::NotFound(id.to_string()))
    }

    /// Create new data
    pub async fn create(&self, request: CreateChild006ChildDataPrivacyRequest) -> Result<Child006ChildDataPrivacyData> {
    ...
    pub async fn list(&self, params: Option<ListParams>) -> Result<Child006ChildDataPrivacyList> {
        let url = format!("{}/data", self.base_url);
