# WIA-ENE-031 — Phase 1: DATA-FORMAT

> Zero Energy Building canonical Phase 1 specification per the WIA Standards four-Phase architecture.

> Domain: 제로에너지 건물 — 제로에너지 빌딩 · 건물 단위 ZEB · K-ZEB · ASHRAE · LEED.

## A.1 Scope

This Phase covers the canonical data-format layer of the WIA-ENE-031 standard. It composes with the Phase 2 document for cross-Phase integration and inherits cross-standard behaviour from the wider WIA Standards family per the README.md scope statement.
...
with `wia.standard.slug` = `zero-energy-building` and `wia.standard.phase` =
...
`/.well-known/wia-zero-energy-building-capabilities` enumerating per-endpoint
