...
################################################################################
# WIA-BIO-004: Biomarker Data - Installation Script
#
# @version 1.0.0
# @license MIT
...
#
# This script installs the WIA-BIO-004 standard components:
# - CLI tool
# - TypeScript SDK
# - Documentation
################################################################################
...
print_header() {
    echo -e "${TEAL}"
    echo "╔════════════════════════════════════════════════════════════════╗"
    echo "║        📊 WIA-BIO-004: Biomarker Data Installer               ║"
    echo "║                      Version 1.0.0                             ║"
    echo "╚════════════════════════════════════════════════════════════════╝"
    echo -e "${RESET}"
}
...
install_cli() {
    ...
    local cli_path="$SCRIPT_DIR/cli/wia-bio-004.sh"
    ...
    if [ -w "$install_dir" ]; then
        cp "$cli_path" "$install_dir/wia-bio-004"
        print_success "Installed CLI to $install_dir/wia-bio-004"
    else
        print_info "Installing to $install_dir requires sudo..."
        sudo cp "$cli_path" "$install_dir/wia-bio-004"
        print_success "Installed CLI to $install_dir/wia-bio-004 (with sudo)"
    fi

    # Verify installation
    if command -v wia-bio-004 &> /dev/null; then
        print_success "CLI tool is now available as 'wia-bio-004'"
        print_info "Try: wia-bio-004 --help"
    else
...
install_docs() {
    ...
    local docs_dir="$HOME/.wia/bio-004/docs"
...
run_tests() {
    ...
    if command -v wia-bio-004 &> /dev/null; then
        ...
        wia-bio-004 version &> /dev/null && print_success "CLI test passed" || print_error "CLI test failed"
    fi
...
show_completion() {
    ...
    echo ""
    echo -e "${GREEN}The WIA-BIO-004 standard has been successfully installed!${RESET}"
    echo ""
    echo "Available commands:"
    echo -e "${CYAN}  wia-bio-004 calc-roc${RESET}         - Calculate ROC curve and AUC"
    echo -e "${CYAN}  wia-bio-004 validate${RESET}         - Validate biomarker performance"
    echo -e "${CYAN}  wia-bio-004 analyze${RESET}          - Analyze sensitivity and specificity"
    echo -e "${CYAN}  wia-bio-004 report${RESET}           - Generate validation report"
    echo -e "${CYAN}  wia-bio-004 predict${RESET}          - Predict clinical outcomes"
    echo -e "${CYAN}  wia-bio-004 help${RESET}             - Show help message"
    echo ""
    echo "Quick start:"
    echo -e "${GRAY}  # Calculate ROC from biomarker data${RESET}"
    echo -e "${CYAN}  wia-bio-004 calc-roc --data biomarker_results.csv${RESET}"
    echo ""
    echo -e "${GRAY}  # Validate protein biomarker${RESET}"
    echo -e "${CYAN}  wia-bio-004 validate --type protein --data measurements.json${RESET}"
    echo ""
    echo -e "${GRAY}  # Generate comprehensive report${RESET}"
    echo -e "${CYAN}  wia-bio-004 report --input data.csv --output validation_report.txt${RESET}"
    echo ""
    echo "TypeScript SDK:"
    echo -e "${GRAY}  import { calculateROC, validateBiomarker } from '@wia/bio-004';${RESET}"
    echo ""
    echo "Documentation:"
    echo -e "${GRAY}  $HOME/.wia/bio-004/docs/${RESET}"
    echo ""
