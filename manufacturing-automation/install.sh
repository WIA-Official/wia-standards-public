...
################################################################################
# WIA-IND-024: Manufacturing Automation - Installation Script
#
# @version 1.0.0
# @license MIT
...
#
# This script installs the WIA-IND-024 standard components:
# - CLI tool
# - TypeScript SDK
# - Documentation
################################################################################
...
print_header() {
    echo -e "${AMBER}"
    echo "╔════════════════════════════════════════════════════════════════╗"
    echo "║    🏭 WIA-IND-024: Manufacturing Automation Installer         ║"
    echo "║                      Version 1.0.0                             ║"
    echo "╚════════════════════════════════════════════════════════════════╝"
    echo -e "${RESET}"
}
...
install_cli() {
    ...
    local cli_source="$SCRIPT_DIR/cli/wia-ind-024.sh"
    local cli_target="/usr/local/bin/wia-ind-024"
    ...
    if [ -w "/usr/local/bin" ]; then
        ...
        if [ -d "$user_bin" ]; then
            ln -sf "$cli_source" "$user_bin/wia-ind-024"
            print_success "Created symlink in $user_bin/wia-ind-024"
    ...
    # Test CLI
    if command -v wia-ind-024 &> /dev/null; then
        print_success "CLI tool is available: $(which wia-ind-024)"
    else
...
install_typescript() {
    ...
    if [ -f "package.json" ]; then
        ...
        print_info "Import with: import { ManufacturingSDK } from '@wia/ind-024';"
    else
...
install_docs() {
    ...
    if [ -f "$SCRIPT_DIR/spec/WIA-IND-024-v1.0.md" ]; then
        print_success "WIA-IND-024-v1.0.md: Complete specification"
    fi
...
run_tests() {
    ...
    if command -v wia-ind-024 &> /dev/null; then
        ...
        if wia-ind-024 --version &> /dev/null; then
...
print_usage() {
    ...
    echo ""
    echo -e "${AMBER}🏭 Manufacturing Automation CLI${RESET}"
    echo ""
    echo "Usage examples:"
    echo "  wia-ind-024 --version                 # Check version"
    echo "  wia-ind-024 monitor --line LINE-A     # Monitor production line"
    echo "  wia-ind-024 oee --line LINE-A         # Calculate OEE"
    echo "  wia-ind-024 robot move --id ROBOT-A1  # Control robot"
    echo "  wia-ind-024 plc connect --type siemens # Connect to PLC"
    echo ""

    echo -e "${AMBER}📦 TypeScript SDK${RESET}"
    echo ""
    echo "Installation:"
    echo "  npm install @wia/ind-024"
    echo ""
    echo "Usage:"
    echo "  import { ManufacturingSDK } from '@wia/ind-024';"
    echo "  const sdk = new ManufacturingSDK({ facilityId: 'FAC-001' });"
    echo ""

    echo -e "${AMBER}📚 Documentation${RESET}"
    echo ""
    echo "  README.md               : $SCRIPT_DIR/README.md"
    echo "  Full Specification      : $SCRIPT_DIR/spec/WIA-IND-024-v1.0.md"
