...
################################################################################
# WIA-BIO-014: CRISPR Protocol - Installation Script
#
# @version 1.0.0
# @license MIT
...
#
# This script installs the WIA-BIO-014 standard components:
# - CLI tool
# - TypeScript SDK
# - Documentation
################################################################################
...
print_header() {
    echo -e "${TEAL}"
    echo "╔════════════════════════════════════════════════════════════════╗"
    echo "║       ✂️  WIA-BIO-014: CRISPR Protocol Installer              ║"
    echo "║                      Version 1.0.0                             ║"
    echo "╚════════════════════════════════════════════════════════════════╝"
    echo -e "${RESET}"
}
...
install_cli() {
    ...
    local cli_path="$SCRIPT_DIR/cli/wia-bio-014.sh"
    ...
    if [ -w "$install_dir" ]; then
        cp "$cli_path" "$install_dir/wia-bio-014"
        print_success "Installed CLI to $install_dir/wia-bio-014"
    else
        print_info "Installing to $install_dir requires sudo..."
        sudo cp "$cli_path" "$install_dir/wia-bio-014"
        print_success "Installed CLI to $install_dir/wia-bio-014 (with sudo)"
    fi

    # Verify installation
    if command -v wia-bio-014 &> /dev/null; then
        print_success "CLI tool is now available as 'wia-bio-014'"
        print_info "Try: wia-bio-014 --help"
    else
...
install_docs() {
    ...
    local docs_dir="$HOME/.wia/bio-014/docs"
...
run_tests() {
    ...
    if command -v wia-bio-014 &> /dev/null; then
        ...
        wia-bio-014 version &> /dev/null && print_success "CLI test passed" || print_error "CLI test failed"
    fi
...
show_completion() {
    ...
    echo ""
    echo -e "${GREEN}The WIA-BIO-014 CRISPR Protocol standard has been successfully installed!${RESET}"
    echo ""
    echo "Available commands:"
    echo -e "${CYAN}  wia-bio-014 design-grna${RESET}        - Design guide RNA"
    echo -e "${CYAN}  wia-bio-014 predict-offtargets${RESET} - Predict off-target sites"
    echo -e "${CYAN}  wia-bio-014 calc-efficiency${RESET}    - Calculate editing efficiency"
    echo -e "${CYAN}  wia-bio-014 validate${RESET}           - Validate CRISPR protocol"
    echo -e "${CYAN}  wia-bio-014 generate-template${RESET}  - Generate protocol template"
    echo -e "${CYAN}  wia-bio-014 help${RESET}               - Show help message"
    echo ""
    echo "Quick start:"
    echo -e "${GRAY}  # Design guide RNA${RESET}"
    echo -e "${CYAN}  wia-bio-014 design-grna --target 'ATCGATCGATCGATCGATCGGG'${RESET}"
    echo ""
    echo -e "${GRAY}  # Predict off-targets${RESET}"
    echo -e "${CYAN}  wia-bio-014 predict-offtargets --grna 'ATCGATCGATCGATCGATC' --genome hg38${RESET}"
    echo ""
    echo -e "${GRAY}  # Calculate editing efficiency${RESET}"
    echo -e "${CYAN}  wia-bio-014 calc-efficiency --edited 450 --total 1000${RESET}"
    echo ""
    echo "TypeScript SDK:"
    echo -e "${GRAY}  import { designGuideRNA } from '@wia/bio-014';${RESET}"
    echo ""
    echo "Documentation:"
    echo -e "${GRAY}  $HOME/.wia/bio-014/docs/${RESET}"
    echo ""
