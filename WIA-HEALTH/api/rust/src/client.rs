//! HTTP client and API implementations for the WIA-HEALTH SDK
//!
...
impl HealthClient {
    /// Create a new WIA-HEALTH client
    pub fn new(config: Config) -> Self {
        let base_url = config
            .base_url
            .clone()
            .unwrap_or_else(|| "https://api.wiastandards.com/WIA-HEALTH".to_string());
    ...
    /// Create new data
    pub async fn create(&self, request: CreateHealthRequest) -> Result<HealthData> {
    ...
    pub async fn list(&self, params: Option<ListParams>) -> Result<HealthList> {
        let url = format!("{}/data", self.base_url);
