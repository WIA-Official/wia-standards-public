...
################################################################################
# WIA-DEF-020: Autonomous Weapon Ethics - Installation Script
#
# @version 1.0.0
# @license MIT
...
#
# This script installs the WIA-DEF-020 standard components:
# - CLI tool
# - TypeScript SDK
# - Documentation
################################################################################
...
print_header() {
    echo -e "${SLATE}"
    echo "╔════════════════════════════════════════════════════════════════╗"
    echo "║   ⚖️  WIA-DEF-020: Autonomous Weapon Ethics Installer        ║"
    echo "║                      Version 1.0.0                             ║"
    echo "╚════════════════════════════════════════════════════════════════╝"
    echo -e "${RESET}"
}
...
install_cli() {
    ...
    local cli_path="$SCRIPT_DIR/cli/wia-def-020.sh"
    ...
    if [ -w "$install_dir" ]; then
        cp "$cli_path" "$install_dir/wia-def-020"
        print_success "Installed CLI to $install_dir/wia-def-020"
    else
        print_info "Installing to $install_dir requires sudo..."
        sudo cp "$cli_path" "$install_dir/wia-def-020"
        print_success "Installed CLI to $install_dir/wia-def-020 (with sudo)"
    fi

    # Verify installation
    if command -v wia-def-020 &> /dev/null; then
        print_success "CLI tool is now available as 'wia-def-020'"
        print_info "Try: wia-def-020 --help"
    else
...
install_docs() {
    ...
    local docs_dir="$HOME/.wia/def-020/docs"
...
run_tests() {
    ...
    if command -v wia-def-020 &> /dev/null; then
        ...
        wia-def-020 version &> /dev/null && print_success "CLI test passed" || print_error "CLI test failed"
    fi
...
show_completion() {
    ...
    echo ""
    echo -e "${GREEN}The WIA-DEF-020 standard has been successfully installed!${RESET}"
    echo ""
    echo "Available commands:"
    echo -e "${CYAN}  wia-def-020 validate-system${RESET}    - Validate AWS configuration"
    echo -e "${CYAN}  wia-def-020 check-ihl${RESET}          - Check IHL compliance"
    echo -e "${CYAN}  wia-def-020 assess-mhc${RESET}         - Assess Meaningful Human Control"
    echo -e "${CYAN}  wia-def-020 generate-report${RESET}    - Generate ethics report"
    echo -e "${CYAN}  wia-def-020 audit${RESET}              - Audit engagement log"
    echo -e "${CYAN}  wia-def-020 help${RESET}               - Show help message"
    echo ""
    echo "Quick start:"
    echo -e "${GRAY}  # Validate autonomous weapon system${RESET}"
    echo -e "${CYAN}  wia-def-020 validate-system --config aws-config.json${RESET}"
    echo ""
    echo -e "${GRAY}  # Check IHL compliance${RESET}"
    echo -e "${CYAN}  wia-def-020 check-ihl --scenario engagement.json${RESET}"
    echo ""
    echo -e "${GRAY}  # Assess Meaningful Human Control${RESET}"
    echo -e "${CYAN}  wia-def-020 assess-mhc --system AWS-001 --operator certified${RESET}"
    echo ""
    echo "TypeScript SDK:"
    echo -e "${GRAY}  import { EthicsFramework, validateMHC } from '@wia/def-020';${RESET}"
    echo ""
    echo "Documentation:"
    echo -e "${GRAY}  $HOME/.wia/def-020/docs/${RESET}"
    echo ""
    echo -e "${SLATE}⚖️  Ethical Principles:${RESET}"
    echo -e "${GRAY}  • Meaningful Human Control${RESET}"
    echo -e "${GRAY}  • Protection of Civilians${RESET}"
    echo -e "${GRAY}  • Accountability & Transparency${RESET}"
    echo -e "${GRAY}  • IHL Compliance${RESET}"
    echo ""
