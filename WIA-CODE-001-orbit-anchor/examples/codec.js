...
function calibrate(grayObj, H, layout) {
  ...
  const blk = [[c, c]];                                    // 코어 중심(검정)
  for (const a of layout.anchors) if (a.type === 'disk') blk.push([a.mx, a.my]); // 위성 원판(검정)
  const wht = [[d, d], [c + 2.5, c], [-2, -2], [N + 2, -2], [N + 2, N + 2], [-2, N + 2]]; // 도넛구멍·흰링·콰이엇
  const mean = (arr) => { let s = 0; for (const p of arr) s += smp(p[0], p[1]); return s / arr.length; };
  return { b0: mean(blk), w0: mean(wht) };
}
