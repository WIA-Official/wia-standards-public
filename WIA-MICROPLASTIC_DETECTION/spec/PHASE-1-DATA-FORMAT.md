# Microplastic Detection — Phase 1: Data Format Specification
...
Phase 1 defines standardized data formats for Microplastic Detection systems. This specification establishes the core data model, field definitions, validation rules, and serialization formats to ensure interoperability across detection systems and analytical platforms.
...
```json
{
  ...
  "title": "WIA Microplastic Detection Data Format",
  "type": "object",
  "required": ["standard", "version", "timestamp", "data"],
  "properties": {
    "standard": {
      "type": "string",
      "const": "WIA-MICROPLASTIC_DETECTION"
    },
    "version": {
      "type": "string",
      "pattern": "^\\d+\\.\\d+\\.\\d+$"
    ...
    "data": {
      "type": "object",
      "description": "Primary micro-scale measurement data payload"
    }
  }
}
```
...
|-------|------|----------|-------------|
| standard | string | Yes | Standard identifier (WIA-MICROPLASTIC_DETECTION) |
...
| data | object | Yes | Domain-specific micro-scale measurement data |
...
### 5.3 File Extensions
- `.wia-microplastic-detecti.json` — JSON format
- `.wia-microplastic-detecti.pb` — Protocol Buffers
