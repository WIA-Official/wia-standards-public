...
################################################################################
# WIA-IND-015: Travel Tech - Installation Script
#
# @version 1.0.0
# @license MIT
...
#
# This script installs the WIA-IND-015 standard components:
# - CLI tool
# - TypeScript SDK
# - Documentation
################################################################################
...
print_header() {
    echo -e "${AMBER}"
    echo "╔════════════════════════════════════════════════════════════════╗"
    echo "║         ✈️  WIA-IND-015: Travel Tech Installer                ║"
    echo "║                      Version 1.0.0                             ║"
    echo "╚════════════════════════════════════════════════════════════════╝"
    echo -e "${RESET}"
}
...
check_prerequisites() {
    ...
    if command -v curl &> /dev/null; then
        ...
        print_warning "curl not found (required for API features)"
        all_ok=false
    fi
...
install_cli() {
    ...
    local cli_path="$SCRIPT_DIR/cli/wia-ind-015.sh"
    ...
    if [ -w "$install_dir" ]; then
        cp "$cli_path" "$install_dir/wia-ind-015"
        print_success "Installed CLI to $install_dir/wia-ind-015"
    else
        print_info "Installing to $install_dir requires sudo..."
        sudo cp "$cli_path" "$install_dir/wia-ind-015"
        print_success "Installed CLI to $install_dir/wia-ind-015 (with sudo)"
    fi

    # Verify installation
    if command -v wia-ind-015 &> /dev/null; then
        print_success "CLI tool is now available as 'wia-ind-015'"
        print_info "Try: wia-ind-015 --help"
    else
...
install_docs() {
    ...
    local docs_dir="$HOME/.wia/ind-015/docs"
...
run_tests() {
    ...
    if command -v wia-ind-015 &> /dev/null; then
        ...
        wia-ind-015 version &> /dev/null && print_success "CLI test passed" || print_error "CLI test failed"
    fi
...
show_completion() {
    ...
    echo ""
    echo -e "${GREEN}The WIA-IND-015 Travel Tech standard has been successfully installed!${RESET}"
    echo ""
    echo "Available commands:"
    echo -e "${CYAN}  wia-ind-015 search-flights${RESET}      - Search for flight options"
    echo -e "${CYAN}  wia-ind-015 book-hotel${RESET}          - Book hotel accommodation"
    echo -e "${CYAN}  wia-ind-015 create-itinerary${RESET}    - Create complete travel itinerary"
    echo -e "${CYAN}  wia-ind-015 check-documents${RESET}     - Verify travel documents"
    echo -e "${CYAN}  wia-ind-015 convert${RESET}             - Convert currency"
    echo -e "${CYAN}  wia-ind-015 accessible${RESET}          - Find accessible travel options"
    echo -e "${CYAN}  wia-ind-015 help${RESET}                - Show help message"
    echo ""
    echo "Quick start:"
    echo -e "${GRAY}  # Search for flights from JFK to LHR${RESET}"
    echo -e "${CYAN}  wia-ind-015 search-flights --from JFK --to LHR --date 2025-06-15${RESET}"
    echo ""
    echo -e "${GRAY}  # Book hotel in London${RESET}"
    echo -e "${CYAN}  wia-ind-015 book-hotel --city London --checkin 2025-06-15 --checkout 2025-06-22${RESET}"
    echo ""
    echo -e "${GRAY}  # Check travel document requirements${RESET}"
    echo -e "${CYAN}  wia-ind-015 check-documents --passport US --destination GB${RESET}"
    echo ""
    echo -e "${GRAY}  # Convert currency${RESET}"
    echo -e "${CYAN}  wia-ind-015 convert --from USD --to GBP --amount 1000${RESET}"
    echo ""
    echo "TypeScript SDK:"
    echo -e "${GRAY}  import { searchFlights, bookHotel } from '@wia/ind-015';${RESET}"
    echo ""
    echo "Documentation:"
    echo -e "${GRAY}  $HOME/.wia/ind-015/docs/${RESET}"
    echo ""
