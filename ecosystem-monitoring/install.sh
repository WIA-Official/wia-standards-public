...
################################################################################
# WIA-ENE-031: Ecosystem Monitoring Standard - Installation Script
#
# 弘益人間 (홍익人間) - Benefit All Humanity
#
# This script installs dependencies and sets up the WIA-ENE-031
# Ecosystem Monitoring Standard SDK and CLI tools.
#
# Usage:
...
while [[ $# -gt 0 ]]; do
  case $1 in
    ...
    --cli-only)
      ...
      ;;
    --help)
      echo "WIA-ENE-031 Installation Script"
      echo ""
      echo "Usage:"
...
print_header() {
  ...
  echo "╔════════════════════════════════════════════════════════════════╗"
  echo "║                                                                ║"
  echo "║      🌲  WIA-ENE-031: Ecosystem Monitoring Standard           ║"
  echo "║                                                                ║"
...
if $INSTALL_CLI; then
  ...
  if [ -f "${CLI_DIR}/ecosystem-monitoring.sh" ]; then
    chmod +x "${CLI_DIR}/ecosystem-monitoring.sh"
    print_step "CLI script made executable"
  else
    print_error "CLI script not found: ${CLI_DIR}/ecosystem-monitoring.sh"
    exit 1
  fi

  # Create symlink in /usr/local/bin (optional)
  if [ -w "/usr/local/bin" ]; then
    ln -sf "${CLI_DIR}/ecosystem-monitoring.sh" /usr/local/bin/wia-ecosystem 2>/dev/null || true
    if [ -L "/usr/local/bin/wia-ecosystem" ]; then
      print_step "Symlink created: /usr/local/bin/wia-ecosystem"
      print_info "You can now run 'wia-ecosystem' from anywhere"
    fi
  else
    print_warning "Cannot create symlink in /usr/local/bin (no write permission)"
    print_info "You can run the CLI using: ${CLI_DIR}/ecosystem-monitoring.sh"
  fi
...
if $INSTALL_SDK; then
  ...
  echo "  Usage:"
  echo "    import { EcosystemMonitoringSDK } from '@wia/ene-031';"
  echo ""
fi

if $INSTALL_CLI; then
  echo -e "${GREEN}✓${NC} CLI tools installed"
  echo "  Location: ${CLI_DIR}/ecosystem-monitoring.sh"
  echo "  Usage:"
  echo "    ./cli/ecosystem-monitoring.sh --help"
  if [ -L "/usr/local/bin/wia-ecosystem" ]; then
    echo "  Or: wia-ecosystem --help"
  fi
  echo ""
fi
...
echo "   - spec/WIA-ENE-031-v1.0.md: Detailed specification"
echo ""
echo "2. Try the SDK:"
echo "   cd ${SDK_DIR}"
echo "   npm test"
echo ""
echo "3. Try the CLI:"
echo "   ${CLI_DIR}/ecosystem-monitoring.sh --help"
echo ""
echo "4. Join the community:"
echo "   - GitHub: https://github.com/WIA-Official/wia-standards"
echo "   - Website: https://wia.org/standards/ene-031"
