#!/usr/bin/env bash
# voting-system.sh — minimal POSIX shell client for the WIA voting-system standard.
# Demonstrates the request/response contract documented under spec/.
# Requires: curl, jq.
#
# Environment variables:
#   WIA_API_BASE   default https://voting-system.example.com/api/v1
#   WIA_API_TOKEN  bearer token (required for write operations)

set -euo pipefail

API_BASE="${WIA_API_BASE:-https://voting-system.example.com/api/v1}"
TOKEN="${WIA_API_TOKEN:-}"
