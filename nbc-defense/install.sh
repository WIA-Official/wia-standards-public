...
################################################################################
# WIA-DEF-013: NBC Defense - Installation Script
#
# @version 1.0.0
# @license MIT
...
#
# This script installs the WIA-DEF-013 standard components:
# - CLI tool
# - TypeScript SDK
# - Documentation
################################################################################
...
print_header() {
    echo -e "${SLATE}"
    echo "╔════════════════════════════════════════════════════════════════╗"
    echo "║        ☢️  WIA-DEF-013: NBC Defense Installer                 ║"
    echo "║                      Version 1.0.0                             ║"
    echo "╚════════════════════════════════════════════════════════════════╝"
    echo -e "${RESET}"
}
...
install_cli() {
    ...
    local cli_path="$SCRIPT_DIR/cli/wia-def-013.sh"
    ...
    if [ -w "$install_dir" ]; then
        cp "$cli_path" "$install_dir/wia-def-013"
        print_success "Installed CLI to $install_dir/wia-def-013"
    else
        print_info "Installing to $install_dir requires sudo..."
        sudo cp "$cli_path" "$install_dir/wia-def-013"
        print_success "Installed CLI to $install_dir/wia-def-013 (with sudo)"
    fi

    # Verify installation
    if command -v wia-def-013 &> /dev/null; then
        print_success "CLI tool is now available as 'wia-def-013'"
        print_info "Try: wia-def-013 --help"
    else
...
install_docs() {
    ...
    local docs_dir="$HOME/.wia/def-013/docs"
...
run_tests() {
    ...
    if command -v wia-def-013 &> /dev/null; then
        ...
        wia-def-013 version &> /dev/null && print_success "CLI test passed" || print_error "CLI test failed"
    fi
...
show_completion() {
    ...
    echo ""
    echo -e "${GREEN}The WIA-DEF-013 NBC Defense standard has been successfully installed!${RESET}"
    echo ""
    echo "Available commands:"
    echo -e "${CYAN}  wia-def-013 detect-agent${RESET}                - Detect CBRN agents"
    echo -e "${CYAN}  wia-def-013 assess-threat${RESET}               - Assess threat level"
    echo -e "${CYAN}  wia-def-013 calc-protection${RESET}             - Calculate protection requirements"
    echo -e "${CYAN}  wia-def-013 plan-decon${RESET}                  - Plan decontamination"
    echo -e "${CYAN}  wia-def-013 dispense-countermeasures${RESET}    - Manage medical countermeasures"
    echo -e "${CYAN}  wia-def-013 emergency-response${RESET}          - Generate emergency response plan"
    echo -e "${CYAN}  wia-def-013 help${RESET}                        - Show help message"
    echo ""
    echo "Quick start:"
    echo -e "${GRAY}  # Detect chemical agent${RESET}"
    echo -e "${CYAN}  wia-def-013 detect-agent --type chemical --concentration 0.05${RESET}"
    echo ""
    echo -e "${GRAY}  # Assess threat from nerve agent${RESET}"
    echo -e "${CYAN}  wia-def-013 assess-threat --agent sarin --concentration 0.08${RESET}"
    echo ""
    echo -e "${GRAY}  # Calculate protection requirements${RESET}"
    echo -e "${CYAN}  wia-def-013 calc-protection --agent vx --concentration 0.1${RESET}"
    echo ""
    echo "TypeScript SDK:"
    echo -e "${GRAY}  import { detectCBRNAgent } from '@wia/def-013';${RESET}"
    echo ""
    echo "Documentation:"
    echo -e "${GRAY}  $HOME/.wia/def-013/docs/${RESET}"
    echo ""
