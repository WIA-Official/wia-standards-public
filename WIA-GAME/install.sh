...
print_message "$BLUE" "======================================"
print_message "$BLUE" "  WIA-GAME Installation Script"
...
# Make CLI executable
print_message "$BLUE" "\nConfiguring CLI..."
if [ -f "cli/wia-game.sh" ]; then
    chmod +x cli/wia-game.sh
    print_message "$GREEN" "CLI made executable"
else
    print_message "$YELLOW" "Warning: CLI script not found at cli/wia-game.sh"
fi
...
# Test CLI
print_message "$BLUE" "\nTesting CLI..."
if ./cli/wia-game.sh --version &> /dev/null; then
...
print_message "$NC" "  2. Run CLI: ./cli/wia-game.sh --help"
print_message "$NC" "  3. Use SDK:"
echo ""
print_message "$NC" "     import { WIADeFiSDK } from '@wia/game-sdk';"
print_message "$NC" "     const sdk = new WIADeFiSDK({ apiKey: '...', network: 'mainnet' });"
echo ""
print_message "$NC" "Documentation:"
print_message "$NC" "  - Spec: ./spec/WIA-GAME-PHASE1.md"
