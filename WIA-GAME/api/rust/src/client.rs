//! HTTP client and API implementations for the WIA-GAME SDK
//!
...
impl GameClient {
    /// Create a new WIA-GAME client
    pub fn new(config: Config) -> Self {
        let base_url = config
            .base_url
            .clone()
            .unwrap_or_else(|| "https://api.wiastandards.com/WIA-GAME".to_string());
    ...
    pub async fn get(&self, id: &str) -> Result<GameData> {
        ...
        let result: GameResponse = response.json().await?;
        result.data.ok_or_else(|| Error::NotFound(id.to_string()))
    }

    /// Create new data
    pub async fn create(&self, request: CreateGameRequest) -> Result<GameData> {
    ...
    pub async fn list(&self, params: Option<ListParams>) -> Result<GameList> {
        let url = format!("{}/data", self.base_url);
