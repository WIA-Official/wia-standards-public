...
################################################################################
# WIA-ENE-035: Environmental Sensor Network Standard - Installation Script
#
# 弘益人間 (홍익인간) - Benefit All Humanity
#
# This script installs dependencies and sets up the WIA-ENE-035
# Environmental Sensor Network Standard SDK and CLI tools.
#
# Usage:
...
while [[ $# -gt 0 ]]; do
  case $1 in
    ...
    --cli-only)
      ...
      ;;
    --help)
      echo "WIA-ENE-035 Installation Script"
      echo ""
      echo "Usage:"
...
print_header() {
  ...
  echo "╔════════════════════════════════════════════════════════════════╗"
  echo "║                                                                ║"
  echo "║      📡  WIA-ENE-035: Environmental Sensor Standard           ║"
  echo "║                                                                ║"
...
if $INSTALL_CLI; then
  ...
  if [ -f "${CLI_DIR}/environmental-sensor.sh" ]; then
    chmod +x "${CLI_DIR}/environmental-sensor.sh"
    print_step "CLI script made executable"
  else
    print_error "CLI script not found: ${CLI_DIR}/environmental-sensor.sh"
    exit 1
  fi

  # Create symlink in /usr/local/bin (optional)
  if [ -w "/usr/local/bin" ]; then
    ln -sf "${CLI_DIR}/environmental-sensor.sh" /usr/local/bin/wia-env-sensor 2>/dev/null || true
    if [ -L "/usr/local/bin/wia-env-sensor" ]; then
      print_step "Symlink created: /usr/local/bin/wia-env-sensor"
      print_info "You can now run 'wia-env-sensor' from anywhere"
    fi
  else
    print_warning "Cannot create symlink in /usr/local/bin (no write permission)"
    print_info "You can run the CLI using: ${CLI_DIR}/environmental-sensor.sh"
  fi
...
if $INSTALL_SDK; then
  ...
  echo "  Usage:"
  echo "    import { EnvironmentalSensorSDK } from '@wia/ene-035';"
  echo ""
fi

if $INSTALL_CLI; then
  echo -e "${GREEN}✓${NC} CLI tools installed"
  echo "  Location: ${CLI_DIR}/environmental-sensor.sh"
  echo "  Usage:"
  echo "    ./cli/environmental-sensor.sh --help"
  if [ -L "/usr/local/bin/wia-env-sensor" ]; then
    echo "  Or: wia-env-sensor --help"
  fi
  echo ""
fi
...
echo "   - spec/WIA-ENE-035-v1.0.md: Detailed specification"
echo ""
echo "2. Try the SDK:"
echo "   cd ${SDK_DIR}"
echo "   npm test"
echo ""
echo "3. Try the CLI:"
echo "   ${CLI_DIR}/environmental-sensor.sh --help"
echo ""
echo "4. Register your first sensor:"
echo "   ${CLI_DIR}/environmental-sensor.sh register-sensor \\"
echo "     --id SENSOR-001 --type AIR_QUALITY --location '서울시청'"
echo ""
echo "5. Join the community:"
echo "   - GitHub: https://github.com/WIA-Official/wia-standards"
echo "   - Website: https://wia.org/standards/ene-035"
