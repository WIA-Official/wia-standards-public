...
################################################################################
# WIA-IND-030: Circular Economy Standard - Installation Script
#
# @version 1.0.0
# @license MIT
...
#
# This script installs the WIA-IND-030 standard components:
# - CLI tool
# - TypeScript SDK
# - Documentation
################################################################################
...
print_header() {
    echo -e "${AMBER}"
    echo "╔════════════════════════════════════════════════════════════════╗"
    echo "║      ♻️  WIA-IND-030: Circular Economy Standard Installer     ║"
    echo "║                      Version 1.0.0                             ║"
    echo "╚════════════════════════════════════════════════════════════════╝"
    echo -e "${RESET}"
}
...
install_cli() {
    ...
    local cli_path="$SCRIPT_DIR/cli/wia-ind-030.sh"
    ...
    if [ -w "$install_dir" ]; then
        cp "$cli_path" "$install_dir/wia-ind-030"
        print_success "Installed CLI to $install_dir/wia-ind-030"
    else
        print_info "Installing to $install_dir requires sudo..."
        sudo cp "$cli_path" "$install_dir/wia-ind-030"
        print_success "Installed CLI to $install_dir/wia-ind-030 (with sudo)"
    fi

    # Verify installation
    if command -v wia-ind-030 &> /dev/null; then
        print_success "CLI tool is now available as 'wia-ind-030'"
        print_info "Try: wia-ind-030 --help"
    else
...
install_docs() {
    ...
    local docs_dir="$HOME/.wia/ind-030/docs"
...
run_tests() {
    ...
    if command -v wia-ind-030 &> /dev/null; then
        ...
        wia-ind-030 version &> /dev/null && print_success "CLI test passed" || print_error "CLI test failed"
    fi
...
show_completion() {
    ...
    echo ""
    echo -e "${GREEN}The WIA-IND-030 Circular Economy Standard has been successfully installed!${RESET}"
    echo ""
    echo "Available commands:"
    echo -e "${CYAN}  wia-ind-030 create-passport${RESET}    - Create material passport"
    echo -e "${CYAN}  wia-ind-030 track${RESET}              - Track product lifecycle"
    echo -e "${CYAN}  wia-ind-030 circularity${RESET}        - Calculate circularity score"
    echo -e "${CYAN}  wia-ind-030 refurbish${RESET}          - Register refurbishment"
    echo -e "${CYAN}  wia-ind-030 find-recycler${RESET}      - Find recycling facility"
    echo -e "${CYAN}  wia-ind-030 report${RESET}             - Generate circular economy report"
    echo -e "${CYAN}  wia-ind-030 waste-reduction${RESET}    - Calculate waste reduction"
    echo -e "${CYAN}  wia-ind-030 register-paas${RESET}      - Register Product-as-a-Service"
    echo -e "${CYAN}  wia-ind-030 design-assess${RESET}      - Design for circularity assessment"
    echo -e "${CYAN}  wia-ind-030 help${RESET}               - Show help message"
    echo ""
    echo "Quick start:"
    echo -e "${GRAY}  # Create a material passport${RESET}"
    echo -e "${CYAN}  wia-ind-030 create-passport --product PROD-001 --material aluminum --mass 0.5${RESET}"
    echo ""
    echo -e "${GRAY}  # Calculate circularity score${RESET}"
    echo -e "${CYAN}  wia-ind-030 circularity --product PROD-001${RESET}"
    echo ""
    echo -e "${GRAY}  # Find recycling facility${RESET}"
    echo -e "${CYAN}  wia-ind-030 find-recycler --location \"San Francisco\" --material aluminum${RESET}"
    echo ""
    echo "TypeScript SDK:"
    echo -e "${GRAY}  import { CircularEconomySDK } from '@wia/ind-030';${RESET}"
    echo ""
    echo "Documentation:"
    echo -e "${GRAY}  $HOME/.wia/ind-030/docs/${RESET}"
    echo ""
