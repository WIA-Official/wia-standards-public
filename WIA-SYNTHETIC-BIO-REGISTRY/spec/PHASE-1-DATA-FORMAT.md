# Synthetic BIO Registry — Phase 1: Data Format Specification
...
Phase 1 defines standardized data formats for Synthetic BIO Registry systems. This specification establishes the core data model, field definitions, validation rules, and serialization formats to ensure interoperability across biotech research platforms and laboratory systems.
...
```json
{
  ...
  "title": "WIA Synthetic BIO Registry Data Format",
  "type": "object",
  "required": ["standard", "version", "timestamp", "data"],
  "properties": {
    "standard": {
      "type": "string",
      "const": "WIA-SYNTHETIC-BIO-REGISTRY"
    },
    "version": {
      "type": "string",
      "pattern": "^\\d+\\.\\d+\\.\\d+$"
    ...
    "data": {
      "type": "object",
      "description": "Primary biological data payload"
    }
  }
}
```
...
|-------|------|----------|-------------|
| standard | string | Yes | Standard identifier (WIA-SYNTHETIC-BIO-REGISTRY) |
...
| data | object | Yes | Domain-specific biological data |
...
### 5.3 File Extensions
- `.wia-synthetic-bio-regist.json` — JSON format
- `.wia-synthetic-bio-regist.pb` — Protocol Buffers
...
```json
{
  "standard": "WIA-SYNTHETIC-BIO-REGISTRY",
  "version": "1.0.0",
