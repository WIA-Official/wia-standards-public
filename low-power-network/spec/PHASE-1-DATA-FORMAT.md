# LOW Power Network — Phase 1: Data Format Specification
...
Phase 1 defines standardized data formats for LOW Power Network systems. This specification establishes the core data model, field definitions, validation rules, and serialization formats to ensure interoperability across network management systems and network infrastructure.
...
```json
{
  ...
  "title": "WIA LOW Power Network Data Format",
  "type": "object",
  "required": ["standard", "version", "timestamp", "data"],
  "properties": {
    "standard": {
      "type": "string",
      "const": "low-power-network"
    },
    "version": {
      "type": "string",
      "pattern": "^\\d+\\.\\d+\\.\\d+$"
    ...
    "data": {
      "type": "object",
      "description": "Primary network traffic data payload"
    }
  }
}
```
...
|-------|------|----------|-------------|
| standard | string | Yes | Standard identifier (low-power-network) |
...
| data | object | Yes | Domain-specific network traffic data |
...
### 5.3 File Extensions
- `.wia-low-power-network.json` — JSON format
- `.wia-low-power-network.pb` — Protocol Buffers
