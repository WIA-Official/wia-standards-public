...
################################################################################
# WIA-AUTO-021: Vehicle Lightweight Material - Installation Script
#
# @version 1.0.0
# @license MIT
...
#
# This script installs the WIA-AUTO-021 standard components:
# - CLI tool
# - TypeScript SDK
# - Documentation
################################################################################
...
print_header() {
    echo -e "${ORANGE}"
    echo "╔════════════════════════════════════════════════════════════════╗"
    echo "║   🚗 WIA-AUTO-021: Vehicle Lightweight Material Installer     ║"
    echo "║                      Version 1.0.0                             ║"
    echo "╚════════════════════════════════════════════════════════════════╝"
    echo -e "${RESET}"
}
...
install_cli() {
    ...
    local cli_path="$SCRIPT_DIR/cli/wia-auto-021.sh"
    ...
    if [ -w "$install_dir" ]; then
        cp "$cli_path" "$install_dir/wia-auto-021"
        print_success "Installed CLI to $install_dir/wia-auto-021"
    else
        print_info "Installing to $install_dir requires sudo..."
        sudo cp "$cli_path" "$install_dir/wia-auto-021"
        print_success "Installed CLI to $install_dir/wia-auto-021 (with sudo)"
    fi

    # Verify installation
    if command -v wia-auto-021 &> /dev/null; then
        print_success "CLI tool is now available as 'wia-auto-021'"
        print_info "Try: wia-auto-021 --help"
    else
...
install_docs() {
    ...
    local docs_dir="$HOME/.wia/auto-021/docs"
...
run_tests() {
    ...
    if command -v wia-auto-021 &> /dev/null; then
        ...
        wia-auto-021 version &> /dev/null && print_success "CLI test passed" || print_error "CLI test failed"
    fi
...
show_completion() {
    ...
    echo ""
    echo -e "${GREEN}The WIA-AUTO-021 standard has been successfully installed!${RESET}"
    echo ""
    echo "Available commands:"
    echo -e "${CYAN}  wia-auto-021 calc-weight${RESET}      - Calculate weight reduction"
    echo -e "${CYAN}  wia-auto-021 material${RESET}         - Show material properties"
    echo -e "${CYAN}  wia-auto-021 compare${RESET}          - Compare materials"
    echo -e "${CYAN}  wia-auto-021 crash-test${RESET}       - Simulate crash test"
    echo -e "${CYAN}  wia-auto-021 cost-benefit${RESET}     - Cost-benefit analysis"
    echo -e "${CYAN}  wia-auto-021 list${RESET}             - List available materials"
    echo -e "${CYAN}  wia-auto-021 help${RESET}             - Show help message"
    echo ""
    echo "Quick start:"
    echo -e "${GRAY}  # Calculate weight reduction from steel to aluminum hood${RESET}"
    echo -e "${CYAN}  wia-auto-021 calc-weight --component hood --from steel --to aluminum${RESET}"
    echo ""
    echo -e "${GRAY}  # Show CFRP material properties${RESET}"
    echo -e "${CYAN}  wia-auto-021 material --name cfrp${RESET}"
    echo ""
    echo -e "${GRAY}  # Compare multiple materials${RESET}"
    echo -e "${CYAN}  wia-auto-021 compare steel aluminum cfrp${RESET}"
    echo ""
    echo -e "${GRAY}  # Crash test simulation${RESET}"
    echo -e "${CYAN}  wia-auto-021 crash-test --material aluminum --impact-speed 60${RESET}"
    echo ""
    echo "TypeScript SDK:"
    echo -e "${GRAY}  import { calculateWeightReduction } from '@wia/auto-021';${RESET}"
    echo ""
    echo "Documentation:"
    echo -e "${GRAY}  $HOME/.wia/auto-021/docs/${RESET}"
    echo ""
