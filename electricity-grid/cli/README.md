# `electricity-grid` CLI

Minimal POSIX shell client that demonstrates the WIA `electricity-grid` standard's
...
```sh
export WIA_API_BASE="https://electricity-grid.example.com/api/v1"
export WIA_API_TOKEN="<your-bearer-token>"
./electricity-grid.sh list
./electricity-grid.sh get <id>
./electricity-grid.sh create payload.json
./electricity-grid.sh validate payload.json
./electricity-grid.sh export <id>
```
