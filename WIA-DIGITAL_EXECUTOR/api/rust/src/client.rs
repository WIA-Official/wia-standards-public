//! HTTP client and API implementations for the WIA-DIGITAL_EXECUTOR SDK
//!
...
/// Main WIA-DIGITAL_EXECUTOR SDK client
///
/// # Example
///
/// ```rust,no_run
...
/// }
/// ```
pub struct DigitalExecutorClient {
    http: Client,
    config: Config,
    base_url: String,
}

impl DigitalExecutorClient {
    /// Create a new WIA-DIGITAL_EXECUTOR client
    pub fn new(config: Config) -> Self {
        let base_url = config
            .base_url
            .clone()
            .unwrap_or_else(|| "https://api.wiastandards.com/WIA-DIGITAL_EXECUTOR".to_string());
    ...
    /// Validate data against WIA-DIGITAL_EXECUTOR standards
    pub async fn validate(&self, data: &serde_json::Value) -> Result<ValidationResult> {
        validators::validate_data(data)
    }

    /// Get data by ID
    pub async fn get(&self, id: &str) -> Result<DigitalExecutorData> {
        ...
        let result: DigitalExecutorResponse = response.json().await?;
        result.data.ok_or_else(|| Error::NotFound(id.to_string()))
    }

    /// Create new data
    pub async fn create(&self, request: CreateDigitalExecutorRequest) -> Result<DigitalExecutorData> {
    ...
    pub async fn list(&self, params: Option<ListParams>) -> Result<DigitalExecutorList> {
        let url = format!("{}/data", self.base_url);
