...
################################################################################
# WIA-AUTO-022: Vehicle Safety - Installation Script
#
# @version 1.0.0
# @license MIT
...
#
# This script installs the WIA-AUTO-022 standard components:
# - CLI tool
# - TypeScript SDK
# - Documentation
################################################################################
...
print_header() {
    echo -e "${ORANGE}"
    echo "╔════════════════════════════════════════════════════════════════╗"
    echo "║      🚗 WIA-AUTO-022: Vehicle Safety Installer                ║"
    echo "║                      Version 1.0.0                             ║"
    echo "╚════════════════════════════════════════════════════════════════╝"
    echo -e "${RESET}"
}
...
install_cli() {
    ...
    local cli_path="$SCRIPT_DIR/cli/wia-auto-022.sh"
    ...
    if [ -w "$install_dir" ]; then
        cp "$cli_path" "$install_dir/wia-auto-022"
        print_success "Installed CLI to $install_dir/wia-auto-022"
    else
        print_info "Installing to $install_dir requires sudo..."
        sudo cp "$cli_path" "$install_dir/wia-auto-022"
        print_success "Installed CLI to $install_dir/wia-auto-022 (with sudo)"
    fi

    # Verify installation
    if command -v wia-auto-022 &> /dev/null; then
        print_success "CLI tool is now available as 'wia-auto-022'"
        print_info "Try: wia-auto-022 --help"
    else
...
install_docs() {
    ...
    local docs_dir="$HOME/.wia/auto-022/docs"
...
run_tests() {
    ...
    if command -v wia-auto-022 &> /dev/null; then
        ...
        wia-auto-022 version &> /dev/null && print_success "CLI test passed" || print_error "CLI test failed"
    fi
...
show_completion() {
    ...
    echo ""
    echo -e "${GREEN}The WIA-AUTO-022 standard has been successfully installed!${RESET}"
    echo ""
    echo "Available commands:"
    echo -e "${CYAN}  wia-auto-022 assess-crash${RESET}      - Assess crash safety and injury risk"
    echo -e "${CYAN}  wia-auto-022 airbag-deploy${RESET}    - Evaluate airbag deployment"
    echo -e "${CYAN}  wia-auto-022 safety-rating${RESET}    - Calculate NCAP safety rating"
    echo -e "${CYAN}  wia-auto-022 child-safety${RESET}     - Assess child restraint safety"
    echo -e "${CYAN}  wia-auto-022 simulate${RESET}         - Run crash test simulation"
    echo -e "${CYAN}  wia-auto-022 help${RESET}             - Show help message"
    echo ""
    echo "Quick start:"
    echo -e "${GRAY}  # Assess frontal crash at 64 km/h${RESET}"
    echo -e "${CYAN}  wia-auto-022 assess-crash --mass 1600 --velocity 64 --angle 0${RESET}"
    echo ""
    echo -e "${GRAY}  # Evaluate airbag deployment${RESET}"
    echo -e "${CYAN}  wia-auto-022 airbag-deploy --crash-severity 9 --occupant-position all${RESET}"
    echo ""
    echo -e "${GRAY}  # Calculate NCAP safety rating${RESET}"
    echo -e "${CYAN}  wia-auto-022 safety-rating --frontal 15.2 --side 16.5 --pole 7.8${RESET}"
    echo ""
    echo "TypeScript SDK:"
    echo -e "${GRAY}  import { assessCrashSafety } from '@wia/auto-022';${RESET}"
    echo ""
    echo "Documentation:"
    echo -e "${GRAY}  $HOME/.wia/auto-022/docs/${RESET}"
    echo ""
