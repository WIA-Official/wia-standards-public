...
################################################################################
# WIA-BIO-010: Clinical Trial Data - Installation Script
#
# @version 1.0.0
# @license MIT
...
#
# This script installs the WIA-BIO-010 standard components:
# - CLI tool
# - TypeScript SDK
# - Documentation
################################################################################
...
print_header() {
    echo -e "${TEAL}"
    echo "╔════════════════════════════════════════════════════════════════╗"
    echo "║      📋 WIA-BIO-010: Clinical Trial Data Installer            ║"
    echo "║                      Version 1.0.0                             ║"
    echo "╚════════════════════════════════════════════════════════════════╝"
    echo -e "${RESET}"
}
...
install_cli() {
    ...
    local cli_path="$SCRIPT_DIR/cli/wia-bio-010.sh"
    ...
    if [ -w "$install_dir" ]; then
        cp "$cli_path" "$install_dir/wia-bio-010"
        print_success "Installed CLI to $install_dir/wia-bio-010"
    else
        print_info "Installing to $install_dir requires sudo..."
        sudo cp "$cli_path" "$install_dir/wia-bio-010"
        print_success "Installed CLI to $install_dir/wia-bio-010 (with sudo)"
    fi

    # Verify installation
    if command -v wia-bio-010 &> /dev/null; then
        print_success "CLI tool is now available as 'wia-bio-010'"
        print_info "Try: wia-bio-010 --help"
    else
...
install_docs() {
    ...
    local docs_dir="$HOME/.wia/bio-010/docs"
...
run_tests() {
    ...
    if command -v wia-bio-010 &> /dev/null; then
        ...
        wia-bio-010 version &> /dev/null && print_success "CLI test passed" || print_error "CLI test failed"
    fi
...
show_completion() {
    ...
    echo ""
    echo -e "${GREEN}The WIA-BIO-010 standard has been successfully installed!${RESET}"
    echo ""
    echo "Available commands:"
    echo -e "${CYAN}  wia-bio-010 register-trial${RESET}       - Register new clinical trial"
    echo -e "${CYAN}  wia-bio-010 record-data${RESET}          - Record patient data"
    echo -e "${CYAN}  wia-bio-010 report-ae${RESET}            - Report adverse event"
    echo -e "${CYAN}  wia-bio-010 calc-sample-size${RESET}     - Calculate sample size"
    echo -e "${CYAN}  wia-bio-010 calc-power${RESET}           - Calculate statistical power"
    echo -e "${CYAN}  wia-bio-010 generate-submission${RESET}  - Generate regulatory submission"
    echo -e "${CYAN}  wia-bio-010 help${RESET}                 - Show help message"
    echo ""
    echo "Quick start:"
    echo -e "${GRAY}  # Register Phase III trial${RESET}"
    echo -e "${CYAN}  wia-bio-010 register-trial --title \"Diabetes Study\" --phase III${RESET}"
    echo ""
    echo -e "${GRAY}  # Calculate sample size${RESET}"
    echo -e "${CYAN}  wia-bio-010 calc-sample-size --alpha 0.05 --power 0.80 --effect-size 0.5${RESET}"
    echo ""
    echo -e "${GRAY}  # Record patient data${RESET}"
    echo -e "${CYAN}  wia-bio-010 record-data --trial-id NCT12345678 --patient-id PT-001 --hba1c 6.8${RESET}"
    echo ""
    echo "TypeScript SDK:"
    echo -e "${GRAY}  import { registerTrial, calculateSampleSize } from '@wia/bio-010';${RESET}"
    echo ""
    echo "Documentation:"
    echo -e "${GRAY}  $HOME/.wia/bio-010/docs/${RESET}"
    echo ""
