...
The WIA-HERITAGE-008 standard defines a comprehensive framework for digitizing, preserving, and sharing cultural artifacts through advanced 3D scanning, photogrammetry, AI-powered restoration, and blockchain-based provenance tracking.
...
```bash
curl -fsSL https://raw.githubusercontent.com/WIA-Official/wia-standards/main/standards/WIA-HERITAGE-008-folklore-archive/install.sh | bash
```
...
```bash
wia-heritage-008 scan --input photos/ --output artifact.gltf --metadata metadata.json
```

### Create Virtual Exhibition

```bash
wia-heritage-008 exhibit --artifacts artifacts/ --output exhibition/ --theme ancient-civilizations
```

### Verify Provenance

```bash
wia-heritage-008 provenance --artifact artifact.json --blockchain ethereum --verify
```
...
```typescript
import { WIAHeritage008 } from '@wia/heritage-008';

const heritage = new WIAHeritage008({
  apiKey: 'your-api-key',
  blockchain: 'ethereum'
});
