...
################################################################################
# WIA-BIO-017: Bio-Safety - Installation Script
#
# @version 1.0.0
# @license MIT
...
#
# This script installs the WIA-BIO-017 standard components:
# - CLI tool
# - TypeScript SDK
# - Documentation
################################################################################
...
print_header() {
    echo -e "${TEAL}"
    echo "╔════════════════════════════════════════════════════════════════╗"
    echo "║           ☣️  WIA-BIO-017: Bio-Safety Installer               ║"
    echo "║                      Version 1.0.0                             ║"
    echo "╚════════════════════════════════════════════════════════════════╝"
    echo -e "${RESET}"
}
...
install_cli() {
    ...
    local cli_path="$SCRIPT_DIR/cli/wia-bio-017.sh"
    ...
    if [ -w "$install_dir" ]; then
        cp "$cli_path" "$install_dir/wia-bio-017"
        print_success "Installed CLI to $install_dir/wia-bio-017"
    else
        print_info "Installing to $install_dir requires sudo..."
        sudo cp "$cli_path" "$install_dir/wia-bio-017"
        print_success "Installed CLI to $install_dir/wia-bio-017 (with sudo)"
    fi

    # Verify installation
    if command -v wia-bio-017 &> /dev/null; then
        print_success "CLI tool is now available as 'wia-bio-017'"
        print_info "Try: wia-bio-017 --help"
    else
...
install_docs() {
    ...
    local docs_dir="$HOME/.wia/bio-017/docs"
...
run_tests() {
    ...
    if command -v wia-bio-017 &> /dev/null; then
        ...
        wia-bio-017 version &> /dev/null && print_success "CLI test passed" || print_error "CLI test failed"
    fi
...
show_completion() {
    ...
    echo ""
    echo -e "${GREEN}The WIA-BIO-017 Bio-Safety standard has been successfully installed!${RESET}"
    echo ""
    echo "Available commands:"
    echo -e "${CYAN}  wia-bio-017 calc-risk${RESET}         - Calculate biosafety risk score"
    echo -e "${CYAN}  wia-bio-017 validate-bsl${RESET}      - Validate BSL compliance"
    echo -e "${CYAN}  wia-bio-017 generate-ppe${RESET}      - Generate PPE requirements"
    echo -e "${CYAN}  wia-bio-017 assess-exposure${RESET}   - Assess exposure incident"
    echo -e "${CYAN}  wia-bio-017 help${RESET}              - Show help message"
    echo ""
    echo "Quick start:"
    echo -e "${GRAY}  # Calculate risk score for BSL-3 pathogen${RESET}"
    echo -e "${CYAN}  wia-bio-017 calc-risk --hazard 3 --probability 0.15 --impact 8${RESET}"
    echo ""
    echo -e "${GRAY}  # Validate BSL-3 compliance${RESET}"
    echo -e "${CYAN}  wia-bio-017 validate-bsl --pathogen BSL-3 --facility BSL-3 --procedure aerosol${RESET}"
    echo ""
    echo -e "${GRAY}  # Generate PPE requirements for BSL-3${RESET}"
    echo -e "${CYAN}  wia-bio-017 generate-ppe --bsl 3 --procedure 'viral culture'${RESET}"
    echo ""
    echo "TypeScript SDK:"
    echo -e "${GRAY}  import { calculateRiskScore } from '@wia/bio-017';${RESET}"
    echo ""
    echo "Documentation:"
    echo -e "${GRAY}  $HOME/.wia/bio-017/docs/${RESET}"
    echo ""
