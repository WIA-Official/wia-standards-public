# Wireless Power Transfer — Phase 1: Data Format Specification
...
Phase 1 defines standardized data formats for Wireless Power Transfer systems. This specification establishes the core data model, field definitions, validation rules, and serialization formats to ensure interoperability across radio communication systems and wireless networks.
...
```json
{
  ...
  "title": "WIA Wireless Power Transfer Data Format",
  "type": "object",
  "required": ["standard", "version", "timestamp", "data"],
  "properties": {
    "standard": {
      "type": "string",
      "const": "wireless-power-transfer"
    },
    "version": {
      "type": "string",
      "pattern": "^\\d+\\.\\d+\\.\\d+$"
    ...
    "data": {
      "type": "object",
      "description": "Primary wireless signal data payload"
    }
  }
}
```
...
|-------|------|----------|-------------|
| standard | string | Yes | Standard identifier (wireless-power-transfer) |
...
| data | object | Yes | Domain-specific wireless signal data |
...
### 5.3 File Extensions
- `.wia-wireless-power-trans.json` — JSON format
- `.wia-wireless-power-trans.pb` — Protocol Buffers
...
```json
{
  "standard": "wireless-power-transfer",
  "version": "1.0.0",
