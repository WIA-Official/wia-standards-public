...
################################################################################
# WIA-BIO-008: Protein Structure - Installation Script
#
# @version 1.0.0
# @license MIT
...
#
# This script installs the WIA-BIO-008 standard components:
# - CLI tool
# - TypeScript SDK
# - Documentation
################################################################################
...
print_header() {
    echo -e "${TEAL}"
    echo "╔════════════════════════════════════════════════════════════════╗"
    echo "║        🔬 WIA-BIO-008: Protein Structure Installer            ║"
    echo "║                      Version 1.0.0                             ║"
    echo "╚════════════════════════════════════════════════════════════════╝"
    echo -e "${RESET}"
}
...
check_prerequisites() {
    ...
    # Check Python (optional, for structure analysis)
    if command -v python3 &> /dev/null; then
        print_success "Python: $(python3 --version)"
    else
        print_warning "Python not found (optional, for advanced analysis)"
    fi
...
install_cli() {
    ...
    local cli_path="$SCRIPT_DIR/cli/wia-bio-008.sh"
    ...
    if [ -w "$install_dir" ]; then
        cp "$cli_path" "$install_dir/wia-bio-008"
        print_success "Installed CLI to $install_dir/wia-bio-008"
    else
        print_info "Installing to $install_dir requires sudo..."
        sudo cp "$cli_path" "$install_dir/wia-bio-008"
        print_success "Installed CLI to $install_dir/wia-bio-008 (with sudo)"
    fi

    # Verify installation
    if command -v wia-bio-008 &> /dev/null; then
        print_success "CLI tool is now available as 'wia-bio-008'"
        print_info "Try: wia-bio-008 --help"
    else
...
install_docs() {
    ...
    local docs_dir="$HOME/.wia/bio-008/docs"
...
run_tests() {
    ...
    if command -v wia-bio-008 &> /dev/null; then
        ...
        wia-bio-008 version &> /dev/null && print_success "CLI test passed" || print_error "CLI test failed"
    fi
...
show_completion() {
    ...
    echo ""
    echo -e "${GREEN}The WIA-BIO-008 standard has been successfully installed!${RESET}"
    echo ""
    echo "Available commands:"
    echo -e "${CYAN}  wia-bio-008 predict${RESET}         - Predict protein structure"
    echo -e "${CYAN}  wia-bio-008 rmsd${RESET}            - Calculate RMSD"
    echo -e "${CYAN}  wia-bio-008 dock${RESET}            - Protein-ligand docking"
    echo -e "${CYAN}  wia-bio-008 simulate${RESET}        - Molecular dynamics"
    echo -e "${CYAN}  wia-bio-008 validate${RESET}        - Quality assessment"
    echo -e "${CYAN}  wia-bio-008 help${RESET}            - Show help message"
    echo ""
    echo "Quick start:"
    echo -e "${GRAY}  # Predict structure from sequence${RESET}"
    echo -e "${CYAN}  wia-bio-008 predict --sequence 'MKTAYIAK...' --method alphafold${RESET}"
    echo ""
    echo -e "${GRAY}  # Calculate RMSD between structures${RESET}"
    echo -e "${CYAN}  wia-bio-008 rmsd --structure1 model.pdb --structure2 native.pdb${RESET}"
    echo ""
    echo -e "${GRAY}  # Perform docking${RESET}"
    echo -e "${CYAN}  wia-bio-008 dock --protein target.pdb --ligand 'CC(C)Cc1ccc(cc1)C(C)C(O)=O'${RESET}"
    echo ""
    echo "TypeScript SDK:"
    echo -e "${GRAY}  import { predictStructure, calculateRMSD } from '@wia/bio-008';${RESET}"
    echo ""
    echo "Documentation:"
    echo -e "${GRAY}  $HOME/.wia/bio-008/docs/${RESET}"
    echo ""
