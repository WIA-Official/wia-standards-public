...
################################################################################
# WIA-AUTO-012: Traffic Management - Installation Script
#
# @version 1.0.0
# @license MIT
...
#
# This script installs the WIA-AUTO-012 standard components:
# - CLI tool
# - TypeScript SDK
# - Documentation
################################################################################
...
print_header() {
    echo -e "${ORANGE}"
    echo "╔════════════════════════════════════════════════════════════════╗"
    echo "║       🚦 WIA-AUTO-012: Traffic Management Installer           ║"
    echo "║                      Version 1.0.0                             ║"
    echo "╚════════════════════════════════════════════════════════════════╝"
    echo -e "${RESET}"
}
...
install_cli() {
    ...
    local cli_path="$SCRIPT_DIR/cli/wia-auto-012.sh"
    ...
    if [ -w "$install_dir" ]; then
        cp "$cli_path" "$install_dir/wia-auto-012"
        print_success "Installed CLI to $install_dir/wia-auto-012"
    else
        print_info "Installing to $install_dir requires sudo..."
        sudo cp "$cli_path" "$install_dir/wia-auto-012"
        print_success "Installed CLI to $install_dir/wia-auto-012 (with sudo)"
    fi

    # Verify installation
    if command -v wia-auto-012 &> /dev/null; then
        print_success "CLI tool is now available as 'wia-auto-012'"
        print_info "Try: wia-auto-012 --help"
    else
...
install_docs() {
    ...
    local docs_dir="$HOME/.wia/auto-012/docs"
...
run_tests() {
    ...
    if command -v wia-auto-012 &> /dev/null; then
        ...
        wia-auto-012 version &> /dev/null && print_success "CLI test passed" || print_error "CLI test failed"
    fi
...
show_completion() {
    ...
    echo ""
    echo -e "${GREEN}The WIA-AUTO-012 standard has been successfully installed!${RESET}"
    echo ""
    echo "Available commands:"
    echo -e "${CYAN}  wia-auto-012 calc-flow${RESET}         - Calculate traffic flow metrics"
    echo -e "${CYAN}  wia-auto-012 optimize-signal${RESET}   - Optimize signal timing"
    echo -e "${CYAN}  wia-auto-012 detect-congestion${RESET} - Detect congestion"
    echo -e "${CYAN}  wia-auto-012 predict${RESET}           - Predict traffic conditions"
    echo -e "${CYAN}  wia-auto-012 help${RESET}              - Show help message"
    echo ""
    echo "Quick start:"
    echo -e "${GRAY}  # Calculate traffic flow${RESET}"
    echo -e "${CYAN}  wia-auto-012 calc-flow --density 45 --speed 60 --lanes 3${RESET}"
    echo ""
    echo -e "${GRAY}  # Optimize signal timing${RESET}"
    echo -e "${CYAN}  wia-auto-012 optimize-signal --phases \"1200,900\" --lost-time 4${RESET}"
    echo ""
    echo -e "${GRAY}  # Detect congestion${RESET}"
    echo -e "${CYAN}  wia-auto-012 detect-congestion --speed 25 --density 120${RESET}"
    echo ""
    echo "TypeScript SDK:"
    echo -e "${GRAY}  import { calculateTrafficFlow } from '@wia/auto-012';${RESET}"
    echo ""
    echo "Documentation:"
    echo -e "${GRAY}  $HOME/.wia/auto-012/docs/${RESET}"
    echo ""
