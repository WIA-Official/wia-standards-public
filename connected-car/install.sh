...
################################################################################
# WIA-AUTO-008: Connected Car - Installation Script
#
# @version 1.0.0
# @license MIT
...
#
# This script installs the WIA-AUTO-008 standard components:
# - CLI tool
# - TypeScript SDK
# - Documentation
################################################################################
...
print_header() {
    echo -e "${ORANGE}"
    echo "╔════════════════════════════════════════════════════════════════╗"
    echo "║       🚗 WIA-AUTO-008: Connected Car Installer                ║"
    echo "║                      Version 1.0.0                             ║"
    echo "╚════════════════════════════════════════════════════════════════╝"
    echo -e "${RESET}"
}
...
install_cli() {
    ...
    local cli_path="$SCRIPT_DIR/cli/wia-auto-008.sh"
    ...
    if [ -w "$install_dir" ]; then
        cp "$cli_path" "$install_dir/wia-auto-008"
        print_success "Installed CLI to $install_dir/wia-auto-008"
    else
        print_info "Installing to $install_dir requires sudo..."
        sudo cp "$cli_path" "$install_dir/wia-auto-008"
        print_success "Installed CLI to $install_dir/wia-auto-008 (with sudo)"
    fi

    # Verify installation
    if command -v wia-auto-008 &> /dev/null; then
        print_success "CLI tool is now available as 'wia-auto-008'"
        print_info "Try: wia-auto-008 --help"
    else
...
install_docs() {
    ...
    local docs_dir="$HOME/.wia/auto-008/docs"
...
run_tests() {
    ...
    if command -v wia-auto-008 &> /dev/null; then
        ...
        wia-auto-008 version &> /dev/null && print_success "CLI test passed" || print_error "CLI test failed"
    fi
...
show_completion() {
    ...
    echo ""
    echo -e "${GREEN}The WIA-AUTO-008 standard has been successfully installed!${RESET}"
    echo ""
    echo "Available commands:"
    echo -e "${CYAN}  wia-auto-008 telemetry${RESET}        - Collect vehicle telemetry"
    echo -e "${CYAN}  wia-auto-008 ota-check${RESET}        - Check for OTA updates"
    echo -e "${CYAN}  wia-auto-008 ota-install${RESET}      - Install OTA update"
    echo -e "${CYAN}  wia-auto-008 diagnostics${RESET}      - Run remote diagnostics"
    echo -e "${CYAN}  wia-auto-008 connectivity${RESET}     - Monitor connectivity"
    echo -e "${CYAN}  wia-auto-008 export${RESET}           - Export telemetry data"
    echo -e "${CYAN}  wia-auto-008 help${RESET}             - Show help message"
    echo ""
    echo "Quick start:"
    echo -e "${GRAY}  # Collect telemetry for 60 seconds${RESET}"
    echo -e "${CYAN}  wia-auto-008 telemetry --vin 1HGBH41JXMN109186 --duration 60${RESET}"
    echo ""
    echo -e "${GRAY}  # Check for OTA updates${RESET}"
    echo -e "${CYAN}  wia-auto-008 ota-check --vin 1HGBH41JXMN109186${RESET}"
    echo ""
    echo -e "${GRAY}  # Run comprehensive diagnostics${RESET}"
    echo -e "${CYAN}  wia-auto-008 diagnostics --vin 1HGBH41JXMN109186 --level comprehensive${RESET}"
    echo ""
    echo "TypeScript SDK:"
    echo -e "${GRAY}  import { ConnectedCarSDK } from '@wia/auto-008';${RESET}"
    echo ""
    echo "Documentation:"
    echo -e "${GRAY}  $HOME/.wia/auto-008/docs/${RESET}"
    echo ""
