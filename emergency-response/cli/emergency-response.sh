#!/usr/bin/env bash
# emergency-response.sh — minimal POSIX shell client for the WIA emergency-response standard.
# Demonstrates the request/response contract documented under spec/.
# Requires: curl, jq.
#
# Environment variables:
#   WIA_API_BASE   default https://emergency-response.example.com/api/v1
#   WIA_API_TOKEN  bearer token (required for write operations)

set -euo pipefail

API_BASE="${WIA_API_BASE:-https://emergency-response.example.com/api/v1}"
TOKEN="${WIA_API_TOKEN:-}"
