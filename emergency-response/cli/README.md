# `emergency-response` CLI

Minimal POSIX shell client that demonstrates the WIA `emergency-response` standard's
...
```sh
export WIA_API_BASE="https://emergency-response.example.com/api/v1"
export WIA_API_TOKEN="<your-bearer-token>"
./emergency-response.sh list
./emergency-response.sh get <id>
./emergency-response.sh create payload.json
./emergency-response.sh validate payload.json
./emergency-response.sh export <id>
```
