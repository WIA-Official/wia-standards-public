//! HTTP client and API implementations for the WIA-IOT_SECURITY SDK
//!
...
/// Main WIA-IOT_SECURITY SDK client
///
/// # Example
///
/// ```rust,no_run
...
/// }
/// ```
pub struct IotSecurityClient {
    http: Client,
    config: Config,
    base_url: String,
}

impl IotSecurityClient {
    /// Create a new WIA-IOT_SECURITY client
    pub fn new(config: Config) -> Self {
        let base_url = config
            .base_url
            .clone()
            .unwrap_or_else(|| "https://api.wiastandards.com/WIA-IOT_SECURITY".to_string());
    ...
    pub async fn get(&self, id: &str) -> Result<IotSecurityData> {
        ...
        let result: IotSecurityResponse = response.json().await?;
        result.data.ok_or_else(|| Error::NotFound(id.to_string()))
    }

    /// Create new data
    pub async fn create(&self, request: CreateIotSecurityRequest) -> Result<IotSecurityData> {
    ...
    pub async fn list(&self, params: Option<ListParams>) -> Result<IotSecurityList> {
        let url = format!("{}/data", self.base_url);
