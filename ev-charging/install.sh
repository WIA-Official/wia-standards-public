...
################################################################################
# WIA-AUTO-005: EV Charging - Installation Script
#
# @version 1.0.0
# @license MIT
...
#
# This script installs the WIA-AUTO-005 standard components:
# - CLI tool
# - TypeScript SDK
# - Documentation
################################################################################
...
print_header() {
    echo -e "${ORANGE}"
    echo "╔════════════════════════════════════════════════════════════════╗"
    echo "║         🔌 WIA-AUTO-005: EV Charging Installer               ║"
    echo "║                      Version 1.0.0                             ║"
    echo "╚════════════════════════════════════════════════════════════════╝"
    echo -e "${RESET}"
}
...
install_cli() {
    ...
    local cli_path="$SCRIPT_DIR/cli/wia-auto-005.sh"
    ...
    if [ -w "$install_dir" ]; then
        cp "$cli_path" "$install_dir/wia-auto-005"
        print_success "Installed CLI to $install_dir/wia-auto-005"
    else
        print_info "Installing to $install_dir requires sudo..."
        sudo cp "$cli_path" "$install_dir/wia-auto-005"
        print_success "Installed CLI to $install_dir/wia-auto-005 (with sudo)"
    fi

    # Verify installation
    if command -v wia-auto-005 &> /dev/null; then
        print_success "CLI tool is now available as 'wia-auto-005'"
        print_info "Try: wia-auto-005 --help"
    else
...
install_docs() {
    ...
    local docs_dir="$HOME/.wia/auto-005/docs"
...
run_tests() {
    ...
    if command -v wia-auto-005 &> /dev/null; then
        ...
        wia-auto-005 version &> /dev/null && print_success "CLI test passed" || print_error "CLI test failed"
    fi
...
show_completion() {
    ...
    echo ""
    echo -e "${GREEN}The WIA-AUTO-005 standard has been successfully installed!${RESET}"
    echo ""
    echo "Available commands:"
    echo -e "${CYAN}  wia-auto-005 calc-time${RESET}        - Calculate charging time"
    echo -e "${CYAN}  wia-auto-005 calc-cost${RESET}        - Calculate charging cost"
    echo -e "${CYAN}  wia-auto-005 validate${RESET}         - Validate charging session"
    echo -e "${CYAN}  wia-auto-005 connector-info${RESET}   - Show connector information"
    echo -e "${CYAN}  wia-auto-005 compare-levels${RESET}   - Compare charging levels"
    echo -e "${CYAN}  wia-auto-005 help${RESET}             - Show help message"
    echo ""
    echo "Quick start:"
    echo -e "${GRAY}  # Calculate charging time for 75 kWh battery${RESET}"
    echo -e "${CYAN}  wia-auto-005 calc-time --battery 75 --power 150 --from 20 --to 80${RESET}"
    echo ""
    echo -e "${GRAY}  # Calculate charging cost${RESET}"
    echo -e "${CYAN}  wia-auto-005 calc-cost --energy 45 --price 0.35 --time 30${RESET}"
    echo ""
    echo -e "${GRAY}  # Validate CCS charging session${RESET}"
    echo -e "${CYAN}  wia-auto-005 validate --connector CCS --power 150 --vehicle BEV${RESET}"
    echo ""
    echo "TypeScript SDK:"
    echo -e "${GRAY}  import { calculateChargingTime } from '@wia/auto-005';${RESET}"
    echo ""
    echo "Documentation:"
    echo -e "${GRAY}  $HOME/.wia/auto-005/docs/${RESET}"
    echo ""
