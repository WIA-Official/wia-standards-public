...
#######################################################
# WIA-EXOSKELETON Installation Script
# Version: 1.0.0
...
print_message "$BLUE" "======================================"
print_message "$BLUE" "  WIA-EXOSKELETON Installation Script"
...
# Make CLI executable
print_message "$BLUE" "\nConfiguring CLI..."
if [ -f "cli/wia-exoskeleton.sh" ]; then
    chmod +x cli/wia-exoskeleton.sh
    print_message "$GREEN" "CLI made executable"
else
    print_message "$YELLOW" "Warning: CLI script not found at cli/wia-exoskeleton.sh"
fi
...
# Test CLI
print_message "$BLUE" "\nTesting CLI..."
if ./cli/wia-exoskeleton.sh --version &> /dev/null; then
...
print_message "$NC" "  2. Run CLI: ./cli/wia-exoskeleton.sh --help"
print_message "$NC" "  3. Use SDK:"
echo ""
print_message "$NC" "     import { WIADeFiSDK } from '@wia/exoskeleton-sdk';"
print_message "$NC" "     const sdk = new WIADeFiSDK({ apiKey: '...', network: 'mainnet' });"
echo ""
print_message "$NC" "Documentation:"
print_message "$NC" "  - Spec: ./spec/WIA-EXOSKELETON-PHASE1.md"
