...
################################################################################
# WIA-QUA-015: Wormhole Navigation - Installation Script
#
# @version 1.0.0
# @license MIT
...
#
# This script installs the WIA-QUA-015 standard components:
# - CLI tool
# - TypeScript SDK
# - Documentation
################################################################################
...
print_header() {
    echo -e "${INDIGO}"
    echo "╔════════════════════════════════════════════════════════════════╗"
    echo "║      🕳️  WIA-QUA-015: Wormhole Navigation Installer          ║"
    echo "║                      Version 1.0.0                             ║"
    echo "╚════════════════════════════════════════════════════════════════╝"
    echo -e "${RESET}"
}
...
install_cli() {
    ...
    local cli_path="$SCRIPT_DIR/cli/wia-qua-015.sh"
    ...
    if [ -w "$install_dir" ]; then
        cp "$cli_path" "$install_dir/wia-qua-015"
        print_success "Installed CLI to $install_dir/wia-qua-015"
    else
        print_info "Installing to $install_dir requires sudo..."
        sudo cp "$cli_path" "$install_dir/wia-qua-015"
        print_success "Installed CLI to $install_dir/wia-qua-015 (with sudo)"
    fi

    # Verify installation
    if command -v wia-qua-015 &> /dev/null; then
        print_success "CLI tool is now available as 'wia-qua-015'"
        print_info "Try: wia-qua-015 --help"
    else
...
install_docs() {
    ...
    local docs_dir="$HOME/.wia/qua-015/docs"
...
run_tests() {
    ...
    if command -v wia-qua-015 &> /dev/null; then
        ...
        wia-qua-015 version &> /dev/null && print_success "CLI test passed" || print_error "CLI test failed"
    fi
...
show_completion() {
    ...
    echo ""
    echo -e "${GREEN}The WIA-QUA-015 standard has been successfully installed!${RESET}"
    echo ""
    echo "Available commands:"
    echo -e "${CYAN}  wia-qua-015 analyze${RESET}           - Analyze wormhole properties"
    echo -e "${CYAN}  wia-qua-015 exotic-matter${RESET}     - Calculate exotic matter requirements"
    echo -e "${CYAN}  wia-qua-015 stability${RESET}         - Check wormhole stability"
    echo -e "${CYAN}  wia-qua-015 navigate${RESET}          - Plan navigation trajectory"
    echo -e "${CYAN}  wia-qua-015 tidal${RESET}             - Calculate tidal forces"
    echo -e "${CYAN}  wia-qua-015 safety-check${RESET}      - Validate safety parameters"
    echo -e "${CYAN}  wia-qua-015 coords${RESET}            - Transform coordinates"
    echo -e "${CYAN}  wia-qua-015 help${RESET}              - Show help message"
    echo ""
    echo "Quick start:"
    echo -e "${GRAY}  # Analyze Morris-Thorne wormhole${RESET}"
    echo -e "${CYAN}  wia-qua-015 analyze --type morris-thorne --throat-radius 1000${RESET}"
    echo ""
    echo -e "${GRAY}  # Calculate exotic matter requirements${RESET}"
    echo -e "${CYAN}  wia-qua-015 exotic-matter --throat-radius 1000${RESET}"
    echo ""
    echo -e "${GRAY}  # Plan navigation through wormhole${RESET}"
    echo -e "${CYAN}  wia-qua-015 navigate --entry \"10000,0,0\" --velocity 0.1c${RESET}"
    echo ""
    echo "TypeScript SDK:"
    echo -e "${GRAY}  import { checkStability, planTrajectory } from '@wia/qua-015';${RESET}"
    echo ""
    echo "Documentation:"
    echo -e "${GRAY}  $HOME/.wia/qua-015/docs/${RESET}"
    echo ""
