...
################################################################################
# WIA-QUA-009: Photonics - Installation Script
#
# @version 1.0.0
# @license MIT
...
#
# This script installs the WIA-QUA-009 standard components:
# - CLI tool
# - TypeScript SDK
# - Documentation
################################################################################
...
print_header() {
    echo -e "${INDIGO}"
    echo "╔════════════════════════════════════════════════════════════════╗"
    echo "║           💡 WIA-QUA-009: Photonics Installer                 ║"
    echo "║                      Version 1.0.0                             ║"
    echo "╚════════════════════════════════════════════════════════════════╝"
    echo -e "${RESET}"
}
...
install_cli() {
    ...
    local cli_path="$SCRIPT_DIR/cli/wia-qua-009.sh"
    ...
    if [ -w "$install_dir" ]; then
        cp "$cli_path" "$install_dir/wia-qua-009"
        print_success "Installed CLI to $install_dir/wia-qua-009"
    else
        print_info "Installing to $install_dir requires sudo..."
        sudo cp "$cli_path" "$install_dir/wia-qua-009"
        print_success "Installed CLI to $install_dir/wia-qua-009 (with sudo)"
    fi

    # Verify installation
    if command -v wia-qua-009 &> /dev/null; then
        print_success "CLI tool is now available as 'wia-qua-009'"
        print_info "Try: wia-qua-009 --help"
    else
...
install_docs() {
    ...
    local docs_dir="$HOME/.wia/qua-009/docs"
...
run_tests() {
    ...
    if command -v wia-qua-009 &> /dev/null; then
        ...
        wia-qua-009 version &> /dev/null && print_success "CLI test passed" || print_error "CLI test failed"
    fi
...
show_completion() {
    ...
    echo ""
    echo -e "${GREEN}The WIA-QUA-009 Photonics standard has been successfully installed!${RESET}"
    echo ""
    echo "Available commands:"
    echo -e "${CYAN}  wia-qua-009 photon-energy${RESET}     - Calculate photon energy"
    echo -e "${CYAN}  wia-qua-009 design-fiber${RESET}      - Design optical fiber"
    echo -e "${CYAN}  wia-qua-009 analyze-laser${RESET}     - Analyze laser system"
    echo -e "${CYAN}  wia-qua-009 simulate-lidar${RESET}    - Simulate LiDAR system"
    echo -e "${CYAN}  wia-qua-009 led-efficiency${RESET}    - Calculate LED efficacy"
    echo -e "${CYAN}  wia-qua-009 help${RESET}              - Show help message"
    echo ""
    echo "Quick start:"
    echo -e "${GRAY}  # Calculate photon energy for telecom wavelength${RESET}"
    echo -e "${CYAN}  wia-qua-009 photon-energy --wavelength 1550nm${RESET}"
    echo ""
    echo -e "${GRAY}  # Design single-mode fiber${RESET}"
    echo -e "${CYAN}  wia-qua-009 design-fiber --core-dia 9um --wavelength 1550nm${RESET}"
    echo ""
    echo -e "${GRAY}  # Analyze diode laser${RESET}"
    echo -e "${CYAN}  wia-qua-009 analyze-laser --type diode --wavelength 850nm --power 10mW${RESET}"
    echo ""
    echo "TypeScript SDK:"
    echo -e "${GRAY}  import { calculatePhotonEnergy } from '@wia/qua-009';${RESET}"
    echo ""
    echo "Documentation:"
    echo -e "${GRAY}  $HOME/.wia/qua-009/docs/${RESET}"
    echo ""
