...
################################################################################
# WIA-BIO-011: Biosensor - Installation Script
#
# @version 1.0.0
# @license MIT
...
#
# This script installs the WIA-BIO-011 standard components:
# - CLI tool
# - TypeScript SDK
# - Documentation
################################################################################
...
print_header() {
    echo -e "${TEAL}"
    echo "╔════════════════════════════════════════════════════════════════╗"
    echo "║           📡 WIA-BIO-011: Biosensor Installer                 ║"
    echo "║                      Version 1.0.0                             ║"
    echo "╚════════════════════════════════════════════════════════════════╝"
    echo -e "${RESET}"
}
...
install_cli() {
    ...
    local cli_path="$SCRIPT_DIR/cli/wia-bio-011.sh"
    ...
    if [ -w "$install_dir" ]; then
        cp "$cli_path" "$install_dir/wia-bio-011"
        print_success "Installed CLI to $install_dir/wia-bio-011"
    else
        print_info "Installing to $install_dir requires sudo..."
        sudo cp "$cli_path" "$install_dir/wia-bio-011"
        print_success "Installed CLI to $install_dir/wia-bio-011 (with sudo)"
    fi

    # Verify installation
    if command -v wia-bio-011 &> /dev/null; then
        print_success "CLI tool is now available as 'wia-bio-011'"
        print_info "Try: wia-bio-011 --help"
    else
...
install_docs() {
    ...
    local docs_dir="$HOME/.wia/bio-011/docs"
...
run_tests() {
    ...
    if command -v wia-bio-011 &> /dev/null; then
        ...
        wia-bio-011 version &> /dev/null && print_success "CLI test passed" || print_error "CLI test failed"
    fi
...
show_completion() {
    ...
    echo ""
    echo -e "${GREEN}The WIA-BIO-011 standard has been successfully installed!${RESET}"
    echo ""
    echo "Available commands:"
    echo -e "${CYAN}  wia-bio-011 create-sensor${RESET}     - Create biosensor configuration"
    echo -e "${CYAN}  wia-bio-011 calibrate${RESET}         - Calibrate sensor"
    echo -e "${CYAN}  wia-bio-011 measure${RESET}           - Measure analyte"
    echo -e "${CYAN}  wia-bio-011 calc-lod${RESET}          - Calculate LOD"
    echo -e "${CYAN}  wia-bio-011 validate${RESET}          - Validate sensor performance"
    echo -e "${CYAN}  wia-bio-011 qc-report${RESET}         - Generate QC report"
    echo -e "${CYAN}  wia-bio-011 help${RESET}              - Show help message"
    echo ""
    echo "Quick start:"
    echo -e "${GRAY}  # Create glucose biosensor${RESET}"
    echo -e "${CYAN}  wia-bio-011 create-sensor --type electrochemical --bioelement glucose-oxidase${RESET}"
    echo ""
    echo -e "${GRAY}  # Calibrate with 6-point curve${RESET}"
    echo -e "${CYAN}  wia-bio-011 calibrate --sensor-id BS-001 \\${RESET}"
    echo -e "${CYAN}    --standards '0,1,5,10,50,100' \\${RESET}"
    echo -e "${CYAN}    --signals '0.05,1.2,5.8,11.5,58.2,115.8'${RESET}"
    echo ""
    echo -e "${GRAY}  # Measure glucose concentration${RESET}"
    echo -e "${CYAN}  wia-bio-011 measure --sensor-id BS-001 --signal 23.5 --temp 37${RESET}"
    echo ""
    echo -e "${GRAY}  # Calculate limit of detection${RESET}"
    echo -e "${CYAN}  wia-bio-011 calc-lod --blank-stddev 0.015 --sensitivity 1.157${RESET}"
    echo ""
    echo "TypeScript SDK:"
    echo -e "${GRAY}  import { BiosensorSDK } from '@wia/bio-011';${RESET}"
    echo ""
    echo "Documentation:"
    echo -e "${GRAY}  $HOME/.wia/bio-011/docs/${RESET}"
    echo ""
