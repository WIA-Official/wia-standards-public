...
################################################################################
# WIA-QUA-010: Holography - Installation Script
#
# @version 1.0.0
# @license MIT
...
#
# This script installs the WIA-QUA-010 standard components:
# - CLI tool
# - TypeScript SDK
# - Documentation
################################################################################
...
print_header() {
    echo -e "${INDIGO}"
    echo "╔════════════════════════════════════════════════════════════════╗"
    echo "║        🌈 WIA-QUA-010: Holography Installer                   ║"
    echo "║                      Version 1.0.0                             ║"
    echo "╚════════════════════════════════════════════════════════════════╝"
    echo -e "${RESET}"
}
...
install_cli() {
    ...
    local cli_path="$SCRIPT_DIR/cli/wia-qua-010.sh"
    ...
    if [ -w "$install_dir" ]; then
        cp "$cli_path" "$install_dir/wia-qua-010"
        print_success "Installed CLI to $install_dir/wia-qua-010"
    else
        print_info "Installing to $install_dir requires sudo..."
        sudo cp "$cli_path" "$install_dir/wia-qua-010"
        print_success "Installed CLI to $install_dir/wia-qua-010 (with sudo)"
    fi

    # Verify installation
    if command -v wia-qua-010 &> /dev/null; then
        print_success "CLI tool is now available as 'wia-qua-010'"
        print_info "Try: wia-qua-010 --help"
    else
...
install_docs() {
    ...
    local docs_dir="$HOME/.wia/qua-010/docs"
...
run_tests() {
    ...
    if command -v wia-qua-010 &> /dev/null; then
        ...
        wia-qua-010 version &> /dev/null && print_success "CLI test passed" || print_error "CLI test failed"
    fi
...
show_completion() {
    ...
    echo ""
    echo -e "${GREEN}The WIA-QUA-010 Holography standard has been successfully installed!${RESET}"
    echo ""
    echo "Available commands:"
    echo -e "${CYAN}  wia-qua-010 calc-interference${RESET}  - Calculate interference patterns"
    echo -e "${CYAN}  wia-qua-010 calc-efficiency${RESET}    - Calculate diffraction efficiency"
    echo -e "${CYAN}  wia-qua-010 analyze${RESET}            - Analyze hologram type"
    echo -e "${CYAN}  wia-qua-010 calc-storage${RESET}       - Calculate storage capacity"
    echo -e "${CYAN}  wia-qua-010 generate-cgh${RESET}       - Analyze CGH requirements"
    echo -e "${CYAN}  wia-qua-010 simulate-display${RESET}   - Simulate holographic display"
    echo -e "${CYAN}  wia-qua-010 help${RESET}               - Show help message"
    echo ""
    echo "Quick start:"
    echo -e "${GRAY}  # Calculate interference pattern for green laser${RESET}"
    echo -e "${CYAN}  wia-qua-010 calc-interference --wavelength 532e-9 --angle 30${RESET}"
    echo ""
    echo -e "${GRAY}  # Analyze hologram type${RESET}"
    echo -e "${CYAN}  wia-qua-010 analyze --thickness 15e-6 --angle 45${RESET}"
    echo ""
    echo -e "${GRAY}  # Calculate storage capacity${RESET}"
    echo -e "${CYAN}  wia-qua-010 calc-storage --volume 1e-6 --dynamic-range 5${RESET}"
    echo ""
    echo "TypeScript SDK:"
    echo -e "${GRAY}  import { recordHologram, generateCGH } from '@wia/qua-010';${RESET}"
    echo ""
    echo "Documentation:"
    echo -e "${GRAY}  $HOME/.wia/qua-010/docs/${RESET}"
    echo ""
