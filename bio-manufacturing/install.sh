...
################################################################################
# WIA-BIO-015: Bio-Manufacturing - Installation Script
#
# @version 1.0.0
# @license MIT
...
#
# This script installs the WIA-BIO-015 standard components:
# - CLI tool
# - TypeScript SDK
# - Documentation
################################################################################
...
print_header() {
    echo -e "${TEAL}"
    echo "╔════════════════════════════════════════════════════════════════╗"
    echo "║       🏭 WIA-BIO-015: Bio-Manufacturing Installer             ║"
    echo "║                      Version 1.0.0                             ║"
    echo "╚════════════════════════════════════════════════════════════════╝"
    echo -e "${RESET}"
}
...
install_cli() {
    ...
    local cli_path="$SCRIPT_DIR/cli/wia-bio-015.sh"
    ...
    if [ -w "$install_dir" ]; then
        cp "$cli_path" "$install_dir/wia-bio-015"
        print_success "Installed CLI to $install_dir/wia-bio-015"
    else
        print_info "Installing to $install_dir requires sudo..."
        sudo cp "$cli_path" "$install_dir/wia-bio-015"
        print_success "Installed CLI to $install_dir/wia-bio-015 (with sudo)"
    fi

    # Verify installation
    if command -v wia-bio-015 &> /dev/null; then
        print_success "CLI tool is now available as 'wia-bio-015'"
        print_info "Try: wia-bio-015 --help"
    else
...
install_docs() {
    ...
    local docs_dir="$HOME/.wia/bio-015/docs"
...
run_tests() {
    ...
    if command -v wia-bio-015 &> /dev/null; then
        ...
        wia-bio-015 version &> /dev/null && print_success "CLI test passed" || print_error "CLI test failed"
    fi
...
show_completion() {
    ...
    echo ""
    echo -e "${GREEN}The WIA-BIO-015 standard has been successfully installed!${RESET}"
    echo ""
    echo "Available commands:"
    echo -e "${CYAN}  wia-bio-015 calc-yield${RESET}        - Calculate product yield"
    echo -e "${CYAN}  wia-bio-015 calc-productivity${RESET} - Calculate volumetric productivity"
    echo -e "${CYAN}  wia-bio-015 monitor${RESET}           - Monitor bioreactor status"
    echo -e "${CYAN}  wia-bio-015 design-purification${RESET} - Design purification protocol"
    echo -e "${CYAN}  wia-bio-015 optimize${RESET}          - Optimize culture conditions"
    echo -e "${CYAN}  wia-bio-015 help${RESET}              - Show help message"
    echo ""
    echo "Quick start:"
    echo -e "${GRAY}  # Calculate product yield${RESET}"
    echo -e "${CYAN}  wia-bio-015 calc-yield --product 5.2 --substrate 50 --volume 1000${RESET}"
    echo ""
    echo -e "${GRAY}  # Monitor bioreactor${RESET}"
    echo -e "${CYAN}  wia-bio-015 monitor --reactor BR-001 --cells 2.5e6 --viability 95${RESET}"
    echo ""
    echo -e "${GRAY}  # Design purification protocol${RESET}"
    echo -e "${CYAN}  wia-bio-015 design-purification --product antibody --scale 1000${RESET}"
    echo ""
    echo "TypeScript SDK:"
    echo -e "${GRAY}  import { calculateYield } from '@wia/bio-015';${RESET}"
    echo ""
    echo "Documentation:"
    echo -e "${GRAY}  $HOME/.wia/bio-015/docs/${RESET}"
    echo ""
