...
print_header() {
    echo -e "${BLUE}╔════════════════════════════════════════════════════════════════╗${RESET}"
    echo -e "${BLUE}║         WIA-COMP-015 Installer                              ║${RESET}"
    echo -e "${BLUE}╚════════════════════════════════════════════════════════════════╝${RESET}"
}

install_cli() {
    echo -e "${GREEN}Installing CLI...${RESET}"
    chmod +x "$SCRIPT_DIR/cli/wia-comp-015.sh"
    if [ -w "/usr/local/bin" ]; then
        cp "$SCRIPT_DIR/cli/wia-comp-015.sh" "/usr/local/bin/wia-comp-015"
    else
        sudo cp "$SCRIPT_DIR/cli/wia-comp-015.sh" "/usr/local/bin/wia-comp-015"
    fi
