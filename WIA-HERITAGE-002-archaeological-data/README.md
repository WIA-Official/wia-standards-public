# WIA-HERITAGE-002: Archaeological Data Standards Standard
...
The WIA-HERITAGE-002 standard defines a comprehensive framework for digitizing, preserving, and sharing cultural artifacts through advanced 3D scanning, photogrammetry, AI-powered restoration, and blockchain-based provenance tracking.
...
```bash
curl -fsSL https://raw.githubusercontent.com/WIA-Official/wia-standards/main/standards/WIA-HERITAGE-002-archaeological-data/install.sh | bash
```
...
```bash
wia-heritage-002 scan --input photos/ --output artifact.gltf --metadata metadata.json
```

### Create Virtual Exhibition

```bash
wia-heritage-002 exhibit --artifacts artifacts/ --output exhibition/ --theme ancient-civilizations
```

### Verify Provenance

```bash
wia-heritage-002 provenance --artifact artifact.json --blockchain ethereum --verify
```
...
```typescript
import { WIAHeritage002 } from '@wia/heritage-002';

const heritage = new WIAHeritage002({
  apiKey: 'your-api-key',
  blockchain: 'ethereum'
});
