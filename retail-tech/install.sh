...
################################################################################
# WIA-IND-020: Retail Tech - Installation Script
#
# @version 1.0.0
# @license MIT
...
#
# This script installs the WIA-IND-020 standard components:
# - CLI tool
# - TypeScript SDK
# - Documentation
################################################################################
...
print_header() {
    echo -e "${AMBER}"
    echo "╔════════════════════════════════════════════════════════════════╗"
    echo "║           🛒 WIA-IND-020: Retail Tech Installer               ║"
    echo "║                      Version 1.0.0                             ║"
    echo "╚════════════════════════════════════════════════════════════════╝"
    echo -e "${RESET}"
}
...
check_prerequisites() {
    ...
    if command -v jq &> /dev/null; then
        ...
        print_warning "jq not found (required for CLI JSON processing)"
...
install_cli() {
    ...
    local cli_path="$SCRIPT_DIR/cli/wia-ind-020.sh"
    ...
    if [ -w "$install_dir" ]; then
        cp "$cli_path" "$install_dir/wia-ind-020"
        print_success "Installed CLI to $install_dir/wia-ind-020"
    else
        print_info "Installing to $install_dir requires sudo..."
        sudo cp "$cli_path" "$install_dir/wia-ind-020"
        print_success "Installed CLI to $install_dir/wia-ind-020 (with sudo)"
    fi

    # Verify installation
    if command -v wia-ind-020 &> /dev/null; then
        print_success "CLI tool is now available as 'wia-ind-020'"
        print_info "Try: wia-ind-020 --help"
    else
...
install_docs() {
    ...
    local docs_dir="$HOME/.wia/ind-020/docs"
...
run_tests() {
    ...
    if command -v wia-ind-020 &> /dev/null; then
        ...
        wia-ind-020 version &> /dev/null && print_success "CLI test passed" || print_error "CLI test failed"
    fi
...
show_completion() {
    ...
    echo ""
    echo -e "${GREEN}The WIA-IND-020 Retail Tech standard has been successfully installed!${RESET}"
    echo ""
    echo "Available commands:"
    echo -e "${CYAN}  wia-ind-020 transaction create${RESET}   - Create new transaction"
    echo -e "${CYAN}  wia-ind-020 payment process${RESET}      - Process payment"
    echo -e "${CYAN}  wia-ind-020 inventory check${RESET}      - Check inventory levels"
    echo -e "${CYAN}  wia-ind-020 loyalty add-points${RESET}   - Add loyalty points"
    echo -e "${CYAN}  wia-ind-020 discount apply${RESET}       - Apply discount code"
    echo -e "${CYAN}  wia-ind-020 report sales${RESET}         - Generate sales report"
    echo -e "${CYAN}  wia-ind-020 help${RESET}                 - Show help message"
    echo ""
    echo "Quick start:"
    echo -e "${GRAY}  # Create a new transaction${RESET}"
    echo -e "${CYAN}  wia-ind-020 transaction create --store STORE-001 --register POS-03${RESET}"
    echo ""
    echo -e "${GRAY}  # Check inventory${RESET}"
    echo -e "${CYAN}  wia-ind-020 inventory check --sku PROD-12345${RESET}"
    echo ""
    echo -e "${GRAY}  # Generate sales report${RESET}"
    echo -e "${CYAN}  wia-ind-020 report sales --start-date 2025-01-01 --end-date 2025-01-31${RESET}"
    echo ""
    echo "TypeScript SDK:"
    echo -e "${GRAY}  import { createTransaction } from '@wia/ind-020';${RESET}"
    echo ""
    echo "Documentation:"
    echo -e "${GRAY}  $HOME/.wia/ind-020/docs/${RESET}"
    echo ""
