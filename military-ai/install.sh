...
################################################################################
# WIA-DEF-018: Military AI - Installation Script
#
# @version 1.0.0
# @license MIT
...
#
# This script installs the WIA-DEF-018 standard components:
# - CLI tool
# - TypeScript SDK
# - Documentation
################################################################################
...
print_header() {
    echo -e "${SLATE}"
    echo "╔════════════════════════════════════════════════════════════════╗"
    echo "║        🤖 WIA-DEF-018: Military AI Installer                  ║"
    echo "║                      Version 1.0.0                             ║"
    echo "╚════════════════════════════════════════════════════════════════╝"
    echo -e "${RESET}"
}
...
install_cli() {
    ...
    local cli_path="$SCRIPT_DIR/cli/wia-def-018.sh"
    ...
    if [ -w "$install_dir" ]; then
        cp "$cli_path" "$install_dir/wia-def-018"
        print_success "Installed CLI to $install_dir/wia-def-018"
    else
        print_info "Installing to $install_dir requires sudo..."
        sudo cp "$cli_path" "$install_dir/wia-def-018"
        print_success "Installed CLI to $install_dir/wia-def-018 (with sudo)"
    fi

    # Verify installation
    if command -v wia-def-018 &> /dev/null; then
        print_success "CLI tool is now available as 'wia-def-018'"
        print_info "Try: wia-def-018 --help"
    else
...
install_docs() {
    ...
    local docs_dir="$HOME/.wia/def-018/docs"
...
run_tests() {
    ...
    if command -v wia-def-018 &> /dev/null; then
        ...
        wia-def-018 version &> /dev/null && print_success "CLI test passed" || print_error "CLI test failed"
    fi
...
show_completion() {
    ...
    echo ""
    echo -e "${GREEN}The WIA-DEF-018 Military AI standard has been successfully installed!${RESET}"
    echo ""
    echo "Available commands:"
    echo -e "${CYAN}  wia-def-018 validate-model${RESET}    - Validate AI model for deployment"
    echo -e "${CYAN}  wia-def-018 assess-ethics${RESET}     - Assess ethical compliance"
    echo -e "${CYAN}  wia-def-018 check-autonomy${RESET}    - Check autonomous system config"
    echo -e "${CYAN}  wia-def-018 audit-report${RESET}      - Generate compliance audit"
    echo -e "${CYAN}  wia-def-018 check-health${RESET}      - Check equipment health"
    echo -e "${CYAN}  wia-def-018 help${RESET}              - Show help message"
    echo ""
    echo "Quick start:"
    echo -e "${GRAY}  # Validate an AI model${RESET}"
    echo -e "${CYAN}  wia-def-018 validate-model --model ./model.pkl --type classifier${RESET}"
    echo ""
    echo -e "${GRAY}  # Assess mission ethics${RESET}"
    echo -e "${CYAN}  wia-def-018 assess-ethics --mission ./mission.json${RESET}"
    echo ""
    echo -e "${GRAY}  # Check autonomous system configuration${RESET}"
    echo -e "${CYAN}  wia-def-018 check-autonomy --config ./uav-config.yaml${RESET}"
    echo ""
    echo "TypeScript SDK:"
    echo -e "${GRAY}  import { MilitaryAISDK } from '@wia/def-018';${RESET}"
    echo ""
    echo "Documentation:"
    echo -e "${GRAY}  $HOME/.wia/def-018/docs/${RESET}"
    echo ""
    echo "Core Principles:"
    echo -e "${GRAY}  - Human control maintained at all levels${RESET}"
    echo -e "${GRAY}  - Explainable AI decisions${RESET}"
    echo -e "${GRAY}  - Ethical compliance with IHL/LOAC${RESET}"
    echo -e "${GRAY}  - Safety-first design${RESET}"
    echo ""
    echo -e "${SLATE}弘익인간 (Benefit All Humanity)${RESET}"
