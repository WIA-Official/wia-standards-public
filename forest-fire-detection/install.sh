...
################################################################################
# WIA-ENE-032: Forest Fire Detection Standard - Installation Script
#
# 弘益人間 (홍익人間) - Benefit All Humanity
#
# This script installs dependencies and sets up the WIA-ENE-032
# Forest Fire Detection Standard SDK and CLI tools.
#
# Usage:
...
while [[ $# -gt 0 ]]; do
  case $1 in
    ...
    --cli-only)
      ...
      ;;
    --help)
      echo "WIA-ENE-032 Installation Script"
      echo ""
      echo "Usage:"
...
print_header() {
  ...
  echo "╔════════════════════════════════════════════════════════════════╗"
  echo "║                                                                ║"
  echo "║    🔥  WIA-ENE-032: Forest Fire Detection Standard            ║"
  echo "║                                                                ║"
...
if $INSTALL_CLI; then
  ...
  if [ -f "${CLI_DIR}/forest-fire-detection.sh" ]; then
    chmod +x "${CLI_DIR}/forest-fire-detection.sh"
    print_step "CLI script made executable"
  else
    print_error "CLI script not found: ${CLI_DIR}/forest-fire-detection.sh"
    exit 1
  fi

  # Create symlink in /usr/local/bin (optional)
  if [ -w "/usr/local/bin" ]; then
    ln -sf "${CLI_DIR}/forest-fire-detection.sh" /usr/local/bin/wia-fire 2>/dev/null || true
    if [ -L "/usr/local/bin/wia-fire" ]; then
      print_step "Symlink created: /usr/local/bin/wia-fire"
      print_info "You can now run 'wia-fire' from anywhere"
    fi
  else
    print_warning "Cannot create symlink in /usr/local/bin (no write permission)"
    print_info "You can run the CLI using: ${CLI_DIR}/forest-fire-detection.sh"
  fi
...
if $INSTALL_SDK; then
  ...
  echo "  Usage:"
  echo "    import { ForestFireDetectionSDK } from '@wia/ene-032';"
  echo ""
fi

if $INSTALL_CLI; then
  echo -e "${GREEN}✓${NC} CLI tools installed"
  echo "  Location: ${CLI_DIR}/forest-fire-detection.sh"
  echo "  Usage:"
  echo "    ./cli/forest-fire-detection.sh --help"
  if [ -L "/usr/local/bin/wia-fire" ]; then
    echo "  Or: wia-fire --help"
  fi
  echo ""
fi
...
echo "   - spec/WIA-ENE-032-v1.0.md: Detailed specification"
echo ""
echo "2. Try the SDK:"
echo "   cd ${SDK_DIR}"
echo "   npm test"
echo ""
echo "3. Try the CLI:"
echo "   ${CLI_DIR}/forest-fire-detection.sh --help"
echo ""
echo "4. Set up API credentials:"
echo "   export WIA_API_KEY='your-api-key'"
echo "   # Or create ~/.wia/forest-fire-detection.conf"
echo ""
echo "5. Join the community:"
echo "   - GitHub: https://github.com/WIA-Official/wia-standards"
echo "   - Website: https://wia.org/standards/ene-032"
