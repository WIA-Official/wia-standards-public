//! HTTP client and API implementations for the WIA-DEFI SDK
//!
...
impl DefiClient {
    /// Create a new WIA-DEFI client
    pub fn new(config: Config) -> Self {
        let base_url = config
            .base_url
            .clone()
            .unwrap_or_else(|| "https://api.wiastandards.com/WIA-DEFI".to_string());
    ...
    pub async fn get(&self, id: &str) -> Result<DefiData> {
        ...
        let result: DefiResponse = response.json().await?;
        result.data.ok_or_else(|| Error::NotFound(id.to_string()))
    }

    /// Create new data
    pub async fn create(&self, request: CreateDefiRequest) -> Result<DefiData> {
    ...
    pub async fn list(&self, params: Option<ListParams>) -> Result<DefiList> {
        let url = format!("{}/data", self.base_url);
