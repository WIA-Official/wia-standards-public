# Financial Fraud Detection — Phase 1: Data Format Specification
...
Phase 1 defines standardized data formats for Financial Fraud Detection systems. This specification establishes the core data model, field definitions, validation rules, and serialization formats to ensure interoperability across banking systems and financial platforms.
...
```json
{
  ...
  "title": "WIA Financial Fraud Detection Data Format",
  "type": "object",
  "required": ["standard", "version", "timestamp", "data"],
  "properties": {
    "standard": {
      "type": "string",
      "const": "WIA-FINANCIAL_FRAUD_DETECTION"
    },
    "version": {
      "type": "string",
      "pattern": "^\\d+\\.\\d+\\.\\d+$"
    ...
    "data": {
      "type": "object",
      "description": "Primary financial transaction data payload"
    }
  }
}
```
...
|-------|------|----------|-------------|
| standard | string | Yes | Standard identifier (WIA-FINANCIAL_FRAUD_DETECTION) |
...
| data | object | Yes | Domain-specific financial transaction data |
...
### 5.3 File Extensions
- `.wia-financial-fraud-dete.json` — JSON format
- `.wia-financial-fraud-dete.pb` — Protocol Buffers
