# Forest Fire Detection — Phase 1: Data Format Specification
...
Phase 1 defines standardized data formats for Forest Fire Detection systems. This specification establishes the core data model, field definitions, validation rules, and serialization formats to ensure interoperability across fire alert systems and environmental monitoring stations.
...
```json
{
  ...
  "title": "WIA Forest Fire Detection Data Format",
  "type": "object",
  "required": ["standard", "version", "timestamp", "data"],
  "properties": {
    "standard": {
      "type": "string",
      "const": "WIA-FOREST_FIRE_DETECTION"
    },
    "version": {
      "type": "string",
      "pattern": "^\\d+\\.\\d+\\.\\d+$"
    ...
    "data": {
      "type": "object",
      "description": "Primary thermal imaging data payload"
    }
  }
}
```
...
|-------|------|----------|-------------|
| standard | string | Yes | Standard identifier (WIA-FOREST_FIRE_DETECTION) |
...
| data | object | Yes | Domain-specific thermal imaging data |
...
### 5.3 File Extensions
- `.wia-forest-fire-detectio.json` — JSON format
- `.wia-forest-fire-detectio.pb` — Protocol Buffers
...
```json
{
  "standard": "WIA-FOREST_FIRE_DETECTION",
  "version": "1.0.0",
