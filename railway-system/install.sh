...
################################################################################
# WIA-AUTO-018: Railway System - Installation Script
#
# @version 1.0.0
# @license MIT
...
#
# This script installs the WIA-AUTO-018 standard components:
# - CLI tool
# - TypeScript SDK
# - Documentation
################################################################################
...
print_header() {
    echo -e "${ORANGE}"
    echo "╔════════════════════════════════════════════════════════════════╗"
    echo "║        🚄 WIA-AUTO-018: Railway System Installer              ║"
    echo "║                      Version 1.0.0                             ║"
    echo "╚════════════════════════════════════════════════════════════════╝"
    echo -e "${RESET}"
}
...
install_cli() {
    ...
    local cli_path="$SCRIPT_DIR/cli/wia-auto-018.sh"
    ...
    if [ -w "$install_dir" ]; then
        cp "$cli_path" "$install_dir/wia-auto-018"
        print_success "Installed CLI to $install_dir/wia-auto-018"
    else
        print_info "Installing to $install_dir requires sudo..."
        sudo cp "$cli_path" "$install_dir/wia-auto-018"
        print_success "Installed CLI to $install_dir/wia-auto-018 (with sudo)"
    fi

    # Verify installation
    if command -v wia-auto-018 &> /dev/null; then
        print_success "CLI tool is now available as 'wia-auto-018'"
        print_info "Try: wia-auto-018 --help"
    else
...
install_docs() {
    ...
    local docs_dir="$HOME/.wia/auto-018/docs"
...
run_tests() {
    ...
    if command -v wia-auto-018 &> /dev/null; then
        ...
        wia-auto-018 version &> /dev/null && print_success "CLI test passed" || print_error "CLI test failed"
    fi
...
show_completion() {
    ...
    echo ""
    echo -e "${GREEN}The WIA-AUTO-018 Railway System standard has been successfully installed!${RESET}"
    echo ""
    echo "Available commands:"
    echo -e "${CYAN}  wia-auto-018 calc-braking${RESET}      - Calculate braking distance"
    echo -e "${CYAN}  wia-auto-018 validate-headway${RESET}  - Validate train separation"
    echo -e "${CYAN}  wia-auto-018 generate-signal${RESET}   - Generate signaling config"
    echo -e "${CYAN}  wia-auto-018 monitor-platform${RESET}  - Monitor platform safety"
    echo -e "${CYAN}  wia-auto-018 calc-energy${RESET}       - Calculate energy consumption"
    echo -e "${CYAN}  wia-auto-018 help${RESET}              - Show help message"
    echo ""
    echo "Quick start:"
    echo -e "${GRAY}  # Calculate braking distance for 300 km/h train${RESET}"
    echo -e "${CYAN}  wia-auto-018 calc-braking --speed 300 --deceleration 0.8${RESET}"
    echo ""
    echo -e "${GRAY}  # Validate headway for 200 km/h operation${RESET}"
    echo -e "${CYAN}  wia-auto-018 validate-headway --speed 200 --braking-distance 3000${RESET}"
    echo ""
    echo -e "${GRAY}  # Generate ETCS Level 2 configuration${RESET}"
    echo -e "${CYAN}  wia-auto-018 generate-signal --type ETCS --level 2${RESET}"
    echo ""
    echo "TypeScript SDK:"
    echo -e "${GRAY}  import { calculateBrakingDistance } from '@wia/auto-018';${RESET}"
    echo ""
    echo "Documentation:"
    echo -e "${GRAY}  $HOME/.wia/auto-018/docs/${RESET}"
    echo ""
