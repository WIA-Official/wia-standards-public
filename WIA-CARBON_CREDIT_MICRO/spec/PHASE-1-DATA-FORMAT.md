# Carbon Credit Micro — Phase 1: Data Format Specification
...
Phase 1 defines standardized data formats for Carbon Credit Micro systems. This specification establishes the core data model, field definitions, validation rules, and serialization formats to ensure interoperability across carbon tracking systems and environmental monitoring.
...
```json
{
  ...
  "title": "WIA Carbon Credit Micro Data Format",
  "type": "object",
  "required": ["standard", "version", "timestamp", "data"],
  "properties": {
    "standard": {
      "type": "string",
      "const": "WIA-CARBON_CREDIT_MICRO"
    },
    "version": {
      "type": "string",
      "pattern": "^\\d+\\.\\d+\\.\\d+$"
    ...
    "data": {
      "type": "object",
      "description": "Primary carbon emissions data payload"
    }
  }
}
```
...
|-------|------|----------|-------------|
| standard | string | Yes | Standard identifier (WIA-CARBON_CREDIT_MICRO) |
...
| data | object | Yes | Domain-specific carbon emissions data |
...
### 5.3 File Extensions
- `.wia-carbon-credit-micro.json` — JSON format
- `.wia-carbon-credit-micro.pb` — Protocol Buffers
...
```json
{
  "standard": "WIA-CARBON_CREDIT_MICRO",
  "version": "1.0.0",
