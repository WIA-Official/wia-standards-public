...
################################################################################
# WIA-BIO-005: Cellular Therapy - Installation Script
#
# @version 1.0.0
# @license MIT
...
#
# This script installs the WIA-BIO-005 standard components:
# - CLI tool
# - TypeScript SDK
# - Documentation
################################################################################
...
print_header() {
    echo -e "${TEAL}"
    echo "╔════════════════════════════════════════════════════════════════╗"
    echo "║        🔬 WIA-BIO-005: Cellular Therapy Installer             ║"
    echo "║                      Version 1.0.0                             ║"
    echo "╚════════════════════════════════════════════════════════════════╝"
    echo -e "${RESET}"
}
...
install_cli() {
    ...
    local cli_path="$SCRIPT_DIR/cli/wia-bio-005.sh"
    ...
    if [ -w "$install_dir" ]; then
        cp "$cli_path" "$install_dir/wia-bio-005"
        print_success "Installed CLI to $install_dir/wia-bio-005"
    else
        print_info "Installing to $install_dir requires sudo..."
        sudo cp "$cli_path" "$install_dir/wia-bio-005"
        print_success "Installed CLI to $install_dir/wia-bio-005 (with sudo)"
    fi

    # Verify installation
    if command -v wia-bio-005 &> /dev/null; then
        print_success "CLI tool is now available as 'wia-bio-005'"
        print_info "Try: wia-bio-005 --help"
    else
...
install_docs() {
    ...
    local docs_dir="$HOME/.wia/bio-005/docs"
...
run_tests() {
    ...
    if command -v wia-bio-005 &> /dev/null; then
        ...
        wia-bio-005 version &> /dev/null && print_success "CLI test passed" || print_error "CLI test failed"
    fi
...
show_completion() {
    ...
    echo ""
    echo -e "${GREEN}The WIA-BIO-005 standard has been successfully installed!${RESET}"
    echo ""
    echo "Available commands:"
    echo -e "${CYAN}  wia-bio-005 calc-dose${RESET}         - Calculate patient dose"
    echo -e "${CYAN}  wia-bio-005 assess-quality${RESET}    - Assess quality control"
    echo -e "${CYAN}  wia-bio-005 track-batch${RESET}       - Track manufacturing batch"
    echo -e "${CYAN}  wia-bio-005 monitor-safety${RESET}    - Monitor patient safety"
    echo -e "${CYAN}  wia-bio-005 help${RESET}              - Show help message"
    echo ""
    echo "Quick start:"
    echo -e "${GRAY}  # Calculate dose for CAR-T therapy${RESET}"
    echo -e "${CYAN}  wia-bio-005 calc-dose --cells 5e8 --weight 70 --viability 92.5${RESET}"
    echo ""
    echo -e "${GRAY}  # Assess quality control${RESET}"
    echo -e "${CYAN}  wia-bio-005 assess-quality --viability 92.5 --potency 85${RESET}"
    echo ""
    echo "TypeScript SDK:"
    echo -e "${GRAY}  import { calculateDose } from '@wia/bio-005';${RESET}"
    echo ""
    echo "Documentation:"
    echo -e "${GRAY}  $HOME/.wia/bio-005/docs/${RESET}"
    echo ""
