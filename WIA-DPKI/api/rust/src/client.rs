//! HTTP client and API implementations for the WIA-DPKI SDK
//!
...
impl DpkiClient {
    /// Create a new WIA-DPKI client
    pub fn new(config: Config) -> Self {
        let base_url = config
            .base_url
            .clone()
            .unwrap_or_else(|| "https://api.wiastandards.com/WIA-DPKI".to_string());
    ...
    pub async fn get(&self, id: &str) -> Result<DpkiData> {
        ...
        let result: DpkiResponse = response.json().await?;
        result.data.ok_or_else(|| Error::NotFound(id.to_string()))
    }

    /// Create new data
    pub async fn create(&self, request: CreateDpkiRequest) -> Result<DpkiData> {
    ...
    pub async fn list(&self, params: Option<ListParams>) -> Result<DpkiList> {
        let url = format!("{}/data", self.base_url);
