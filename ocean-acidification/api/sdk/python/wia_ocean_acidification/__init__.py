"""WIA Ocean Acidification — Python SDK.
...
class OceanAcidificationClient:
    """HTTP client for WIA Ocean Acidification."""

    DEFAULT_BASE_URL = "https://api.wia.live/ocean-acidification/v1"
    ...
    def __init__(
        ...
        self.base_url = (base_url or os.environ.get("WIA_OCEAN_ACIDIFICATION_URL", self.DEFAULT_BASE_URL)).rstrip("/")
        self.api_key = api_key or os.environ.get("WIA_OCEAN_ACIDIFICATION_API_KEY")
...
def list_trends(self, limit: int = 20, offset: int = 0) -> dict:
    return self._request("GET", f"/trends", params={"limit": limit, "offset": offset})

def get_trend(self, id: str) -> dict:
    return self._request("GET", f"/trends/{id}")

def create_trend(self, body: dict) -> dict:
    return self._request("POST", f"/trends", json=body)

def replace_trend(self, id: str, body: dict) -> dict:
    return self._request("PUT", f"/trends/{id}", json=body)

def delete_trend(self, id: str) -> None:
    self._request("DELETE", f"/trends/{id}")
...
__all__ = ["OceanAcidificationClient"]
