...
################################################################################
# WIA-AUTO-002: ADAS - Advanced Driver Assistance System - Installation Script
#
# @version 1.0.0
# @license MIT
...
#
# This script installs the WIA-AUTO-002 standard components:
# - CLI tool
# - TypeScript SDK
# - Documentation
################################################################################
...
print_header() {
    echo -e "${ORANGE}"
    echo "╔════════════════════════════════════════════════════════════════╗"
    echo "║          🚗 WIA-AUTO-002: ADAS System Installer               ║"
    echo "║                      Version 1.0.0                             ║"
    echo "╚════════════════════════════════════════════════════════════════╝"
    echo -e "${RESET}"
}
...
install_cli() {
    ...
    local cli_path="$SCRIPT_DIR/cli/wia-auto-002.sh"
    ...
    if [ -w "$install_dir" ]; then
        cp "$cli_path" "$install_dir/wia-auto-002"
        print_success "Installed CLI to $install_dir/wia-auto-002"
    else
        print_info "Installing to $install_dir requires sudo..."
        sudo cp "$cli_path" "$install_dir/wia-auto-002"
        print_success "Installed CLI to $install_dir/wia-auto-002 (with sudo)"
    fi

    # Verify installation
    if command -v wia-auto-002 &> /dev/null; then
        print_success "CLI tool is now available as 'wia-auto-002'"
        print_info "Try: wia-auto-002 --help"
    else
...
install_docs() {
    ...
    local docs_dir="$HOME/.wia/auto-002/docs"
...
run_tests() {
    ...
    if command -v wia-auto-002 &> /dev/null; then
        ...
        wia-auto-002 version &> /dev/null && print_success "CLI test passed" || print_error "CLI test failed"
    fi
...
show_completion() {
    ...
    echo ""
    echo -e "${GREEN}The WIA-AUTO-002 ADAS standard has been successfully installed!${RESET}"
    echo ""
    echo "Available commands:"
    echo -e "${CYAN}  wia-auto-002 calc-distance${RESET}      - Calculate safe following distance"
    echo -e "${CYAN}  wia-auto-002 calc-ttc${RESET}           - Calculate Time-to-Collision"
    echo -e "${CYAN}  wia-auto-002 lane-check${RESET}         - Check lane departure status"
    echo -e "${CYAN}  wia-auto-002 simulate-collision${RESET} - Simulate collision scenario"
    echo -e "${CYAN}  wia-auto-002 report${RESET}             - Generate ADAS system report"
    echo -e "${CYAN}  wia-auto-002 help${RESET}               - Show help message"
    echo ""
    echo "Quick start:"
    echo -e "${GRAY}  # Calculate safe distance at 120 km/h on wet road${RESET}"
    echo -e "${CYAN}  wia-auto-002 calc-distance --speed 120 --road-condition wet${RESET}"
    echo ""
    echo -e "${GRAY}  # Calculate Time-to-Collision${RESET}"
    echo -e "${CYAN}  wia-auto-002 calc-ttc --ego-speed 100 --target-speed 60 --distance 40${RESET}"
    echo ""
    echo -e "${GRAY}  # Simulate collision scenario${RESET}"
    echo -e "${CYAN}  wia-auto-002 simulate-collision --ego-speed 90 --target-speed 50 --distance 30${RESET}"
    echo ""
    echo "TypeScript SDK:"
    echo -e "${GRAY}  import { ADASSystem, calculateSafeDistance } from '@wia/auto-002';${RESET}"
    echo ""
    echo "Documentation:"
    echo -e "${GRAY}  $HOME/.wia/auto-002/docs/${RESET}"
    echo ""
