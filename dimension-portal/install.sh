...
################################################################################
# WIA-QUA-018: Dimension Portal - Installation Script
#
# @version 1.0.0
# @license MIT
...
#
# This script installs the WIA-QUA-018 standard components:
# - CLI tool
# - TypeScript SDK
# - Documentation
################################################################################
...
print_header() {
    echo -e "${INDIGO}"
    echo "╔════════════════════════════════════════════════════════════════╗"
    echo "║      🚪 WIA-QUA-018: Dimension Portal Installer              ║"
    echo "║                      Version 1.0.0                             ║"
    echo "╚════════════════════════════════════════════════════════════════╝"
    echo -e "${RESET}"
}
...
install_cli() {
    ...
    local cli_path="$SCRIPT_DIR/cli/wia-qua-018.sh"
    ...
    if [ -w "$install_dir" ]; then
        cp "$cli_path" "$install_dir/wia-qua-018"
        print_success "Installed CLI to $install_dir/wia-qua-018"
    else
        print_info "Installing to $install_dir requires sudo..."
        sudo cp "$cli_path" "$install_dir/wia-qua-018"
        print_success "Installed CLI to $install_dir/wia-qua-018 (with sudo)"
    fi

    # Verify installation
    if command -v wia-qua-018 &> /dev/null; then
        print_success "CLI tool is now available as 'wia-qua-018'"
        print_info "Try: wia-qua-018 --help"
    else
...
install_docs() {
    ...
    local docs_dir="$HOME/.wia/qua-018/docs"
...
run_tests() {
    ...
    if command -v wia-qua-018 &> /dev/null; then
        ...
        wia-qua-018 version &> /dev/null && print_success "CLI test passed" || print_error "CLI test failed"
    fi
...
show_completion() {
    ...
    echo ""
    echo -e "${GREEN}The WIA-QUA-018 standard has been successfully installed!${RESET}"
    echo ""
    echo "Available commands:"
    echo -e "${CYAN}  wia-qua-018 portal create${RESET}     - Create dimensional portal"
    echo -e "${CYAN}  wia-qua-018 barrier calculate${RESET} - Calculate barrier energy"
    echo -e "${CYAN}  wia-qua-018 portal stabilize${RESET}  - Stabilize portal"
    echo -e "${CYAN}  wia-qua-018 navigate${RESET}          - Navigate dimensions"
    echo -e "${CYAN}  wia-qua-018 transfer${RESET}          - Transfer matter"
    echo -e "${CYAN}  wia-qua-018 map dimensions${RESET}    - Map dimensional topology"
    echo -e "${CYAN}  wia-qua-018 help${RESET}              - Show help message"
    echo ""
    echo "Quick start:"
    echo -e "${GRAY}  # Create 11-dimensional portal${RESET}"
    echo -e "${CYAN}  wia-qua-018 portal create --dimensions 11 --aperture 2.5${RESET}"
    echo ""
    echo -e "${GRAY}  # Calculate barrier energy${RESET}"
    echo -e "${CYAN}  wia-qua-018 barrier calculate --radius 1e-35 --dimensions 7${RESET}"
    echo ""
    echo -e "${GRAY}  # Stabilize portal for 1 hour${RESET}"
    echo -e "${CYAN}  wia-qua-018 portal stabilize --duration 3600 --field-strength 1e15${RESET}"
    echo ""
    echo "TypeScript SDK:"
    echo -e "${GRAY}  import { createPortal, stabilizePortal } from '@wia/qua-018';${RESET}"
    echo ""
    echo "Documentation:"
    echo -e "${GRAY}  $HOME/.wia/qua-018/docs/${RESET}"
    echo ""
