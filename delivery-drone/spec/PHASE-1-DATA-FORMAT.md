# Delivery Drone — Phase 1: Data Format Specification
...
Phase 1 defines standardized data formats for Delivery Drone systems. This specification establishes the core data model, field definitions, validation rules, and serialization formats to ensure interoperability across drone management systems and unmanned aerial platforms.
...
```json
{
  ...
  "title": "WIA Delivery Drone Data Format",
  "type": "object",
  "required": ["standard", "version", "timestamp", "data"],
  "properties": {
    "standard": {
      "type": "string",
      "const": "delivery-drone"
    },
    "version": {
      "type": "string",
      "pattern": "^\\d+\\.\\d+\\.\\d+$"
    ...
    "data": {
      "type": "object",
      "description": "Primary drone telemetry payload"
    }
  }
}
```
...
|-------|------|----------|-------------|
| standard | string | Yes | Standard identifier (delivery-drone) |
...
| data | object | Yes | Domain-specific drone telemetry |
...
### 5.3 File Extensions
- `.wia-delivery-drone.json` — JSON format
- `.wia-delivery-drone.pb` — Protocol Buffers
