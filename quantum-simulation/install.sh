...
################################################################################
# WIA-QUA-005: Quantum Simulation - Installation Script
#
# @version 1.0.0
# @license MIT
...
#
# This script installs the WIA-QUA-005 standard components:
# - CLI tool
# - TypeScript SDK
# - Documentation
################################################################################
...
print_header() {
    echo -e "${INDIGO}"
    echo "╔════════════════════════════════════════════════════════════════╗"
    echo "║      💻 WIA-QUA-005: Quantum Simulation Installer             ║"
    echo "║                      Version 1.0.0                             ║"
    echo "╚════════════════════════════════════════════════════════════════╝"
    echo -e "${RESET}"
}
...
install_cli() {
    ...
    local cli_path="$SCRIPT_DIR/cli/wia-qua-005.sh"
    ...
    if [ -w "$install_dir" ]; then
        cp "$cli_path" "$install_dir/wia-qua-005"
        print_success "Installed CLI to $install_dir/wia-qua-005"
    else
        print_info "Installing to $install_dir requires sudo..."
        sudo cp "$cli_path" "$install_dir/wia-qua-005"
        print_success "Installed CLI to $install_dir/wia-qua-005 (with sudo)"
    fi

    # Verify installation
    if command -v wia-qua-005 &> /dev/null; then
        print_success "CLI tool is now available as 'wia-qua-005'"
        print_info "Try: wia-qua-005 --help"
    else
...
install_docs() {
    ...
    local docs_dir="$HOME/.wia/qua-005/docs"
...
run_tests() {
    ...
    if command -v wia-qua-005 &> /dev/null; then
        ...
        wia-qua-005 version &> /dev/null && print_success "CLI test passed" || print_error "CLI test failed"
    fi
...
show_completion() {
    ...
    echo ""
    echo -e "${GREEN}The WIA-QUA-005 standard has been successfully installed!${RESET}"
    echo ""
    echo "Available commands:"
    echo -e "${CYAN}  wia-qua-005 simulate${RESET}         - Simulate quantum circuits"
    echo -e "${CYAN}  wia-qua-005 chemistry${RESET}        - Calculate molecular energies"
    echo -e "${CYAN}  wia-qua-005 vqe${RESET}              - Run VQE algorithm"
    echo -e "${CYAN}  wia-qua-005 noise${RESET}            - Simulate with noise"
    echo -e "${CYAN}  wia-qua-005 tn-sim${RESET}           - Tensor network simulation"
    echo -e "${CYAN}  wia-qua-005 help${RESET}             - Show help message"
    echo ""
    echo "Quick start:"
    echo -e "${GRAY}  # Simulate GHZ state on 3 qubits${RESET}"
    echo -e "${CYAN}  wia-qua-005 simulate --qubits 3${RESET}"
    echo ""
    echo -e "${GRAY}  # Calculate H2 molecule energy${RESET}"
    echo -e "${CYAN}  wia-qua-005 chemistry --molecule H2 --basis sto-3g${RESET}"
    echo ""
    echo -e "${GRAY}  # Run VQE for Hamiltonian${RESET}"
    echo -e "${CYAN}  wia-qua-005 vqe --hamiltonian \"Z_0 + 0.5*X_1\"${RESET}"
    echo ""
    echo "TypeScript SDK:"
    echo -e "${GRAY}  import { QuantumSimulator } from '@wia/qua-005';${RESET}"
    echo -e "${GRAY}  const sim = new QuantumSimulator({ numQubits: 3 });${RESET}"
    echo ""
    echo "Documentation:"
    echo -e "${GRAY}  $HOME/.wia/qua-005/docs/${RESET}"
    echo ""
