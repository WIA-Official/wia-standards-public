...
################################################################################
# WIA-DEF-016: Military Communication - Installation Script
#
# @version 1.0.0
# @license MIT
...
#
# This script installs the WIA-DEF-016 standard components:
# - CLI tool
# - TypeScript SDK
# - Documentation
################################################################################
...
print_header() {
    echo -e "${SLATE}"
    echo "╔════════════════════════════════════════════════════════════════╗"
    echo "║       📡 WIA-DEF-016: Military Communication Installer        ║"
    echo "║                      Version 1.0.0                             ║"
    echo "╚════════════════════════════════════════════════════════════════╝"
    echo -e "${RESET}"
}
...
install_cli() {
    ...
    local cli_path="$SCRIPT_DIR/cli/wia-def-016.sh"
    ...
    if [ -w "$install_dir" ]; then
        cp "$cli_path" "$install_dir/wia-def-016"
        print_success "Installed CLI to $install_dir/wia-def-016"
    else
        print_info "Installing to $install_dir requires sudo..."
        sudo cp "$cli_path" "$install_dir/wia-def-016"
        print_success "Installed CLI to $install_dir/wia-def-016 (with sudo)"
    fi

    # Verify installation
    if command -v wia-def-016 &> /dev/null; then
        print_success "CLI tool is now available as 'wia-def-016'"
        print_info "Try: wia-def-016 --help"
    else
...
install_docs() {
    ...
    local docs_dir="$HOME/.wia/def-016/docs"
...
run_tests() {
    ...
    if command -v wia-def-016 &> /dev/null; then
        ...
        wia-def-016 version &> /dev/null && print_success "CLI test passed" || print_error "CLI test failed"
    fi
...
show_completion() {
    ...
    echo ""
    echo -e "${GREEN}The WIA-DEF-016 standard has been successfully installed!${RESET}"
    echo ""
    echo "Available commands:"
    echo -e "${CYAN}  wia-def-016 calc-link${RESET}          - Calculate link budget"
    echo -e "${CYAN}  wia-def-016 encrypt${RESET}            - Encrypt tactical message"
    echo -e "${CYAN}  wia-def-016 validate-freq${RESET}      - Validate frequency allocation"
    echo -e "${CYAN}  wia-def-016 establish-link${RESET}     - Establish secure link"
    echo -e "${CYAN}  wia-def-016 calc-satcom${RESET}        - Calculate SATCOM link"
    echo -e "${CYAN}  wia-def-016 help${RESET}               - Show help message"
    echo ""
    echo "Quick start:"
    echo -e "${GRAY}  # Calculate VHF tactical radio link${RESET}"
    echo -e "${CYAN}  wia-def-016 calc-link --power 50 --freq 150 --distance 25${RESET}"
    echo ""
    echo -e "${GRAY}  # Encrypt a tactical message${RESET}"
    echo -e "${CYAN}  wia-def-016 encrypt --message 'SITREP: ALL CLEAR' --level secret${RESET}"
    echo ""
    echo -e "${GRAY}  # Validate frequency allocation${RESET}"
    echo -e "${CYAN}  wia-def-016 validate-freq --freq 148.25 --band vhf${RESET}"
    echo ""
    echo "TypeScript SDK:"
    echo -e "${GRAY}  import { calculateLinkBudget } from '@wia/def-016';${RESET}"
    echo ""
    echo "Documentation:"
    echo -e "${GRAY}  $HOME/.wia/def-016/docs/${RESET}"
    echo ""
