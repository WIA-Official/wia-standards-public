...
################################################################################
# WIA-BIO-012: Synthetic Biology - Installation Script
#
# @version 1.0.0
# @license MIT
...
#
# This script installs the WIA-BIO-012 standard components:
# - CLI tool
# - TypeScript SDK
# - Documentation
################################################################################
...
print_header() {
    echo -e "${TEAL}"
    echo "╔════════════════════════════════════════════════════════════════╗"
    echo "║      🧬 WIA-BIO-012: Synthetic Biology Installer             ║"
    echo "║                      Version 1.0.0                             ║"
    echo "╚════════════════════════════════════════════════════════════════╝"
    echo -e "${RESET}"
}
...
install_cli() {
    ...
    local cli_path="$SCRIPT_DIR/cli/wia-bio-012.sh"
    ...
    if [ -w "$install_dir" ]; then
        cp "$cli_path" "$install_dir/wia-bio-012"
        print_success "Installed CLI to $install_dir/wia-bio-012"
    else
        print_info "Installing to $install_dir requires sudo..."
        sudo cp "$cli_path" "$install_dir/wia-bio-012"
        print_success "Installed CLI to $install_dir/wia-bio-012 (with sudo)"
    fi

    # Verify installation
    if command -v wia-bio-012 &> /dev/null; then
        print_success "CLI tool is now available as 'wia-bio-012'"
        print_info "Try: wia-bio-012 --help"
    else
...
install_docs() {
    ...
    local docs_dir="$HOME/.wia/bio-012/docs"
...
run_tests() {
    ...
    if command -v wia-bio-012 &> /dev/null; then
        ...
        wia-bio-012 version &> /dev/null && print_success "CLI test passed" || print_error "CLI test failed"
    fi
...
show_completion() {
    ...
    echo ""
    echo -e "${GREEN}The WIA-BIO-012 standard has been successfully installed!${RESET}"
    echo ""
    echo "Available commands:"
    echo -e "${CYAN}  wia-bio-012 calc-promoter${RESET}     - Calculate promoter strength"
    echo -e "${CYAN}  wia-bio-012 calc-expression${RESET}   - Calculate gene expression"
    echo -e "${CYAN}  wia-bio-012 design-circuit${RESET}    - Design genetic circuit"
    echo -e "${CYAN}  wia-bio-012 optimize-pathway${RESET}  - Optimize metabolic pathway"
    echo -e "${CYAN}  wia-bio-012 simulate${RESET}          - Simulate circuit behavior"
    echo -e "${CYAN}  wia-bio-012 assess-biosafety${RESET}  - Assess biosafety level"
    echo -e "${CYAN}  wia-bio-012 help${RESET}              - Show help message"
    echo ""
    echo "Quick start:"
    echo -e "${GRAY}  # Calculate promoter strength${RESET}"
    echo -e "${CYAN}  wia-bio-012 calc-promoter --pmax 1000 --inducer 1e-6 --kd 1e-7${RESET}"
    echo ""
    echo -e "${GRAY}  # Design a genetic circuit${RESET}"
    echo -e "${CYAN}  wia-bio-012 design-circuit --parts 'BBa_J23100,BBa_B0034,BBa_E0040'${RESET}"
    echo ""
    echo "TypeScript SDK:"
    echo -e "${GRAY}  import { calculatePromoterStrength } from '@wia/bio-012';${RESET}"
    echo ""
    echo "Documentation:"
    echo -e "${GRAY}  $HOME/.wia/bio-012/docs/${RESET}"
    echo ""
