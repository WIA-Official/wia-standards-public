...
################################################################################
# WIA-TIME-008: Temporal Power Generation - Installation Script
#
# @version 1.0.0
# @license MIT
...
#
# This script installs the WIA-TIME-008 standard components:
# - CLI tool
# - TypeScript SDK
# - Documentation
################################################################################
...
print_header() {
    echo -e "${VIOLET}"
    echo "╔════════════════════════════════════════════════════════════════╗"
    echo "║     🔋 WIA-TIME-008: Temporal Power Generation Installer      ║"
    echo "║                      Version 1.0.0                             ║"
    echo "╚════════════════════════════════════════════════════════════════╝"
    echo -e "${RESET}"
}
...
install_cli() {
    ...
    local cli_path="$SCRIPT_DIR/cli/wia-time-008.sh"
    ...
    if [ -w "$install_dir" ]; then
        cp "$cli_path" "$install_dir/wia-time-008"
        print_success "Installed CLI to $install_dir/wia-time-008"
    else
        print_info "Installing to $install_dir requires sudo..."
        sudo cp "$cli_path" "$install_dir/wia-time-008"
        print_success "Installed CLI to $install_dir/wia-time-008 (with sudo)"
    fi
    
    # Verify installation
    if command -v wia-time-008 &> /dev/null; then
        print_success "CLI tool is now available as 'wia-time-008'"
        print_info "Try: wia-time-008 --help"
    else
...
install_docs() {
    ...
    local docs_dir="$HOME/.wia/time-008/docs"
...
run_tests() {
    ...
    if command -v wia-time-008 &> /dev/null; then
        ...
        wia-time-008 version &> /dev/null && print_success "CLI test passed" || print_error "CLI test failed"
    fi
...
show_completion() {
    ...
    echo ""
    echo -e "${GREEN}The WIA-TIME-008 standard has been successfully installed!${RESET}"
    echo ""
    echo "Available commands:"
    echo -e "${CYAN}  wia-time-008 calc-power${RESET}        - Calculate power output"
    echo -e "${CYAN}  wia-time-008 reactor init${RESET}      - Initialize reactor"
    echo -e "${CYAN}  wia-time-008 reactor start${RESET}     - Start reactor"
    echo -e "${CYAN}  wia-time-008 reactor status${RESET}    - Check reactor status"
    echo -e "${CYAN}  wia-time-008 crystal config${RESET}    - Configure time crystal"
    echo -e "${CYAN}  wia-time-008 harvest${RESET}           - Harvest entropy"
    echo -e "${CYAN}  wia-time-008 optimize${RESET}          - Optimize power output"
    echo -e "${CYAN}  wia-time-008 safety-check${RESET}      - Safety check"
    echo -e "${CYAN}  wia-time-008 help${RESET}              - Show help message"
    echo ""
    echo "Quick start:"
    echo -e "${GRAY}  # Calculate power output${RESET}"
    echo -e "${CYAN}  wia-time-008 calc-power --chronon-rate 1e15 --efficiency 0.85${RESET}"
    echo ""
    echo -e "${GRAY}  # Initialize and start reactor${RESET}"
    echo -e "${CYAN}  wia-time-008 reactor init --type multi-stage --power 1.21e9${RESET}"
    echo ""
    echo "TypeScript SDK:"
    echo -e "${GRAY}  import { TemporalPowerSDK } from '@wia/time-008';${RESET}"
    echo ""
    echo "Documentation:"
    echo -e "${GRAY}  $HOME/.wia/time-008/docs/${RESET}"
    echo ""
