...
##############################################################################
# WIA-CITY-014: Security System Standard - Installation Script
#
...
print_header() {
    echo -e "${MAGENTA}"
    echo "╔════════════════════════════════════════════════════════════════╗"
    echo "║  WIA-CITY-014: Security System Standard                       ║"
...
setup_cli_tool() {
    ...
    local cli_path="$SCRIPT_DIR/cli/security-system-city.sh"
    ...
    if [ -f "$cli_path" ]; then
        ...
        if command_exists sudo && [ -w "/usr/local/bin" ] || sudo -n true 2>/dev/null; then
            ...
            echo
            if [[ $REPLY =~ ^[Yy]$ ]]; then
                sudo ln -sf "$cli_path" /usr/local/bin/security-system-city
                print_success "Symlink created: /usr/local/bin/security-system-city"
            fi
        else
...
create_example_config() {
    ...
    cat > "$config_file" <<EOF
# WIA-CITY-014 Security System Configuration

# API Configuration
WIA_SECURITY_SYSTEM_ENDPOINT=https://api.wia.org/city-014/v1
WIA_SECURITY_SYSTEM_API_KEY=your-api-key-here

# WebSocket Configuration (optional)
WIA_SECURITY_SYSTEM_WS_ENDPOINT=wss://api.wia.org/city-014/ws
...
EMERGENCY_AUTO_BROADCAST=true
EOF

    print_success "Example configuration created: .env.example"
    print_info "Copy .env.example to .env and update with your settings:"
...
print_usage() {
    ...
    echo "TypeScript SDK:"
    echo "  import { SecuritySystemSDK } from '@wia/city-security-system';"
    echo
    echo "CLI Tool:"
    echo "  export WIA_SECURITY_SYSTEM_API_KEY='your-api-key'"
    echo "  security-system-city list-cameras"
    echo "  security-system-city arm-zone zone-001"
    echo
    echo "Documentation:"
    echo "  README.md                      - Overview and quick start"
    echo "  spec/WIA-CITY-014-v1.0.md      - Full specification"
    echo
    echo "API Endpoint:"
    echo "  https://api.wia.org/city-014/v1"
    echo
}
...
verify_installation() {
    ...
    # Check spec file
    if [ -f "$SCRIPT_DIR/spec/WIA-CITY-014-v1.0.md" ]; then
    ...
    # Check CLI tool
    if [ -f "$SCRIPT_DIR/cli/security-system-city.sh" ] && [ -x "$SCRIPT_DIR/cli/security-system-city.sh" ]; then
...
print_final_message() {
    ...
    echo "     ./cli/security-system-city.sh dashboard"
    echo
    echo -e "${CYAN}Quick commands:${NC}"
    echo "  ./cli/security-system-city.sh list-cameras     # 카메라 목록"
    echo "  ./cli/security-system-city.sh list-sensors     # 센서 목록"
    echo "  ./cli/security-system-city.sh list-zones       # 보안 존 목록"
    echo "  ./cli/security-system-city.sh list-alerts      # 경보 목록"
    echo "  ./cli/security-system-city.sh dashboard        # 통합 대시보드"
    echo
    echo -e "${CYAN}Resources:${NC}"
    echo "  📖 Documentation: https://wia.org/standards/city-014"
    echo "  💬 Community: https://github.com/WIA-Official/wia-standards"
    echo "  📧 Support: standards@wia-official.org"
    echo
    echo -e "${MAGENTA}弘익人間 (홍익인간) - Benefit All Humanity${NC}"
    echo
}
