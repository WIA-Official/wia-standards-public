...
##############################################################################
# WIA-CITY-009: Smart Lighting Standard - Installation Script
#
...
print_header() {
    echo -e "${MAGENTA}"
    echo "╔════════════════════════════════════════════════════════════════╗"
    echo "║  WIA-CITY-009: Smart Lighting Standard                        ║"
...
setup_cli_tool() {
    ...
    local cli_path="$SCRIPT_DIR/cli/smart-lighting.sh"
    ...
    if [ -f "$cli_path" ]; then
        ...
        if command_exists sudo && [ -w "/usr/local/bin" ] || sudo -n true 2>/dev/null; then
            ...
            echo
            if [[ $REPLY =~ ^[Yy]$ ]]; then
                sudo ln -sf "$cli_path" /usr/local/bin/smart-lighting
                print_success "Symlink created: /usr/local/bin/smart-lighting"
            fi
        else
...
create_example_config() {
    ...
    cat > "$config_file" <<EOF
# WIA-CITY-009 Smart Lighting Configuration

# API Configuration
WIA_SMART_LIGHTING_ENDPOINT=https://api.wia.org/city-009/v1
WIA_SMART_LIGHTING_API_KEY=your-api-key-here

# WebSocket Configuration (optional)
WIA_SMART_LIGHTING_WS_ENDPOINT=wss://api.wia.org/city-009/ws
...
print_usage() {
    ...
    echo "TypeScript SDK:"
    echo "  import { SmartLightingSDK } from '@wia/city-009';"
    echo
    echo "CLI Tool:"
    echo "  export WIA_SMART_LIGHTING_API_KEY='your-api-key'"
    echo "  smart-lighting list-fixtures"
    echo "  smart-lighting turn-on fixture-001"
    echo
    echo "Documentation:"
    echo "  README.md           - Overview and quick start"
    echo "  spec/WIA-CITY-009-v1.0.md - Full specification"
    echo
    echo "API Endpoint:"
    echo "  https://api.wia.org/city-009/v1"
    echo
}
...
verify_installation() {
    ...
    # Check spec file
    if [ -f "$SCRIPT_DIR/spec/WIA-CITY-009-v1.0.md" ]; then
    ...
    # Check CLI tool
    if [ -f "$SCRIPT_DIR/cli/smart-lighting.sh" ] && [ -x "$SCRIPT_DIR/cli/smart-lighting.sh" ]; then
...
print_final_message() {
    ...
    echo "     ./cli/smart-lighting.sh list-fixtures"
    echo
    echo -e "${CYAN}Resources:${NC}"
    echo "  📖 Documentation: https://wia.org/standards/city-009"
