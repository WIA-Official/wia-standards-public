...
################################################################################
# WIA-IND-022: Inventory Management - Installation Script
#
# @version 1.0.0
# @license MIT
...
#
# This script installs the WIA-IND-022 standard components:
# - CLI tool
# - TypeScript SDK
# - Documentation
################################################################################
...
print_header() {
    echo -e "${AMBER}"
    echo "╔════════════════════════════════════════════════════════════════╗"
    echo "║    📦 WIA-IND-022: Inventory Management Installer             ║"
    echo "║                      Version 1.0.0                             ║"
    echo "╚════════════════════════════════════════════════════════════════╝"
    echo -e "${RESET}"
}
...
install_cli() {
    ...
    local cli_path="$SCRIPT_DIR/cli/wia-ind-022.sh"
    ...
    if [ -w "$install_dir" ]; then
        cp "$cli_path" "$install_dir/wia-ind-022"
        print_success "Installed CLI to $install_dir/wia-ind-022"
    else
        print_info "Installing to $install_dir requires sudo..."
        sudo cp "$cli_path" "$install_dir/wia-ind-022"
        print_success "Installed CLI to $install_dir/wia-ind-022 (with sudo)"
    fi

    # Verify installation
    if command -v wia-ind-022 &> /dev/null; then
        print_success "CLI tool is now available as 'wia-ind-022'"
        print_info "Try: wia-ind-022 --help"
    else
...
install_docs() {
    ...
    local docs_dir="$HOME/.wia/ind-022/docs"
...
run_tests() {
    ...
    if command -v wia-ind-022 &> /dev/null; then
        ...
        wia-ind-022 version &> /dev/null && print_success "CLI test passed" || print_error "CLI test failed"
    fi
...
show_completion() {
    ...
    echo ""
    echo -e "${GREEN}The WIA-IND-022 Inventory Management standard has been successfully installed!${RESET}"
    echo ""
    echo "Available commands:"
    echo -e "${AMBER}  wia-ind-022 add-item${RESET}              - Add new inventory item"
    echo -e "${AMBER}  wia-ind-022 check-stock${RESET}           - Check stock levels"
    echo -e "${AMBER}  wia-ind-022 calculate-reorder${RESET}     - Calculate reorder point"
    echo -e "${AMBER}  wia-ind-022 forecast-demand${RESET}       - Forecast future demand"
    echo -e "${AMBER}  wia-ind-022 transfer${RESET}              - Transfer between warehouses"
    echo -e "${AMBER}  wia-ind-022 scan${RESET}                  - Scan barcode/RFID"
    echo -e "${AMBER}  wia-ind-022 check-expiration${RESET}      - Check expiring items"
    echo -e "${AMBER}  wia-ind-022 analyze-dead-stock${RESET}    - Analyze slow-moving inventory"
    echo -e "${AMBER}  wia-ind-022 report${RESET}                - Generate inventory reports"
    echo -e "${AMBER}  wia-ind-022 cycle-count${RESET}           - Perform cycle count"
    echo -e "${AMBER}  wia-ind-022 help${RESET}                  - Show help message"
    echo ""
    echo "Quick start:"
    echo -e "${GRAY}  # Add inventory item${RESET}"
    echo -e "${AMBER}  wia-ind-022 add-item --sku PROD-001 --quantity 500 --cost 19.99${RESET}"
    echo ""
    ...
    echo -e "${AMBER}  wia-ind-022 calculate-reorder --sku PROD-001 --lead-time 7${RESET}"
    echo ""
    echo -e "${GRAY}  # Forecast demand${RESET}"
    echo -e "${AMBER}  wia-ind-022 forecast-demand --sku PROD-001 --months 3${RESET}"
    echo ""
    echo -e "${GRAY}  # Analyze dead stock${RESET}"
    echo -e "${AMBER}  wia-ind-022 analyze-dead-stock --threshold 90${RESET}"
    echo ""
    echo "TypeScript SDK:"
    echo -e "${GRAY}  import { InventoryManager, calculateEOQ } from '@wia/ind-022';${RESET}"
    echo ""
    echo "Documentation:"
    echo -e "${GRAY}  $HOME/.wia/ind-022/docs/${RESET}"
    echo ""
