...
################################################################################
# WIA-DEF-001: Unmanned Weapon - Installation Script
#
# @version 1.0.0
# @license MIT
...
#
# This script installs the WIA-DEF-001 standard components:
# - CLI tool
# - TypeScript SDK
# - Documentation
################################################################################
...
print_header() {
    echo -e "${SLATE}"
    echo "╔════════════════════════════════════════════════════════════════╗"
    echo "║        🎯 WIA-DEF-001: Unmanned Weapon Installer              ║"
    echo "║                      Version 1.0.0                             ║"
    echo "╚════════════════════════════════════════════════════════════════╝"
    echo -e "${RESET}"
}
...
install_cli() {
    ...
    local cli_path="$SCRIPT_DIR/cli/wia-def-001.sh"
    ...
    if [ -w "$install_dir" ]; then
        cp "$cli_path" "$install_dir/wia-def-001"
        print_success "Installed CLI to $install_dir/wia-def-001"
    else
        print_info "Installing to $install_dir requires sudo..."
        sudo cp "$cli_path" "$install_dir/wia-def-001"
        print_success "Installed CLI to $install_dir/wia-def-001 (with sudo)"
    fi

    # Verify installation
    if command -v wia-def-001 &> /dev/null; then
        print_success "CLI tool is now available as 'wia-def-001'"
        print_info "Try: wia-def-001 --help"
    else
...
install_docs() {
    ...
    local docs_dir="$HOME/.wia/def-001/docs"
...
run_tests() {
    ...
    if command -v wia-def-001 &> /dev/null; then
        ...
        wia-def-001 version &> /dev/null && print_success "CLI test passed" || print_error "CLI test failed"
    fi
...
show_completion() {
    ...
    echo ""
    echo -e "${GREEN}The WIA-DEF-001 standard has been successfully installed!${RESET}"
    echo ""
    echo "Available commands:"
    echo -e "${CYAN}  wia-def-001 validate-engagement${RESET}  - Validate engagement parameters"
    echo -e "${CYAN}  wia-def-001 configure-swarm${RESET}      - Configure swarm coordination"
    echo -e "${CYAN}  wia-def-001 simulate${RESET}             - Simulate defense scenario"
    echo -e "${CYAN}  wia-def-001 check-compliance${RESET}     - Check system compliance"
    echo -e "${CYAN}  wia-def-001 assess-threat${RESET}        - Assess threat level"
    echo -e "${CYAN}  wia-def-001 help${RESET}                 - Show help message"
    echo ""
    echo "Quick start:"
    echo -e "${GRAY}  # Validate engagement${RESET}"
    echo -e "${CYAN}  wia-def-001 validate-engagement --target hostile-uav --certainty 0.98${RESET}"
    echo ""
    echo -e "${GRAY}  # Configure defensive swarm${RESET}"
    echo -e "${CYAN}  wia-def-001 configure-swarm --units 10 --formation defensive${RESET}"
    echo ""
    echo -e "${GRAY}  # Simulate perimeter defense${RESET}"
    echo -e "${CYAN}  wia-def-001 simulate --scenario perimeter-defense --duration 3600${RESET}"
    echo ""
    echo "TypeScript SDK:"
    echo -e "${GRAY}  import { UnmannedWeaponSDK } from '@wia/def-001';${RESET}"
    echo ""
    echo "Documentation:"
    echo -e "${GRAY}  $HOME/.wia/def-001/docs/${RESET}"
    echo ""
