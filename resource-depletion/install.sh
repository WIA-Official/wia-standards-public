...
################################################################################
# WIA-ENE-039: Resource Depletion Response Standard - Installation Script
#
# 弘益人間 (홍익人간) - Benefit All Humanity
#
# This script installs dependencies and sets up the WIA-ENE-039
# Resource Depletion Response Standard SDK and CLI tools.
#
# Usage:
...
while [[ $# -gt 0 ]]; do
  case $1 in
    ...
    --cli-only)
      ...
      ;;
    --help)
      echo "WIA-ENE-039 Installation Script"
      echo ""
      echo "Usage:"
...
print_header() {
  ...
  echo "╔════════════════════════════════════════════════════════════════╗"
  echo "║                                                                ║"
  echo "║     ⚠️  WIA-ENE-039: Resource Depletion Response Standard     ║"
  echo "║                                                                ║"
...
if $INSTALL_CLI; then
  ...
  if [ -f "${CLI_DIR}/resource-depletion.sh" ]; then
    chmod +x "${CLI_DIR}/resource-depletion.sh"
    print_step "CLI script made executable"
  else
    print_error "CLI script not found: ${CLI_DIR}/resource-depletion.sh"
    exit 1
  fi

  # Create symlink in /usr/local/bin (optional)
  if [ -w "/usr/local/bin" ]; then
    ln -sf "${CLI_DIR}/resource-depletion.sh" /usr/local/bin/wia-resource 2>/dev/null || true
    if [ -L "/usr/local/bin/wia-resource" ]; then
      print_step "Symlink created: /usr/local/bin/wia-resource"
      print_info "You can now run 'wia-resource' from anywhere"
    fi
  else
    print_warning "Cannot create symlink in /usr/local/bin (no write permission)"
    print_info "You can run the CLI using: ${CLI_DIR}/resource-depletion.sh"
  fi
...
if $INSTALL_SDK; then
  ...
  echo "  Usage:"
  echo "    import { ResourceDepletionSDK } from '@wia/ene-039';"
  echo ""
fi

if $INSTALL_CLI; then
  echo -e "${GREEN}✓${NC} CLI tools installed"
  echo "  Location: ${CLI_DIR}/resource-depletion.sh"
  echo "  Usage:"
  echo "    ./cli/resource-depletion.sh --help"
  if [ -L "/usr/local/bin/wia-resource" ]; then
    echo "  Or: wia-resource --help"
  fi
  echo ""
fi
...
echo "   - spec/WIA-ENE-039-v1.0.md: Detailed specification"
echo ""
echo "2. Try the SDK:"
echo "   cd ${SDK_DIR}"
echo "   npm test"
echo ""
echo "3. Try the CLI:"
echo "   ${CLI_DIR}/resource-depletion.sh --help"
echo ""
echo "4. Configure your API credentials:"
echo "   ${CLI_DIR}/resource-depletion.sh config"
echo ""
echo "5. Check resource status:"
echo "   ${CLI_DIR}/resource-depletion.sh status lithium"
echo ""
echo "6. Join the community:"
echo "   - GitHub: https://github.com/WIA-Official/wia-standards"
echo "   - Website: https://wia.org/standards/ene-039"
