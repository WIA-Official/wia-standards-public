//! HTTP client and API implementations for the WIA-DIGITAL_WILL SDK
//!
...
/// Main WIA-DIGITAL_WILL SDK client
///
/// # Example
///
/// ```rust,no_run
...
/// }
/// ```
pub struct DigitalWillClient {
    http: Client,
    config: Config,
    base_url: String,
}

impl DigitalWillClient {
    /// Create a new WIA-DIGITAL_WILL client
    pub fn new(config: Config) -> Self {
        let base_url = config
            .base_url
            .clone()
            .unwrap_or_else(|| "https://api.wiastandards.com/WIA-DIGITAL_WILL".to_string());
    ...
    /// Get data by ID
    pub async fn get(&self, id: &str) -> Result<DigitalWillData> {
        ...
        let result: DigitalWillResponse = response.json().await?;
        result.data.ok_or_else(|| Error::NotFound(id.to_string()))
    }

    /// Create new data
    pub async fn create(&self, request: CreateDigitalWillRequest) -> Result<DigitalWillData> {
    ...
    pub async fn list(&self, params: Option<ListParams>) -> Result<DigitalWillList> {
        let url = format!("{}/data", self.base_url);
