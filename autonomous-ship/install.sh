...
################################################################################
# WIA-AUTO-015: Autonomous Ship - Installation Script
#
# @version 1.0.0
# @license MIT
...
#
# This script installs the WIA-AUTO-015 standard components:
# - CLI tool
# - TypeScript SDK
# - Documentation
################################################################################
...
print_header() {
    echo -e "${ORANGE}"
    echo "╔════════════════════════════════════════════════════════════════╗"
    echo "║        🚢 WIA-AUTO-015: Autonomous Ship Installer             ║"
    echo "║                      Version 1.0.0                             ║"
    echo "╚════════════════════════════════════════════════════════════════╝"
    echo -e "${RESET}"
}
...
install_cli() {
    ...
    local cli_path="$SCRIPT_DIR/cli/wia-auto-015.sh"
    ...
    if [ -w "$install_dir" ]; then
        cp "$cli_path" "$install_dir/wia-auto-015"
        print_success "Installed CLI to $install_dir/wia-auto-015"
    else
        print_info "Installing to $install_dir requires sudo..."
        sudo cp "$cli_path" "$install_dir/wia-auto-015"
        print_success "Installed CLI to $install_dir/wia-auto-015 (with sudo)"
    fi

    # Verify installation
    if command -v wia-auto-015 &> /dev/null; then
        print_success "CLI tool is now available as 'wia-auto-015'"
        print_info "Try: wia-auto-015 --help"
    else
...
install_docs() {
    ...
    local docs_dir="$HOME/.wia/auto-015/docs"
...
run_tests() {
    ...
    if command -v wia-auto-015 &> /dev/null; then
        ...
        wia-auto-015 version &> /dev/null && print_success "CLI test passed" || print_error "CLI test failed"
    fi
...
show_completion() {
    ...
    echo ""
    echo -e "${GREEN}The WIA-AUTO-015 standard has been successfully installed!${RESET}"
    echo ""
    echo "Available commands:"
    echo -e "${CYAN}  wia-auto-015 init${RESET}                - Initialize ship system"
    echo -e "${CYAN}  wia-auto-015 collision-risk${RESET}      - Calculate collision risk"
    echo -e "${CYAN}  wia-auto-015 plan-route${RESET}          - Plan optimal route"
    echo -e "${CYAN}  wia-auto-015 monitor${RESET}             - Monitor ship status"
    echo -e "${CYAN}  wia-auto-015 avoid-collision${RESET}     - Execute avoidance maneuver"
    echo -e "${CYAN}  wia-auto-015 set-autonomy${RESET}        - Change autonomy level"
    echo -e "${CYAN}  wia-auto-015 help${RESET}                - Show help message"
    echo ""
    echo "Quick start:"
    echo -e "${GRAY}  # Initialize autonomous ship${RESET}"
    echo -e "${CYAN}  wia-auto-015 init --imo 9876543 --mmsi 123456789${RESET}"
    echo ""
    echo -e "${GRAY}  # Calculate collision risk${RESET}"
    echo -e "${CYAN}  wia-auto-015 collision-risk --own-pos '35.6762,139.6503' \\${RESET}"
    echo -e "${CYAN}    --own-heading 90 --own-speed 15 \\${RESET}"
    echo -e "${CYAN}    --target-pos '35.6850,139.7500' \\${RESET}"
    echo -e "${CYAN}    --target-heading 270 --target-speed 12${RESET}"
    echo ""
    echo -e "${GRAY}  # Plan route${RESET}"
    echo -e "${CYAN}  wia-auto-015 plan-route --from '35.6762,139.6503' --to '1.2897,103.8501'${RESET}"
    echo ""
    echo "TypeScript SDK:"
    echo -e "${GRAY}  import { AutonomousShipController } from '@wia/auto-015';${RESET}"
    echo ""
    echo "Documentation:"
    echo -e "${GRAY}  $HOME/.wia/auto-015/docs/${RESET}"
    echo ""
