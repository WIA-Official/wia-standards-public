//! HTTP client and API implementations for the WIA-INSURTECH SDK
//!
...
/// }
/// ```
pub struct InsurtechClient {
    http: Client,
    config: Config,
    base_url: String,
}

impl InsurtechClient {
    /// Create a new WIA-INSURTECH client
    pub fn new(config: Config) -> Self {
        let base_url = config
            .base_url
            .clone()
            .unwrap_or_else(|| "https://api.wiastandards.com/WIA-INSURTECH".to_string());
    ...
    pub async fn get(&self, id: &str) -> Result<InsurtechData> {
        ...
        let result: InsurtechResponse = response.json().await?;
        result.data.ok_or_else(|| Error::NotFound(id.to_string()))
    }

    /// Create new data
    pub async fn create(&self, request: CreateInsurtechRequest) -> Result<InsurtechData> {
    ...
    pub async fn list(&self, params: Option<ListParams>) -> Result<InsurtechList> {
        let url = format!("{}/data", self.base_url);
