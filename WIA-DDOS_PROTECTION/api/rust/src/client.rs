//! HTTP client and API implementations for the WIA-DDOS_PROTECTION SDK
//!
...
/// Main WIA-DDOS_PROTECTION SDK client
///
/// # Example
///
/// ```rust,no_run
...
/// }
/// ```
pub struct DdosProtectionClient {
    http: Client,
    config: Config,
    base_url: String,
}

impl DdosProtectionClient {
    /// Create a new WIA-DDOS_PROTECTION client
    pub fn new(config: Config) -> Self {
        let base_url = config
            .base_url
            .clone()
            .unwrap_or_else(|| "https://api.wiastandards.com/WIA-DDOS_PROTECTION".to_string());
    ...
    /// Validate data against WIA-DDOS_PROTECTION standards
    pub async fn validate(&self, data: &serde_json::Value) -> Result<ValidationResult> {
        validators::validate_data(data)
    }

    /// Get data by ID
    pub async fn get(&self, id: &str) -> Result<DdosProtectionData> {
        ...
        let result: DdosProtectionResponse = response.json().await?;
        result.data.ok_or_else(|| Error::NotFound(id.to_string()))
    }

    /// Create new data
    pub async fn create(&self, request: CreateDdosProtectionRequest) -> Result<DdosProtectionData> {
    ...
    pub async fn list(&self, params: Option<ListParams>) -> Result<DdosProtectionList> {
        let url = format!("{}/data", self.base_url);
