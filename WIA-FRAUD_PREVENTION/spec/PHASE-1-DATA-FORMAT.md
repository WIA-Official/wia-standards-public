# Fraud Prevention — Phase 1: Data Format Specification
...
Phase 1 defines standardized data formats for Fraud Prevention systems. This specification establishes the core data model, field definitions, validation rules, and serialization formats to ensure interoperability across risk assessment systems and compliance platforms.
...
```json
{
  ...
  "title": "WIA Fraud Prevention Data Format",
  "type": "object",
  "required": ["standard", "version", "timestamp", "data"],
  "properties": {
    "standard": {
      "type": "string",
      "const": "WIA-FRAUD_PREVENTION"
    },
    "version": {
      "type": "string",
      "pattern": "^\\d+\\.\\d+\\.\\d+$"
    ...
    "data": {
      "type": "object",
      "description": "Primary fraud detection data payload"
    }
  }
}
```
...
|-------|------|----------|-------------|
| standard | string | Yes | Standard identifier (WIA-FRAUD_PREVENTION) |
...
| data | object | Yes | Domain-specific fraud detection data |
...
### 5.3 File Extensions
- `.wia-fraud-prevention.json` — JSON format
- `.wia-fraud-prevention.pb` — Protocol Buffers
