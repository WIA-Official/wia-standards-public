...
################################################################################
# WIA-DEF-005: Cyber Defense - Installation Script
#
# @version 1.0.0
# @license MIT
...
#
# This script installs the WIA-DEF-005 standard components:
# - CLI tool
# - TypeScript SDK
# - Documentation
################################################################################
...
print_header() {
    echo -e "${SLATE}"
    echo "╔════════════════════════════════════════════════════════════════╗"
    echo "║       🛡️  WIA-DEF-005: Cyber Defense Installer               ║"
    echo "║                      Version 1.0.0                             ║"
    echo "╚════════════════════════════════════════════════════════════════╝"
    echo -e "${RESET}"
}
...
install_cli() {
    ...
    local cli_path="$SCRIPT_DIR/cli/wia-def-005.sh"
    ...
    if [ -w "$install_dir" ]; then
        cp "$cli_path" "$install_dir/wia-def-005"
        print_success "Installed CLI to $install_dir/wia-def-005"
    else
        print_info "Installing to $install_dir requires sudo..."
        sudo cp "$cli_path" "$install_dir/wia-def-005"
        print_success "Installed CLI to $install_dir/wia-def-005 (with sudo)"
    fi

    # Verify installation
    if command -v wia-def-005 &> /dev/null; then
        print_success "CLI tool is now available as 'wia-def-005'"
        print_info "Try: wia-def-005 --help"
    else
...
install_docs() {
    ...
    local docs_dir="$HOME/.wia/def-005/docs"
...
run_tests() {
    ...
    if command -v wia-def-005 &> /dev/null; then
        ...
        wia-def-005 version &> /dev/null && print_success "CLI test passed" || print_error "CLI test failed"
    fi
...
show_completion() {
    ...
    echo ""
    echo -e "${GREEN}The WIA-DEF-005 standard has been successfully installed!${RESET}"
    echo ""
    echo "Available commands:"
    echo -e "${CYAN}  wia-def-005 detect-threats${RESET}        - Detect security threats"
    echo -e "${CYAN}  wia-def-005 analyze-incident${RESET}      - Analyze security incident"
    echo -e "${CYAN}  wia-def-005 scan-vulnerabilities${RESET}  - Scan for vulnerabilities"
    echo -e "${CYAN}  wia-def-005 harden-network${RESET}        - Harden network security"
    echo -e "${CYAN}  wia-def-005 monitor-soc${RESET}           - Monitor SOC dashboard"
    echo -e "${CYAN}  wia-def-005 help${RESET}                  - Show help message"
    echo ""
    echo "Quick start:"
    echo -e "${GRAY}  # Detect active threats${RESET}"
    echo -e "${CYAN}  wia-def-005 detect-threats --source network --severity HIGH${RESET}"
    echo ""
    echo -e "${GRAY}  # Analyze incident${RESET}"
    echo -e "${CYAN}  wia-def-005 analyze-incident --id INC-2024-001 --forensics${RESET}"
    echo ""
    echo -e "${GRAY}  # Scan vulnerabilities${RESET}"
    echo -e "${CYAN}  wia-def-005 scan-vulnerabilities --target web-server-01${RESET}"
    echo ""
    echo "TypeScript SDK:"
    echo -e "${GRAY}  import { detectThreats } from '@wia/def-005';${RESET}"
    echo ""
    echo "Documentation:"
    echo -e "${GRAY}  $HOME/.wia/def-005/docs/${RESET}"
    echo ""
