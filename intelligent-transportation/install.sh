...
################################################################################
# WIA-AUTO-011: Intelligent Transportation System - Installation Script
#
# @version 1.0.0
# @license MIT
...
#
# This script installs the WIA-AUTO-011 standard components:
# - CLI tool
# - TypeScript SDK
# - Documentation
################################################################################
...
install_cli() {
    ...
    local cli_path="$SCRIPT_DIR/cli/wia-auto-011.sh"
    ...
    if [ -w "$install_dir" ]; then
        cp "$cli_path" "$install_dir/wia-auto-011"
        print_success "Installed CLI to $install_dir/wia-auto-011"
    else
        print_info "Installing to $install_dir requires sudo..."
        sudo cp "$cli_path" "$install_dir/wia-auto-011"
        print_success "Installed CLI to $install_dir/wia-auto-011 (with sudo)"
    fi

    # Verify installation
    if command -v wia-auto-011 &> /dev/null; then
        print_success "CLI tool is now available as 'wia-auto-011'"
        print_info "Try: wia-auto-011 --help"
    else
...
install_docs() {
    ...
    local docs_dir="$HOME/.wia/auto-011/docs"
...
run_tests() {
    ...
    if command -v wia-auto-011 &> /dev/null; then
        ...
        wia-auto-011 version &> /dev/null && print_success "CLI test passed" || print_error "CLI test failed"
    fi
...
show_completion() {
    ...
    echo ""
    echo -e "${GREEN}The WIA-AUTO-011 standard has been successfully installed!${RESET}"
    echo ""
    echo "Available commands:"
    echo -e "${CYAN}  wia-auto-011 calc-flow${RESET}           - Calculate traffic flow"
    echo -e "${CYAN}  wia-auto-011 optimize-signals${RESET}    - Optimize signal timing"
    echo -e "${CYAN}  wia-auto-011 predict${RESET}             - Predict congestion"
    echo -e "${CYAN}  wia-auto-011 monitor${RESET}             - Monitor traffic"
    echo -e "${CYAN}  wia-auto-011 preempt${RESET}             - Emergency vehicle preemption"
    echo -e "${CYAN}  wia-auto-011 help${RESET}                - Show help message"
    echo ""
    echo "Quick start:"
    echo -e "${GRAY}  # Calculate traffic flow${RESET}"
    echo -e "${CYAN}  wia-auto-011 calc-flow --density 50 --speed 60${RESET}"
    echo ""
    echo -e "${GRAY}  # Optimize signal timing${RESET}"
    echo -e "${CYAN}  wia-auto-011 optimize-signals --flow 800,600 --saturation 1800,1600${RESET}"
    echo ""
    echo -e "${GRAY}  # Predict congestion${RESET}"
    echo -e "${CYAN}  wia-auto-011 predict --location 'Downtown' --window 60${RESET}"
    echo ""
    echo "TypeScript SDK:"
    echo -e "${GRAY}  import { calculateTrafficFlow, optimizeSignalTiming } from '@wia/auto-011';${RESET}"
    echo ""
    echo "Documentation:"
    echo -e "${GRAY}  $HOME/.wia/auto-011/docs/${RESET}"
    echo ""
