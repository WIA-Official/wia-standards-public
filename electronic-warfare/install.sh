...
################################################################################
# WIA-DEF-006: Electronic Warfare - Installation Script
#
# @version 1.0.0
# @license MIT
...
#
# This script installs the WIA-DEF-006 standard components:
# - CLI tool
# - TypeScript SDK
# - Documentation
################################################################################
...
print_header() {
    echo -e "${SLATE}"
    echo "╔════════════════════════════════════════════════════════════════╗"
    echo "║      📡 WIA-DEF-006: Electronic Warfare Installer            ║"
    echo "║                      Version 1.0.0                             ║"
    echo "╚════════════════════════════════════════════════════════════════╝"
    echo -e "${RESET}"
}
...
install_cli() {
    ...
    local cli_path="$SCRIPT_DIR/cli/wia-def-006.sh"
    ...
    if [ -w "$install_dir" ]; then
        cp "$cli_path" "$install_dir/wia-def-006"
        print_success "Installed CLI to $install_dir/wia-def-006"
    else
        print_info "Installing to $install_dir requires sudo..."
        sudo cp "$cli_path" "$install_dir/wia-def-006"
        print_success "Installed CLI to $install_dir/wia-def-006 (with sudo)"
    fi

    # Verify installation
    if command -v wia-def-006 &> /dev/null; then
        print_success "CLI tool is now available as 'wia-def-006'"
        print_info "Try: wia-def-006 --help"
    else
...
install_docs() {
    ...
    local docs_dir="$HOME/.wia/def-006/docs"
...
run_tests() {
    ...
    if command -v wia-def-006 &> /dev/null; then
        ...
        wia-def-006 version &> /dev/null && print_success "CLI test passed" || print_error "CLI test failed"
    fi
...
show_completion() {
    ...
    echo ""
    echo -e "${GREEN}The WIA-DEF-006 standard has been successfully installed!${RESET}"
    echo ""
    echo "Available commands:"
    echo -e "${CYAN}  wia-def-006 calc-jamming${RESET}      - Calculate jamming power"
    echo -e "${CYAN}  wia-def-006 analyze-signal${RESET}    - Analyze signal parameters"
    echo -e "${CYAN}  wia-def-006 generate-fhss${RESET}     - Generate frequency hopping"
    echo -e "${CYAN}  wia-def-006 spectrum-scan${RESET}     - Scan spectrum"
    echo -e "${CYAN}  wia-def-006 calc-path-loss${RESET}    - Calculate path loss"
    echo -e "${CYAN}  wia-def-006 calc-rf-safety${RESET}    - RF safety calculations"
    echo -e "${CYAN}  wia-def-006 help${RESET}              - Show help message"
    echo ""
    echo "Quick start:"
    echo -e "${GRAY}  # Calculate jamming power for 2.4 GHz target${RESET}"
    echo -e "${CYAN}  wia-def-006 calc-jamming --freq 2400 --distance 10000 --target-power 100${RESET}"
    echo ""
    echo -e "${GRAY}  # Analyze radar signal${RESET}"
    echo -e "${CYAN}  wia-def-006 analyze-signal --freq 5800 --bandwidth 20 --type radar${RESET}"
    echo ""
    echo -e "${GRAY}  # Generate frequency hopping pattern${RESET}"
    echo -e "${CYAN}  wia-def-006 generate-fhss --channels 50 --dwell-time 100${RESET}"
    echo ""
    echo "TypeScript SDK:"
    echo -e "${GRAY}  import { calculateJammingPower } from '@wia/def-006';${RESET}"
    echo ""
    echo "Documentation:"
    echo -e "${GRAY}  $HOME/.wia/def-006/docs/${RESET}"
    echo ""
