# WIA-HERITAGE-004: Intangible Cultural Heritage Standard
...
The WIA-HERITAGE-004 standard defines a comprehensive framework for digitizing, preserving, and sharing cultural artifacts through advanced 3D scanning, photogrammetry, AI-powered restoration, and blockchain-based provenance tracking.
...
```bash
curl -fsSL https://raw.githubusercontent.com/WIA-Official/wia-standards/main/standards/WIA-HERITAGE-004-intangible-heritage/install.sh | bash
```
...
```bash
wia-heritage-004 scan --input photos/ --output artifact.gltf --metadata metadata.json
```

### Create Virtual Exhibition

```bash
wia-heritage-004 exhibit --artifacts artifacts/ --output exhibition/ --theme ancient-civilizations
```

### Verify Provenance

```bash
wia-heritage-004 provenance --artifact artifact.json --blockchain ethereum --verify
```
...
```typescript
import { WIAHeritage004 } from '@wia/heritage-004';

const heritage = new WIAHeritage004({
  apiKey: 'your-api-key',
  blockchain: 'ethereum'
});
