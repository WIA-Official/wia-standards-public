...
################################################################################
# WIA-AUTO-004: Electric Vehicle Standard - Installation Script
#
# @version 1.0.0
# @license MIT
...
#
# This script installs the WIA-AUTO-004 standard components:
# - CLI tool
# - TypeScript SDK
# - Documentation
################################################################################
...
print_header() {
    echo -e "${ORANGE}"
    echo "╔════════════════════════════════════════════════════════════════╗"
    echo "║        🔋 WIA-AUTO-004: Electric Vehicle Installer            ║"
    echo "║                      Version 1.0.0                             ║"
    echo "╚════════════════════════════════════════════════════════════════╝"
    echo -e "${RESET}"
}
...
install_cli() {
    ...
    local cli_path="$SCRIPT_DIR/cli/wia-auto-004.sh"
    ...
    if [ -w "$install_dir" ]; then
        cp "$cli_path" "$install_dir/wia-auto-004"
        print_success "Installed CLI to $install_dir/wia-auto-004"
    else
        print_info "Installing to $install_dir requires sudo..."
        sudo cp "$cli_path" "$install_dir/wia-auto-004"
        print_success "Installed CLI to $install_dir/wia-auto-004 (with sudo)"
    fi

    # Verify installation
    if command -v wia-auto-004 &> /dev/null; then
        print_success "CLI tool is now available as 'wia-auto-004'"
        print_info "Try: wia-auto-004 --help"
    else
...
install_docs() {
    ...
    local docs_dir="$HOME/.wia/auto-004/docs"
...
run_tests() {
    ...
    if command -v wia-auto-004 &> /dev/null; then
        ...
        wia-auto-004 version &> /dev/null && print_success "CLI test passed" || print_error "CLI test failed"
    fi
...
show_completion() {
    ...
    echo ""
    echo -e "${GREEN}The WIA-AUTO-004 standard has been successfully installed!${RESET}"
    echo ""
    echo "Available commands:"
    echo -e "${CYAN}  wia-auto-004 calc-range${RESET}       - Calculate vehicle range"
    echo -e "${CYAN}  wia-auto-004 calc-energy${RESET}      - Calculate energy consumption"
    echo -e "${CYAN}  wia-auto-004 calc-charging${RESET}    - Calculate charging time"
    echo -e "${CYAN}  wia-auto-004 simulate${RESET}         - Simulate drive profile"
    echo -e "${CYAN}  wia-auto-004 calc-regen${RESET}       - Calculate regenerative braking"
    echo -e "${CYAN}  wia-auto-004 help${RESET}             - Show help message"
    echo ""
    echo "Quick start:"
    echo -e "${GRAY}  # Calculate range with 75 kWh battery at 80% SoC${RESET}"
    echo -e "${CYAN}  wia-auto-004 calc-range --battery 75 --soc 80 --consumption 0.18${RESET}"
    echo ""
    echo -e "${GRAY}  # Calculate charging time from 20% to 80% with 150 kW charger${RESET}"
    echo -e "${CYAN}  wia-auto-004 calc-charging --battery 75 --from 20 --to 80 --power 150${RESET}"
    echo ""
    echo -e "${GRAY}  # Simulate highway drive profile${RESET}"
    echo -e "${CYAN}  wia-auto-004 simulate --profile highway --distance 100 --battery 75${RESET}"
    echo ""
    echo "TypeScript SDK:"
    echo -e "${GRAY}  import { calculateRange } from '@wia/auto-004';${RESET}"
    echo ""
    echo "Documentation:"
    echo -e "${GRAY}  $HOME/.wia/auto-004/docs/${RESET}"
    echo ""
