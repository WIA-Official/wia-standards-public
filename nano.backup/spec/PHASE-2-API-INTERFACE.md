# WIA-nano.backup PHASE 2 — API-INTERFACE Specification

**Standard:** WIA-nano.backup
...
`urn:wia:nano.backup:status-transition`.
...
`422` with type
`urn:wia:nano.backup:source-mix-required`.
...
with type
`urn:wia:nano.backup:laboratory-not-accredited`.
...
`409` with type
`urn:wia:nano.backup:quality-certificate-required`.
...
- `urn:wia:nano.backup:hydrogen-quality-out-of-grade`
- `urn:wia:nano.backup:cryogenic-temperature-excursion`
- `urn:wia:nano.backup:guarantee-of-origin-mismatch`
- `urn:wia:nano.backup:evidence-mismatch`
