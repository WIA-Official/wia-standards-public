...
################################################################################
# WIA-ENE-028: Noise Pollution Standard - Installation Script
#
# 弘익人間 (홍익인간) - Benefit All Humanity
#
# This script installs dependencies and sets up the WIA-ENE-028
# Noise Pollution Standard SDK and CLI tools.
#
# Usage:
...
while [[ $# -gt 0 ]]; do
  case $1 in
    ...
    --cli-only)
      ...
      ;;
    --help)
      echo "WIA-ENE-028 Installation Script"
      echo ""
      echo "Usage:"
...
print_header() {
  ...
  echo "╔════════════════════════════════════════════════════════════════╗"
  echo "║                                                                ║"
  echo "║         🔊  WIA-ENE-028: Noise Pollution Standard             ║"
  echo "║                                                                ║"
...
if $INSTALL_CLI; then
  ...
  if [ -f "${CLI_DIR}/noise-pollution.sh" ]; then
    chmod +x "${CLI_DIR}/noise-pollution.sh"
    print_step "CLI script made executable"
  else
    print_error "CLI script not found: ${CLI_DIR}/noise-pollution.sh"
    exit 1
  fi

  # Create symlink in /usr/local/bin (optional)
  if [ -w "/usr/local/bin" ]; then
    ln -sf "${CLI_DIR}/noise-pollution.sh" /usr/local/bin/wia-noise 2>/dev/null || true
    if [ -L "/usr/local/bin/wia-noise" ]; then
      print_step "Symlink created: /usr/local/bin/wia-noise"
      print_info "You can now run 'wia-noise' from anywhere"
    fi
  else
    print_warning "Cannot create symlink in /usr/local/bin (no write permission)"
    print_info "You can run the CLI using: ${CLI_DIR}/noise-pollution.sh"
  fi
...
if $INSTALL_SDK; then
  ...
  echo "  Usage:"
  echo "    import { NoisePollutionSDK } from '@wia/ene-028';"
  echo ""
fi

if $INSTALL_CLI; then
  echo -e "${GREEN}✓${NC} CLI tools installed"
  echo "  Location: ${CLI_DIR}/noise-pollution.sh"
  echo "  Usage:"
  echo "    ./cli/noise-pollution.sh --help"
  if [ -L "/usr/local/bin/wia-noise" ]; then
    echo "  Or: wia-noise --help"
  fi
  echo ""
fi
...
echo "   - spec/WIA-ENE-028-v1.0.md: Detailed specification"
echo ""
echo "2. Set your API key:"
echo "   export WIA_API_KEY=your-api-key-here"
echo ""
echo "3. Try the SDK:"
echo "   cd ${SDK_DIR}"
echo "   npm test"
echo ""
echo "4. Try the CLI:"
echo "   ${CLI_DIR}/noise-pollution.sh realtime Seoul"
echo ""
echo "5. Join the community:"
echo "   - GitHub: https://github.com/WIA-Official/wia-standards"
echo "   - Website: https://wia.org/standards/ene-028"
