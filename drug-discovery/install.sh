...
################################################################################
# WIA-BIO-009: Drug Discovery - Installation Script
#
# @version 1.0.0
# @license MIT
...
#
# This script installs the WIA-BIO-009 standard components:
# - CLI tool
# - TypeScript SDK
# - Documentation
################################################################################
...
print_header() {
    echo -e "${TEAL}"
    echo "╔════════════════════════════════════════════════════════════════╗"
    echo "║        💊 WIA-BIO-009: Drug Discovery Installer               ║"
    echo "║                      Version 1.0.0                             ║"
    echo "╚════════════════════════════════════════════════════════════════╝"
    echo -e "${RESET}"
}
...
install_cli() {
    ...
    local cli_path="$SCRIPT_DIR/cli/wia-bio-009.sh"
    ...
    if [ -w "$install_dir" ]; then
        cp "$cli_path" "$install_dir/wia-bio-009"
        print_success "Installed CLI to $install_dir/wia-bio-009"
    else
        print_info "Installing to $install_dir requires sudo..."
        sudo cp "$cli_path" "$install_dir/wia-bio-009"
        print_success "Installed CLI to $install_dir/wia-bio-009 (with sudo)"
    fi

    # Verify installation
    if command -v wia-bio-009 &> /dev/null; then
        print_success "CLI tool is now available as 'wia-bio-009'"
        print_info "Try: wia-bio-009 --help"
    else
...
install_docs() {
    ...
    local docs_dir="$HOME/.wia/bio-009/docs"
...
run_tests() {
    ...
    if command -v wia-bio-009 &> /dev/null; then
        ...
        wia-bio-009 version &> /dev/null && print_success "CLI test passed" || print_error "CLI test failed"
    fi
...
show_completion() {
    ...
    echo ""
    echo -e "${GREEN}The WIA-BIO-009 standard has been successfully installed!${RESET}"
    echo ""
    echo "Available commands:"
    echo -e "${CYAN}  wia-bio-009 screen${RESET}             - Screen compound library"
    echo -e "${CYAN}  wia-bio-009 optimize${RESET}           - Optimize lead compound"
    echo -e "${CYAN}  wia-bio-009 predict-admet${RESET}      - Predict ADMET properties"
    echo -e "${CYAN}  wia-bio-009 assess-safety${RESET}      - Assess safety profile"
    echo -e "${CYAN}  wia-bio-009 generate-dossier${RESET}   - Generate regulatory dossier"
    echo -e "${CYAN}  wia-bio-009 help${RESET}               - Show help message"
    echo ""
    echo "Quick start:"
    echo -e "${GRAY}  # Screen compounds against EGFR${RESET}"
    echo -e "${CYAN}  wia-bio-009 screen --target 'EGFR' --library compounds.sdf${RESET}"
    echo ""
    echo -e "${GRAY}  # Predict ADMET for caffeine${RESET}"
    echo -e "${CYAN}  wia-bio-009 predict-admet --smiles 'CN1C=NC2=C1C(=O)N(C(=O)N2C)C'${RESET}"
    echo ""
    echo -e "${GRAY}  # Generate Phase II dossier${RESET}"
    echo -e "${CYAN}  wia-bio-009 generate-dossier --compound-id WIA-001 --phase Phase-II${RESET}"
    echo ""
    echo "TypeScript SDK:"
    echo -e "${GRAY}  import { DrugDiscoverySDK } from '@wia/bio-009';${RESET}"
    echo -e "${GRAY}  const sdk = new DrugDiscoverySDK();${RESET}"
    echo -e "${GRAY}  const admet = sdk.predictADMET({ smiles: 'CC(=O)O' });${RESET}"
    echo ""
    echo "Documentation:"
    echo -e "${GRAY}  $HOME/.wia/bio-009/docs/${RESET}"
    echo ""
