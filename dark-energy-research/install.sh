...
################################################################################
# WIA-QUA-014: Dark Energy Research - Installation Script
#
# @version 1.0.0
# @license MIT
...
#
# This script installs the WIA-QUA-014 standard components:
# - CLI tool
# - TypeScript SDK
# - Documentation
################################################################################
...
print_header() {
    echo -e "${INDIGO}"
    echo "╔════════════════════════════════════════════════════════════════╗"
    echo "║      🌌 WIA-QUA-014: Dark Energy Research Installer           ║"
    echo "║                      Version 1.0.0                             ║"
    echo "╚════════════════════════════════════════════════════════════════╝"
    echo -e "${RESET}"
}
...
install_cli() {
    ...
    local cli_script="$SCRIPT_DIR/cli/wia-qua-014.sh"
    local install_dir="/usr/local/bin"
    local install_path="$install_dir/wia-qua-014"
    ...
    # Verify installation
    if command -v wia-qua-014 &> /dev/null; then
        print_success "CLI tool installed successfully"
        print_info "Run 'wia-qua-014 --version' to verify"
    else
...
install_typescript_sdk() {
    ...
    print_info "SDK can be imported with: import { DarkEnergyResearchSDK } from '@wia/qua-014'"
...
setup_documentation() {
    ...
    local spec_file="$SCRIPT_DIR/spec/WIA-QUA-014-v1.0.md"
    ...
    print_info "  - https://wiastandards.com/standards/dark-energy-research"
    print_info "  - https://docs.wiastandards.com/qua-014"
}
...
run_tests() {
    ...
    if wia-qua-014 --version &> /dev/null; then
...
print_summary() {
    ...
    echo ""
    print_success "WIA-QUA-014 Dark Energy Research Standard installed successfully!"
    echo ""

    print_info "Available commands:"
    print_info "  wia-qua-014 distance --model LCDM --H0 70 --OmegaM 0.3 --redshift 1.0"
    print_info "  wia-qua-014 equation-of-state --model CPL --w0 -1.0 --wa 0.0"
    print_info "  wia-qua-014 hubble-tension --early planck --late sh0es"
    print_info "  wia-qua-014 fate --model phantom --w -1.2"
    print_info "  wia-qua-014 hubble-diagram --data pantheon --model LCDM"
    print_info "  wia-qua-014 compare --models LCDM,wCDM,CPL"
    print_info "  wia-qua-014 vacuum-energy --cutoff Planck"
    echo ""

    print_info "For help: wia-qua-014 help"
    echo ""

    print_info "TypeScript SDK usage:"
    print_info "  import { DarkEnergyResearchSDK } from '@wia/qua-014';"
    print_info "  const sdk = new DarkEnergyResearchSDK();"
    print_info "  const model = sdk.createModel('LCDM', { H0: 70, OmegaM: 0.3, OmegaLambda: 0.7 });"
    print_info "  const distances = model.getDistances(1.0);"
    echo ""

    print_info "Documentation:"
    print_info "  Specification: $SCRIPT_DIR/spec/WIA-QUA-014-v1.0.md"
    print_info "  README: $SCRIPT_DIR/README.md"
    print_info "  Online: https://wiastandards.com/standards/dark-energy-research"
    echo ""
