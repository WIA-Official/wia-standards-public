...
################################################################################
# WIA-CORE-002: Universal Consent - Installation Script
#
# @version 1.0.0
# @license MIT
...
#
# This script installs the WIA-CORE-002 standard components:
# - CLI tool
# - TypeScript SDK
# - Documentation
################################################################################
...
print_header() {
    echo -e "${INDIGO}"
    echo "╔════════════════════════════════════════════════════════════════╗"
    echo "║       ✅ WIA-CORE-002: Universal Consent Installer            ║"
    echo "║                      Version 1.0.0                             ║"
    echo "╚════════════════════════════════════════════════════════════════╝"
    echo -e "${RESET}"
}
...
install_cli() {
    ...
    local cli_path="$SCRIPT_DIR/cli/wia-core-002.sh"
    ...
    if [ -w "$install_dir" ]; then
        cp "$cli_path" "$install_dir/wia-core-002"
        print_success "Installed CLI to $install_dir/wia-core-002"
    else
        print_info "Installing to $install_dir requires sudo..."
        sudo cp "$cli_path" "$install_dir/wia-core-002"
        print_success "Installed CLI to $install_dir/wia-core-002 (with sudo)"
    fi

    # Verify installation
    if command -v wia-core-002 &> /dev/null; then
        print_success "CLI tool is now available as 'wia-core-002'"
        print_info "Try: wia-core-002 --help"
    else
...
create_storage() {
    print_section "Creating Storage Directory"

    local storage_dir="$HOME/.wia/consent"

    mkdir -p "$storage_dir"
    print_success "Created storage directory: $storage_dir"

    # Initialize database
    local db_file="$storage_dir/consents.json"
    if [ ! -f "$db_file" ]; then
        echo '{"consents": [], "audit_log": []}' > "$db_file"
        print_success "Initialized consent database"
    else
        print_info "Database already exists"
    fi

    print_info "Storage location: $storage_dir"
}
...
install_docs() {
    ...
    local docs_dir="$HOME/.wia/core-002/docs"
...
run_tests() {
    ...
    if command -v wia-core-002 &> /dev/null; then
        ...
        wia-core-002 version &> /dev/null && print_success "CLI test passed" || print_error "CLI test failed"
    fi
    ...
    # Test storage
    if [ -f "$HOME/.wia/consent/consents.json" ]; then
...
show_completion() {
    ...
    echo ""
    echo -e "${GREEN}The WIA-CORE-002 standard has been successfully installed!${RESET}"
    echo ""
    echo "Available commands:"
    echo -e "${CYAN}  wia-core-002 grant${RESET}            - Grant consent"
    echo -e "${CYAN}  wia-core-002 check${RESET}            - Check consent status"
    echo -e "${CYAN}  wia-core-002 list${RESET}             - List all consents"
    echo -e "${CYAN}  wia-core-002 revoke${RESET}           - Revoke consent"
    echo -e "${CYAN}  wia-core-002 export${RESET}           - Export consent history"
    echo -e "${CYAN}  wia-core-002 validate${RESET}         - Validate GDPR compliance"
    echo -e "${CYAN}  wia-core-002 help${RESET}             - Show help message"
    echo ""
    echo "Quick start:"
    echo -e "${GRAY}  # Grant marketing consent${RESET}"
    echo -e "${CYAN}  wia-core-002 grant --user user-123 --purpose email_marketing${RESET}"
    echo ""
    echo -e "${GRAY}  # Check consent status${RESET}"
    echo -e "${CYAN}  wia-core-002 check --user user-123 --purpose email_marketing${RESET}"
    echo ""
    echo -e "${GRAY}  # List all consents for user${RESET}"
    echo -e "${CYAN}  wia-core-002 list --user user-123${RESET}"
    echo ""
    echo -e "${GRAY}  # Revoke consent${RESET}"
    echo -e "${CYAN}  wia-core-002 revoke --user user-123 --purpose email_marketing${RESET}"
    echo ""
    echo "TypeScript SDK:"
    echo -e "${GRAY}  import { ConsentManager } from '@wia/core-002';${RESET}"
    echo -e "${GRAY}  const manager = new ConsentManager();${RESET}"
    echo ""
    echo "Documentation:"
    echo -e "${GRAY}  $HOME/.wia/core-002/docs/${RESET}"
    echo ""
    echo "Storage:"
    echo -e "${GRAY}  $HOME/.wia/consent/${RESET}"
    echo ""
