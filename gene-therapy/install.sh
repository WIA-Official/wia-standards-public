...
################################################################################
# WIA-BIO-003: Gene Therapy - Installation Script
#
# @version 1.0.0
# @license MIT
...
#
# This script installs the WIA-BIO-003 standard components:
# - CLI tool
# - TypeScript SDK
# - Documentation
################################################################################
...
print_header() {
    echo -e "${TEAL}"
    echo "╔════════════════════════════════════════════════════════════════╗"
    echo "║        🧬 WIA-BIO-003: Gene Therapy Installer                 ║"
    echo "║                      Version 1.0.0                             ║"
    echo "╚════════════════════════════════════════════════════════════════╝"
    echo -e "${RESET}"
}
...
install_cli() {
    ...
    local cli_path="$SCRIPT_DIR/cli/wia-bio-003.sh"
    ...
    if [ -w "$install_dir" ]; then
        cp "$cli_path" "$install_dir/wia-bio-003"
        print_success "Installed CLI to $install_dir/wia-bio-003"
    else
        print_info "Installing to $install_dir requires sudo..."
        sudo cp "$cli_path" "$install_dir/wia-bio-003"
        print_success "Installed CLI to $install_dir/wia-bio-003 (with sudo)"
    fi

    # Verify installation
    if command -v wia-bio-003 &> /dev/null; then
        print_success "CLI tool is now available as 'wia-bio-003'"
        print_info "Try: wia-bio-003 --help"
    else
...
install_docs() {
    ...
    local docs_dir="$HOME/.wia/bio-003/docs"
...
run_tests() {
    ...
    if command -v wia-bio-003 &> /dev/null; then
        ...
        wia-bio-003 version &> /dev/null && print_success "CLI test passed" || print_error "CLI test failed"
    fi
...
show_completion() {
    ...
    echo ""
    echo -e "${GREEN}The WIA-BIO-003 standard has been successfully installed!${RESET}"
    echo ""
    echo "Available commands:"
    echo -e "${CYAN}  wia-bio-003 calc-dosage${RESET}        - Calculate viral vector dosage"
    echo -e "${CYAN}  wia-bio-003 assess-safety${RESET}      - Assess patient safety profile"
    echo -e "${CYAN}  wia-bio-003 predict-efficiency${RESET} - Predict transduction efficiency"
    echo -e "${CYAN}  wia-bio-003 monitor-expression${RESET} - Monitor gene expression"
    echo -e "${CYAN}  wia-bio-003 generate-protocol${RESET}  - Generate clinical protocol"
    echo -e "${CYAN}  wia-bio-003 help${RESET}               - Show help message"
    echo ""
    echo "Quick start:"
    echo -e "${GRAY}  # Calculate dosage for liver-directed AAV9 therapy${RESET}"
    echo -e "${CYAN}  wia-bio-003 calc-dosage --weight 70 --tissue liver --vector AAV9${RESET}"
    echo ""
    echo -e "${GRAY}  # Assess safety for patient with normal liver function${RESET}"
    echo -e "${CYAN}  wia-bio-003 assess-safety --dose 1e13 --alt 25 --ast 30${RESET}"
    echo ""
    echo -e "${GRAY}  # Generate Phase I/II protocol for hemophilia${RESET}"
    echo -e "${CYAN}  wia-bio-003 generate-protocol --condition hemophilia --vector AAV9${RESET}"
    echo ""
    echo "TypeScript SDK:"
    echo -e "${GRAY}  import { calculateDosage, assessSafety } from '@wia/bio-003';${RESET}"
    echo ""
    echo "Documentation:"
    echo -e "${GRAY}  $HOME/.wia/bio-003/docs/${RESET}"
    echo ""
