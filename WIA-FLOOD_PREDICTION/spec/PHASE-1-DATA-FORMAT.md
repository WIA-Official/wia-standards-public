# Flood Prediction — Phase 1: Data Format Specification
...
Phase 1 defines standardized data formats for Flood Prediction systems. This specification establishes the core data model, field definitions, validation rules, and serialization formats to ensure interoperability across flood monitoring systems and early warning platforms.
...
```json
{
  ...
  "title": "WIA Flood Prediction Data Format",
  "type": "object",
  "required": ["standard", "version", "timestamp", "data"],
  "properties": {
    "standard": {
      "type": "string",
      "const": "WIA-FLOOD_PREDICTION"
    },
    "version": {
      "type": "string",
      "pattern": "^\\d+\\.\\d+\\.\\d+$"
    ...
    "data": {
      "type": "object",
      "description": "Primary hydrological data payload"
    }
  }
}
```
...
|-------|------|----------|-------------|
| standard | string | Yes | Standard identifier (WIA-FLOOD_PREDICTION) |
...
| data | object | Yes | Domain-specific hydrological data |
...
### 5.3 File Extensions
- `.wia-flood-prediction.json` — JSON format
- `.wia-flood-prediction.pb` — Protocol Buffers
