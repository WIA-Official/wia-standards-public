//! HTTP client and API implementations for the 3d-image-sensor SDK
//!
...
/// Main 3d-image-sensor SDK client
///
/// # Example
///
/// ```rust,no_run
...
/// }
/// ```
pub struct ThreeDImageSensorClient {
    http: Client,
    config: Config,
    base_url: String,
}

impl ThreeDImageSensorClient {
    /// Create a new 3d-image-sensor client
    pub fn new(config: Config) -> Self {
        let base_url = config
            .base_url
            .clone()
            .unwrap_or_else(|| "https://api.wiastandards.com/3d-image-sensor".to_string());
    ...
    /// Validate data against 3d-image-sensor standards
    pub async fn validate(&self, data: &serde_json::Value) -> Result<ValidationResult> {
        validators::validate_data(data)
    }

    /// Get data by ID
    pub async fn get(&self, id: &str) -> Result<ThreeDImageSensorData> {
        ...
        let result: ThreeDImageSensorResponse = response.json().await?;
        result.data.ok_or_else(|| Error::NotFound(id.to_string()))
    }

    /// Create new data
    pub async fn create(&self, request: CreateThreeDImageSensorRequest) -> Result<ThreeDImageSensorData> {
    ...
    pub async fn list(&self, params: Option<ListParams>) -> Result<ThreeDImageSensorList> {
        let url = format!("{}/data", self.base_url);
