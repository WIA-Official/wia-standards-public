...
################################################################################
# WIA-CORE-010: Universal Error Handling - Installation Script
#
# @version 1.0.0
# @license MIT
...
#
# This script installs the WIA-CORE-010 standard components:
# - CLI tool
# - TypeScript SDK
# - Documentation
################################################################################
...
print_header() {
    echo -e "${INDIGO}"
    echo "╔════════════════════════════════════════════════════════════════╗"
    echo "║     ⚠️  WIA-CORE-010: Universal Error Handling Installer       ║"
    echo "║                      Version 1.0.0                             ║"
    echo "╚════════════════════════════════════════════════════════════════╝"
    echo -e "${RESET}"
}
...
check_prerequisites() {
    ...
    if command -v grep &> /dev/null; then
        print_success "grep: installed"
    else
        print_warning "grep not found (required for CLI)"
        all_ok=false
    fi
...
install_cli() {
    ...
    local cli_path="$SCRIPT_DIR/cli/wia-core-010.sh"
    ...
    if [ -w "$install_dir" ]; then
        cp "$cli_path" "$install_dir/wia-core-010"
        print_success "Installed CLI to $install_dir/wia-core-010"
    else
        print_info "Installing to $install_dir requires sudo..."
        sudo cp "$cli_path" "$install_dir/wia-core-010"
        print_success "Installed CLI to $install_dir/wia-core-010 (with sudo)"
    fi

    # Verify installation
    if command -v wia-core-010 &> /dev/null; then
        print_success "CLI tool is now available as 'wia-core-010'"
        print_info "Try: wia-core-010 --help"
    else
...
install_docs() {
    ...
    local docs_dir="$HOME/.wia/core-010/docs"
    ...
    if command -v wia-core-010 &> /dev/null; then
        wia-core-010 docs --output "$docs_dir" &> /dev/null || true
        print_success "Generated error code reference"
    fi
...
run_tests() {
    ...
    if command -v wia-core-010 &> /dev/null; then
        ...
        wia-core-010 version &> /dev/null && print_success "CLI test passed" || print_error "CLI test failed"

        print_info "Testing error code validation..."
        wia-core-010 validate --code "WIA-CORE-AUTH-001" &> /dev/null && print_success "Validation test passed" || print_warning "Validation test failed"
    fi
...
show_completion() {
    ...
    echo ""
    echo -e "${GREEN}The WIA-CORE-010 standard has been successfully installed!${RESET}"
    echo ""
    echo "Available commands:"
    echo -e "${CYAN}  wia-core-010 validate${RESET}         - Validate error code format"
    echo -e "${CYAN}  wia-core-010 list${RESET}             - List error codes"
    echo -e "${CYAN}  wia-core-010 parse${RESET}            - Parse error from JSON"
    echo -e "${CYAN}  wia-core-010 test-recovery${RESET}    - Test error recovery"
    echo -e "${CYAN}  wia-core-010 docs${RESET}             - Generate documentation"
    echo -e "${CYAN}  wia-core-010 help${RESET}             - Show help message"
    echo ""
    echo "Quick start:"
    echo -e "${GRAY}  # Validate an error code${RESET}"
    echo -e "${CYAN}  wia-core-010 validate --code 'WIA-CORE-AUTH-001'${RESET}"
    echo ""
    echo -e "${GRAY}  # List all CORE error codes${RESET}"
    echo -e "${CYAN}  wia-core-010 list --domain CORE${RESET}"
    echo ""
    echo -e "${GRAY}  # Test recovery strategy${RESET}"
    echo -e "${CYAN}  wia-core-010 test-recovery --code 'WIA-CORE-NETWORK-200'${RESET}"
    echo ""
    echo "TypeScript SDK:"
    echo -e "${GRAY}  import { createError, ErrorHandler } from '@wia/core-010';${RESET}"
    echo ""
    echo "Documentation:"
    echo -e "${GRAY}  $HOME/.wia/core-010/docs/${RESET}"
    echo ""
