...
################################################################################
# WIA-BIO-006: Tissue Engineering - Installation Script
#
# @version 1.0.0
# @license MIT
...
#
# This script installs the WIA-BIO-006 standard components:
# - CLI tool
# - TypeScript SDK
# - Documentation
################################################################################
...
print_header() {
    echo -e "${TEAL}"
    echo "╔════════════════════════════════════════════════════════════════╗"
    echo "║      🧫 WIA-BIO-006: Tissue Engineering Installer            ║"
    echo "║                      Version 1.0.0                             ║"
    echo "╚════════════════════════════════════════════════════════════════╝"
    echo -e "${RESET}"
}
...
install_cli() {
    ...
    local cli_path="$SCRIPT_DIR/cli/wia-bio-006.sh"
    ...
    if [ -w "$install_dir" ]; then
        cp "$cli_path" "$install_dir/wia-bio-006"
        print_success "Installed CLI to $install_dir/wia-bio-006"
    else
        print_info "Installing to $install_dir requires sudo..."
        sudo cp "$cli_path" "$install_dir/wia-bio-006"
        print_success "Installed CLI to $install_dir/wia-bio-006 (with sudo)"
    fi

    # Verify installation
    if command -v wia-bio-006 &> /dev/null; then
        print_success "CLI tool is now available as 'wia-bio-006'"
        print_info "Try: wia-bio-006 --help"
    else
...
install_docs() {
    ...
    local docs_dir="$HOME/.wia/bio-006/docs"
...
run_tests() {
    ...
    if command -v wia-bio-006 &> /dev/null; then
        ...
        wia-bio-006 version &> /dev/null && print_success "CLI test passed" || print_error "CLI test failed"
    fi
...
show_completion() {
    ...
    echo ""
    echo -e "${GREEN}The WIA-BIO-006 standard has been successfully installed!${RESET}"
    echo ""
    echo "Available commands:"
    echo -e "${CYAN}  wia-bio-006 design-scaffold${RESET}      - Design tissue engineering scaffold"
    echo -e "${CYAN}  wia-bio-006 optimize-culture${RESET}     - Optimize bioreactor conditions"
    echo -e "${CYAN}  wia-bio-006 assess-mechanics${RESET}     - Assess mechanical properties"
    echo -e "${CYAN}  wia-bio-006 validate-biocompat${RESET}   - Validate biocompatibility"
    echo -e "${CYAN}  wia-bio-006 generate-protocol${RESET}    - Generate tissue engineering protocol"
    echo -e "${CYAN}  wia-bio-006 help${RESET}                 - Show help message"
    echo ""
    echo "Quick start:"
    echo -e "${GRAY}  # Design a cartilage scaffold${RESET}"
    echo -e "${CYAN}  wia-bio-006 design-scaffold --tissue cartilage --material pcl --porosity 75${RESET}"
    echo ""
    echo -e "${GRAY}  # Optimize culture conditions${RESET}"
    echo -e "${CYAN}  wia-bio-006 optimize-culture --cell-type chondrocytes --duration 21${RESET}"
    echo ""
    echo -e "${GRAY}  # Validate biocompatibility${RESET}"
    echo -e "${CYAN}  wia-bio-006 validate-biocompat --material collagen --viability 85${RESET}"
    echo ""
    echo "TypeScript SDK:"
    echo -e "${GRAY}  import { designScaffold } from '@wia/bio-006';${RESET}"
    echo ""
    echo "Documentation:"
    echo -e "${GRAY}  $HOME/.wia/bio-006/docs/${RESET}"
    echo ""
