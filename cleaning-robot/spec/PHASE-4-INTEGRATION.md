# WIA-ROB-011 Phase 4: Integration Specification
  ...
  "manifest": {
    "publishingInformation": {
      "locales": {
        "en-US": {
          "name": "Cleaning Robot",
          "summary": "Control your WIA cleaning robot"
        }
      },
      "category": "SMART_HOME"
    },
...
metadata {
  definition (
    name: "WIA Cleaning Robot",
    namespace: "wia",
    author: "WIA"
  ) {
    capability "Robot Vacuum"
    capability "Battery"
    capability "Actuator"
    capability "Sensor"
  }
...
```http
POST /fleet/robots
  - Register robot to fleet

GET /fleet/robots
  - List all robots

PUT /fleet/robots/{id}/assign
  ...
  "zones": [
    {
      ...
      "assignedRobots": ["robot_id1", "robot_id2"],
      "schedule": {
        "frequency": "daily",
        "time": "02:00"
      },
...
**Algorithm:**
1. Calculate workload per robot
...
**MQTT Topics:**
```
$aws/things/{robot_id}/shadow/update
$aws/things/{robot_id}/shadow/get
```
...
```
Registry: wia-cleaning-robots
Device ID: ROB-2025-XXXX
Cloud Pub/Sub Topic: robot-telemetry
```
...
```typescript
import { WiaCleaningRobot } from 'wia-rob-011';

const robot = new WiaCleaningRobot({
  host: '192.168.1.100',
...
await robot.startCleaning({ mode: 'auto' });
```
