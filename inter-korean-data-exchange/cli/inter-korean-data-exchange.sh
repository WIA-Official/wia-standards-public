#!/usr/bin/env bash
# inter-korean-data-exchange.sh — minimal POSIX shell client for the WIA inter-korean-data-exchange standard.
# Demonstrates the request/response contract documented under spec/.
# Requires: curl, jq.
#
# Environment variables:
#   WIA_API_BASE   default https://inter-korean-data-exchange.example.com/api/v1
#   WIA_API_TOKEN  bearer token (required for write operations)

set -euo pipefail

API_BASE="${WIA_API_BASE:-https://inter-korean-data-exchange.example.com/api/v1}"
TOKEN="${WIA_API_TOKEN:-}"
