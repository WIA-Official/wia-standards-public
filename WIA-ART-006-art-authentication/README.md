# 🔐 WIA-ART-006: Art Authentication Standard
...
cd wia-standards/standards/WIA-ART-006-art-authentication
```
...
Provenance and authenticity verification standards
...
- **Standardized Formats**: Universal data formats for art authentication
...
Standardized formats for art authentication assets ensuring compatibility and preservation.
...
```typescript
import { ArtAuthenticationSDK } from '@wia/art-006';

const sdk = new ArtAuthenticationSDK({
  apiKey: 'your-api-key'
});
...
```bash
# Create new asset
wia-art-006 create --title "My Asset"

# Validate asset
wia-art-006 validate asset.json

# Export asset
wia-art-006 export --format json
```

### TypeScript SDK

```bash
npm install @wia/art-006
```
