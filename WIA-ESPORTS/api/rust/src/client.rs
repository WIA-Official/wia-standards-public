//! HTTP client and API implementations for the WIA-ESPORTS SDK
//!
...
impl EsportsClient {
    /// Create a new WIA-ESPORTS client
    pub fn new(config: Config) -> Self {
        let base_url = config
            .base_url
            .clone()
            .unwrap_or_else(|| "https://api.wiastandards.com/WIA-ESPORTS".to_string());
    ...
    pub async fn get(&self, id: &str) -> Result<EsportsData> {
        ...
        let result: EsportsResponse = response.json().await?;
        result.data.ok_or_else(|| Error::NotFound(id.to_string()))
    }

    /// Create new data
    pub async fn create(&self, request: CreateEsportsRequest) -> Result<EsportsData> {
    ...
    pub async fn list(&self, params: Option<ListParams>) -> Result<EsportsList> {
        let url = format!("{}/data", self.base_url);
