# WIA-tools PHASE 2 — API-INTERFACE Specification
...
`urn:wia:tools:status-transition`.
...
`422` with type
`urn:wia:tools:source-mix-required`.
...
with type
`urn:wia:tools:laboratory-not-accredited`.
...
`409` with type
`urn:wia:tools:quality-certificate-required`.
...
- `urn:wia:tools:hydrogen-quality-out-of-grade`
- `urn:wia:tools:cryogenic-temperature-excursion`
- `urn:wia:tools:guarantee-of-origin-mismatch`
- `urn:wia:tools:evidence-mismatch`
