...
################################################################################
# WIA-QUA-008: Plasma Technology - Installation Script
#
# @version 1.0.0
# @license MIT
...
#
# This script installs the WIA-QUA-008 standard components:
# - CLI tool
# - TypeScript SDK
# - Documentation
################################################################################
...
print_header() {
    echo -e "${INDIGO}"
    echo "╔════════════════════════════════════════════════════════════════╗"
    echo "║       🔮 WIA-QUA-008: Plasma Technology Installer            ║"
    echo "║                      Version 1.0.0                             ║"
    echo "╚════════════════════════════════════════════════════════════════╝"
    echo -e "${RESET}"
}
...
install_cli() {
    ...
    local cli_path="$SCRIPT_DIR/cli/wia-qua-008.sh"
    ...
    if [ -w "$install_dir" ]; then
        cp "$cli_path" "$install_dir/wia-qua-008"
        print_success "Installed CLI to $install_dir/wia-qua-008"
    else
        print_info "Installing to $install_dir requires sudo..."
        sudo cp "$cli_path" "$install_dir/wia-qua-008"
        print_success "Installed CLI to $install_dir/wia-qua-008 (with sudo)"
    fi

    # Verify installation
    if command -v wia-qua-008 &> /dev/null; then
        print_success "CLI tool is now available as 'wia-qua-008'"
        print_info "Try: wia-qua-008 --help"
    else
...
install_docs() {
    ...
    local docs_dir="$HOME/.wia/qua-008/docs"
...
run_tests() {
    ...
    if command -v wia-qua-008 &> /dev/null; then
        ...
        wia-qua-008 version &> /dev/null && print_success "CLI test passed" || print_error "CLI test failed"
    fi
...
show_completion() {
    ...
    echo ""
    echo -e "${GREEN}The WIA-QUA-008 standard has been successfully installed!${RESET}"
    echo ""
    echo "Available commands:"
    echo -e "${CYAN}  wia-qua-008 debye-length${RESET}      - Calculate Debye length"
    echo -e "${CYAN}  wia-qua-008 generate-plasma${RESET}   - Simulate plasma generation"
    echo -e "${CYAN}  wia-qua-008 fusion-power${RESET}      - Calculate fusion reactor power"
    echo -e "${CYAN}  wia-qua-008 diagnostics${RESET}       - Run plasma diagnostics"
    echo -e "${CYAN}  wia-qua-008 propulsion${RESET}        - Calculate thruster performance"
    echo -e "${CYAN}  wia-qua-008 help${RESET}              - Show help message"
    echo ""
    echo "Quick start:"
    echo -e "${GRAY}  # Calculate Debye length${RESET}"
    echo -e "${CYAN}  wia-qua-008 debye-length --density 1e18 --temperature 10000${RESET}"
    echo ""
    echo -e "${GRAY}  # Simulate tokamak fusion${RESET}"
    echo -e "${CYAN}  wia-qua-008 fusion-power --reactor tokamak --density 1e20 --temp 150e6${RESET}"
    echo ""
    echo "TypeScript SDK:"
    echo -e "${GRAY}  import { calculateDebyeLength } from '@wia/qua-008';${RESET}"
    echo ""
    echo "Documentation:"
    echo -e "${GRAY}  $HOME/.wia/qua-008/docs/${RESET}"
    echo ""
