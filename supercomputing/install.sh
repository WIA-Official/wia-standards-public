...
################################################################################
# WIA-COMP-001: Supercomputing - Installation Script
#
# @version 1.0.0
# @license MIT
...
#
# This script installs the WIA-COMP-001 standard components:
# - CLI tool
# - TypeScript SDK
# - Documentation
################################################################################
...
print_header() {
    echo -e "${BLUE}"
    echo "╔════════════════════════════════════════════════════════════════╗"
    echo "║       🚀 WIA-COMP-001: Supercomputing Installer              ║"
    echo "║                      Version 1.0.0                             ║"
    echo "╚════════════════════════════════════════════════════════════════╝"
    echo -e "${RESET}"
}
...
install_cli() {
    ...
    local cli_path="$SCRIPT_DIR/cli/wia-comp-001.sh"
    ...
    if [ -w "$install_dir" ]; then
        cp "$cli_path" "$install_dir/wia-comp-001"
        print_success "Installed CLI to $install_dir/wia-comp-001"
    else
        print_info "Installing to $install_dir requires sudo..."
        sudo cp "$cli_path" "$install_dir/wia-comp-001"
        print_success "Installed CLI to $install_dir/wia-comp-001 (with sudo)"
    fi

    # Verify installation
    if command -v wia-comp-001 &> /dev/null; then
        print_success "CLI tool is now available as 'wia-comp-001'"
        print_info "Try: wia-comp-001 --help"
    else
...
install_docs() {
    ...
    local docs_dir="$HOME/.wia/comp-001/docs"
...
run_tests() {
    ...
    if command -v wia-comp-001 &> /dev/null; then
        ...
        wia-comp-001 version &> /dev/null && print_success "CLI test passed" || print_error "CLI test failed"
    fi
...
show_completion() {
    ...
    echo ""
    echo -e "${GREEN}The WIA-COMP-001 Supercomputing standard has been successfully installed!${RESET}"
    echo ""
    echo "Available commands:"
    echo -e "${CYAN}  wia-comp-001 calc-flops${RESET}           - Calculate system FLOPS"
    echo -e "${CYAN}  wia-comp-001 validate-cluster${RESET}     - Validate cluster config"
    echo -e "${CYAN}  wia-comp-001 benchmark${RESET}            - Run performance benchmark"
    echo -e "${CYAN}  wia-comp-001 analyze-scalability${RESET}  - Analyze scalability"
    echo -e "${CYAN}  wia-comp-001 help${RESET}                 - Show help message"
    echo ""
    echo "Quick start:"
    echo -e "${GRAY}  # Calculate exascale system FLOPS${RESET}"
    echo -e "${CYAN}  wia-comp-001 calc-flops --nodes 10000 --cores 128 --gpus 4${RESET}"
    echo ""
    echo -e "${GRAY}  # Validate fat-tree topology${RESET}"
    echo -e "${CYAN}  wia-comp-001 validate-cluster --topology fat-tree --nodes 1000${RESET}"
    echo ""
    echo -e "${GRAY}  # Run LINPACK benchmark${RESET}"
    echo -e "${CYAN}  wia-comp-001 benchmark --type LINPACK --nodes 1000${RESET}"
    echo ""
    echo "TypeScript SDK:"
    echo -e "${GRAY}  import { calculateFLOPS } from '@wia/comp-001';${RESET}"
    echo ""
    echo "Documentation:"
    echo -e "${GRAY}  $HOME/.wia/comp-001/docs/${RESET}"
    echo ""
